...
!  use MAPL,              only: MAPL_UNDEF
 ...
 use MAPL_SatVaporMod,  only: MAPL_EQsat, set_EST_arrays, MAPL_EQsatSET
 use MAPL_SatVaporMod,  only: TableReady
...
! NOTE : Ignoring pointer-based arrays for now in OpenACC data portion

!$acc data present(phii_inv,phil_inv,tkh_inv,dm_inv,prsl_inv,u_inv,v_inv,omega_inv,&
!$acc              tabs_inv,qwv_inv,qc_inv,qi_inv,cld_sgs_inv,tke_inv,wthv_sec_inv,&
!$acc              prnum,shocparams,isotropy_inv)&
!$acc      create(zi,zl,tkh,dm,prsl,u,v,omega,tabs,qwv,qcl,qci,tke,wthv_sec,&
!$acc             qv,thv,w,qpl,qpi,total_water,bet,gamaz,hl,hvl,adzi,adzl,isotropy,&
!$acc             cld_sgs,tkesbdiss,tkesbbuoy,tkesbshear,tkesbtrans,smixt,smixt1,&
!$acc             smixt2,smixt3,smixt_incld,smixt_outcld,brunt,conv_vel2,brunt2,&
!$acc             def2,brunt_smooth,denom,numer,l_inf,cldarr,zcb,qsatt,dtqw,dtqi)

! Map GEOS variables to those of SHOC
!$acc parallel
!$acc loop gang vector collapse(3) private(kinv)
  do k=1,nz
    do j=1,ny
      do i=1,nx
        kinv = nz-k+1
...
! NOTE : Cannot connect these two parallel sections without error in C90 solution

!!!Changes precision
!$acc parallel
!$acc loop gang vector collapse(3) private(wrk,prespot)
  do k=1,nzm
    do j=1,ny
      do i=1,nx
        wrk          = 1.0 / prsl(i,j,k)
...
!$acc loop gang vector collapse(3)
  do k=2,nzm
    do j=1,ny
      do i=1,nx
        adzi(i,j,k)   = (zl(i,j,k) - zl(i,j,k-1))
        adzl(i,j,k-1) = (zi(i,j,k) - zi(i,j,k-1))   ! from 1 to nzm-1
      enddo
    enddo
  enddo
!$acc end parallel
...
!$acc loop gang vector collapse(3)
  do k=1,nzm
    do j=1,ny
      do i=1,nx
        tkh_inv(i,j,k)       = tkh(i,j,nzm-k+1)
        isotropy_inv(i,j,k)  = isotropy(i,j,nzm-k+1)
        tke_inv(i,j,k)       = tke(i,j,nzm-k+1)
      enddo
    enddo
  enddo
!$acc end parallel
...
!  w3_canuto_inv(:,:,1:nzm) = w3(:,:,nzm:1:-1)
  ...
  if (associated(tkesbdiss_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          tkesbdiss_inv(i,j,k)  = tkesbdiss(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  endif
  if (associated(tkesbbuoy_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
        tkesbbuoy_inv(i,j,k)  = tkesbbuoy(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  endif
  if (associated(tkesbshear_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          tkesbshear_inv(i,j,k) = tkesbshear(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  endif
  if (associated(tkesbtrans_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          tkesbtrans_inv(i,j,k) = tkesbtrans(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  ...
  if (associated(smixt_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          smixt_inv(i,j,k)    = smixt(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  endif
  if (associated(smixt1_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          smixt1_inv(i,j,k)   = smixt1(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  endif
  if (associated(smixt2_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          smixt2_inv(i,j,k)   = smixt2(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  endif
  if (associated(smixt3_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          smixt3_inv(i,j,k)   = smixt3(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  endif
  if (associated(smixt_oc_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          smixt_oc_inv(i,j,k) = smixt_outcld(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  endif
  if (associated(smixt_ic_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          smixt_ic_inv(i,j,k) = smixt_incld(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  ...
  if (associated(brunt_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          brunt_inv(i,j,k)    = brunt2(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
  endif
  if (associated(shear_inv)) then
!$acc parallel
!$acc loop gang vector collapse(3)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
         shear_inv(i,j,k)    = def2(i,j,nzm-k+1)
        enddo
      enddo
    enddo
!$acc end parallel
...
! Heavily based on SAM's tke_full.f90 by Marat Khairoutdinov
    ...
    real grd,betdz,Cek,Cee,lstarn, lstarp, bbb, omn, omp,dqsat, smix,         &
...
!$acc data present(adzi,u,v,def2,tke,tkesbdiss,tkesbshear,tkesbbuoy,tkesbtrans, &
!$acc              qcl,qci,tabs,prsl,dtqw,dtqi,bet,qpl,qpi,brunt,cld_sgs,hl,total_water,&
!$acc              qv,brunt2,brunt_smooth,zl,shocparams%SUS12LEN,l_inf,smixt1,smixt2,&
!$acc              smixt3,smixt,smixt_outcld,cldarr,wthv_sec,smixt_incld,shocparams%CLDLEN,&
!$acc              numer,denom,thv,adzl,tkh,prnum,isotropy) &
!$acc       create(qsatt)
...
!$acc parallel
!$acc loop gang vector collapse(3) private(ku,kd,Cek,grd,a_prod_bu,wrk,buoy_sgs,smix,&
!$acc      ratio,Cee,a_prod_sh,wtke,wtk2,wrk1,a_diss,tscale1)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
...
!$acc data present(adzi,u,v,def2) create(wrku,wrkv)
...
!!!Changes precision
!$acc parallel
!$acc loop gang vector collapse(3) private(rdzw_up,wrku,wrkv,rdzw_dn)
    do k=1,nzm      
      do j=1,ny
        do i=1,nx
          if (k == 1) then           
            rdzw_up     = 1./adzi(i,j,k+1)
            wrku(1)     = (u(i,j,k+1)-u(i,j,k))*rdzw_up
            wrkv(1)     = (v(i,j,k+1)-v(i,j,k))*rdzw_up
!           wrkw(1)     = (w(i,j,k+1)-w(i,j,k))*rdzw_up
            def2(i,j,1) = wrku(1)*wrku(1) + wrkv(1)*wrkv(1) !+ 2*wrkw(1) * wrkw(1)
            ! txd(i,j)    = rdzw_up             
          elseif (k < nzm ) then
            rdzw_up     = 1./adzi(i,j,k+1)
            rdzw_dn     = 1./adzi(i,j,k)
            wrku(1)     = (u(i,j,k+1)-u(i,j,k))*rdzw_up
            wrku(2)     = (u(i,j,k)-u(i,j,k-1))*rdzw_dn
            wrkv(1)     = (v(i,j,k+1)-v(i,j,k))*rdzw_up
            wrkv(2)     = (v(i,j,k)-v(i,j,k-1))*rdzw_dn
!           wrkw(1)     = (w(i,j,k+1)-w(i,j,k))*rdzw_up
!           wrkw(2)     = (w(i,j,k)-w(i,j,k-1))*rdzw_dn
...
!$acc data present(qcl,qci,qsatt,tabs,prsl,dtqw,dtqi,adzi,bet,tke,qpl,qpi,brunt,cld_sgs,hl,total_water,&
!$acc              qv,brunt2,brunt_smooth,zl,shocparams%SUS12LEN,l_inf,smixt1,smixt2,&
!$acc              smixt3,smixt,smixt_outcld,cldarr,wthv_sec,smixt_incld,shocparams%CLDLEN,&
!$acc              numer,denom,thv,adzl) 
...
!$acc parallel
!$acc loop gang vector collapse(3) private(kb,kc,thedz,betdz) 
    do k=1,nzm
      do j=1,ny
        do i=1,nx
...
!$acc loop seq
    do k=1,nzm
!$acc loop gang vector collapse(2) private(tkes)
      do j=1,ny
        do i=1,nx
...
!$acc loop gang vector collapse(2)
      do j = 1,ny
        do i = 1,nx
          ! smooth 3-layers of brunt freq to reduce influence of single layers
          brunt_smooth(i,j,k) = (brunt_smooth(i,j,k-2) + brunt_smooth(i,j,k-1) + brunt_smooth(i,j,k)) / 3.0
        enddo
      enddo
    enddo
!$acc end parallel
...
!$acc parallel
!$acc loop gang vector collapse(3) private(wrk,omn)
    do k=1,nzm
! NOTE : Currently, collapsing and running the i and j loop in parallel does not produce the correct solution
!        due to a race condition when calling MAPL_EQsat.
!        Running these two loops serially will produce the proper solution
      do j=1,ny
        do i=1,nx
          wrk = qcl(i,j,k) + qci(i,j,k)
          omn = qcl(i,j,k) / (wrk+1.e-20)
! NOTE : I suspect that calling the MAPL_EQsat function in parallel leads to race conditions
          qsatt(i,j,k) =     omn  * MAPL_EQsat(tabs(i,j,k),prsl(i,j,k),dtqw(i,j,k))                &
                + (1.-omn) * MAPL_EQsat(tabs(i,j,k),prsl(i,j,k),dtqi(i,j,k),OverIce=.TRUE.)
        enddo
      enddo
    enddo
!$acc end parallel
...
!$acc parallel
!$acc loop gang vector collapse(3) private(kb,kc,thedz,betdz,tkes,wrk,omn,lstarn,dqsat,bbb)
    do k=1,nzm
! NOTE : Currently, collapsing and running the i and j loop in parallel does not produce the correct solution
!        due to a race condition when calling MAPL_EQsat.
!        Running these two loops serially will produce the proper solution
...
!               + (1.-omn) * dtqsati(tabs(i,j,k),prsl(i,j,k))
          dqsat =  omn * dtqw(i,j,k) + (1.-omn) * dtqi(i,j,k)

! liquid/ice moist static energy static energy divided by cp?
          bbb = (1. + epsv*qsatt(i,j,k)-wrk-qpl(i,j,k)-qpi(i,j,k)                &
...
!!!Changes precision
!$acc parallel
!$acc loop gang vector collapse(3) private(kk,zdecay,ktop,wrk1,wrk2,wrk3,tkes)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          tkes = sqrt(tke(i,j,k))
...
!$acc parallel
!$acc loop gang vector collapse(2) private(kl,ku,wrk,conv_var,depth)
    do j=1,ny
      do i=1,nx
...
!$acc loop gang vector collapse(2)
    ...
    enddo
!$acc end parallel
!$acc update host(conv_vel2)

!$acc update device(conv_vel2,adzi,bet,wthv_sec)
!$acc parallel
!$acc loop seq
! Integrate velocity scale in the vertical
    do k=2,nzm
...
!**********************************************************************
    ...
    enddo
!$acc end parallel
!$acc update host(conv_vel2,adzi,bet,wthv_sec)
...
!$acc data present(adzl,smixt,qcl,brunt)

!$acc parallel
!$acc loop gang vector collapse(3) private(kb,wrk)
    do k=1,nzm
      do j=1,ny
        do i=1,nx
          if (k == nzm) then
