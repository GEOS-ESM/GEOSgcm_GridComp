MODULE ConvPar_GF_SharedParams
    ...
    use MAPL_ConstantsMod
     ...
     CONTAINS
      ...
      subroutine get_incloud_sc_chem_up(cumulus_1,mtp,se,se_cup,sc_up,pw_up,tot_pw_up_chem&
                                       ,z_cup,rho,po,po_cup  &
    ...
    !    real, parameter :: kc = 5.e-3  ! s-1
         ...
         do i=its,itf
           ...
           do ispc=1,mtp
                 call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),se_cup(ispc,i,kts:kte),sc_b(ispc,i),k22(i))
           enddo
           do k=kts,k22(i)
                sc_up   (:,i,k) = sc_b  (:,i)
           enddo
    ...
    loopk:      do k=k22(i)+1,ktop(i)+1
                ...
                !-- scavenging section
                if(USE_TRACER_SCAVEN==0 .or. cumulus_1 == 's') cycle loopk
    ...
    !---------------------------------------------------------------------------------------------------
      subroutine get_incloud_sc_chem_dd(cumulus_1,mtp,se,se_cup,sc_dn,pw_dn ,pw_up,sc_up                          &
    ...
    !---------------------------------------------------------------------------------------------------
       subroutine get_cloud_bc(cumulus_1,kts,kte,ktf,xland,po,array,x_aver,k22,add,Tpert)
        implicit none
        character,intent (in)         :: cumulus_1
        ...
        !-- order_aver = kts !=> average between k22, k22-1 and k22-2
         ...
         elseif(bc_meth == 1) then
          ...
          if(cumulus_1 == 'd'   ) x_ave_layer = cum_ave_layer(deep)*effec_frac
          if(cumulus_1 == 's') x_ave_layer = cum_ave_layer(shal)*effec_frac
          if(cumulus_1 == 'm'    ) x_ave_layer = cum_ave_layer(mid )*effec_frac
