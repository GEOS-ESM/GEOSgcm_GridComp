MODULE ConvPar_GF_SharedParams
    ...
    use MAPL_ConstantsMod
     ...
     CHARACTER(len=10) :: GF_ENV_SETTING = 'DYNAMICS' ! or 'CURRENT'
      ...
      ! *** CK : Added in to account for CNV_Tracer_Type ***
...
!$acc declare create(BC_METH, cum_ave_layer)
     CONTAINS
    ...
    !---------------------------------------------------------------------------------------------------
      subroutine get_cloud_bc(cumulus,kts,kte,ktf,xland,po,array,x_aver,k22,add,Tpert)
         ...
         real                          :: count,dp,dp_layer,effec_frac,x_ave_layer,test
         ...
         elseif(bc_meth == 1) then
            ! Note : This branch didn't get tested with verification data.  This branch was tested by
            !        setting bc_meth to 1 and comparing the sum of errors based on verification data
            !        based on building with the Intel compiler
            effec_frac  = (1.-xland) +xland*frac_ave_layer_ocean

            ! Note : nvfortran compiler with OpenACC does not know how to handle the "trim" function
            ...
            if(cumulus(1:4) == 'deep'   ) x_ave_layer = cum_ave_layer(deep)*effec_frac
            if(cumulus(1:7) == 'shallow') x_ave_layer = cum_ave_layer(shal)*effec_frac
            if(cumulus(1:3) == 'mid'    ) x_ave_layer = cum_ave_layer(mid )*effec_frac
    
            ! Note : nvfortran compiler with OpenACC does not know how to handle the "minloc" function
            !  i_beg = minloc(abs(po(kts:ktf)-(po(k22)+0.5*x_ave_layer)),1)
            !  i_end = minloc(abs(po(kts:ktf)-(po(k22)-0.5*x_ave_layer)),1)

            i_beg = minloc_acc(abs(po(kts:ktf)-(po(k22)+0.5*x_ave_layer)))
            i_end = minloc_acc(abs(po(kts:ktf)-(po(k22)-0.5*x_ave_layer)))
       ...
       function maxloc_acc(oneD_arr)
         !$acc routine seq
         implicit none
 
         integer :: i, maxloc_acc
         real    :: val
         real, dimension(:) :: oneD_arr
 
         val = -HUGE(0.0)
         maxloc_acc = 0
 
         do i = 1,size(oneD_arr)
             if(val < oneD_arr(i)) then
                 val = oneD_arr(i)
                 maxloc_acc = i
             endif
         enddo
     ...
     function minloc_acc(oneD_arr)
         !$acc routine seq
         implicit none
 
         integer :: i, minloc_acc
         real    :: val
         real, dimension(:) :: oneD_arr
