...
!--------------------------------------------------------------------------
  src_flag = .TRUE.
!$acc data present(band, pint, pmid, delp, u, v, t, sgh, zm, nm, &
!$acc              src_level, ubi, ubm, xv, yv, tau, &
!$acc              tend_level, c) &
!$acc      create(hdsp, tauoro, nsrc, rsrc, usrc, vsrc)

  k = pver

!$acc parallel copy(src_flag)
!$acc loop gang vector
  do i = 1,ncol
    hdsp(i) = 2.0_r8 * sgh(i)
    src_level(i) = k-1
    rsrc(i) = pmid(i,k)/(rair*t(i,k)) * delp(i,k)
    usrc(i) = u(i,k) * delp(i,k)
    vsrc(i) = v(i,k) * delp(i,k)
    nsrc(i) = nm(i,k)* delp(i,k)
  enddo
...
!$acc loop gang vector
    ...
    ! Set src_flag to False when all source levels found.
    if(src_flag) then
      src_sum = 0
!$acc loop gang vector reduction(+ : src_sum)
      do i = 1,ncol
        if(src_level(i) >= k) then
          src_sum = src_sum + 1
        endif
      enddo
      if(src_sum .eq. ncol) then
        src_flag = .FALSE.
      endif
    endif
  end do

!$acc loop gang vector private(dpsrc)
  do i = 1, ncol
    dpsrc = pint(i,pver+1) - pint(i,src_level(i)+1)
    rsrc(i) = rsrc(i) / dpsrc
    usrc(i) = usrc(i) / dpsrc
    vsrc(i) = vsrc(i) / dpsrc
    nsrc(i) = nsrc(i) / dpsrc
...
! NOTE : Cannot combine the above and below parallel sections together
...
!   ubi(:,2:pver) = midpoint_interp(ubm)
  ...
  call midpoint_interp_s(ubm, ubi(:,2:pver))
...
!$acc loop gang vector private(sghmax)
  do i = 1, ncol
...
!$acc data present(lats, flx_heat) &
    ...
    do l = -band%ngwv,band%ngwv
     do i = 1, ncol
        if (lats(i) < 0._r8) then
           tau(i,l,k) = tau(i,l,k) * gw_oro_south_fac
        end if
        enddo
     end do
     enddo
...
!$acc parallel loop gang vector present(flx_heat)
    do i = 1,ncol
     flx_heat(i) = 0._r8
     enddo
!$acc end parallel loop
