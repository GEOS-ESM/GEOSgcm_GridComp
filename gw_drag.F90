...
! !$acc data present(pint_dev, t_dev, u_dev, v_dev, ht_dpc_dev, sgh_dev, &
! !$acc              pref_dev, pmid_dev, pdel_dev, rpdel_dev, lnpint_dev, &
! !$acc              zm_dev, rlat_dev, dudt_gwd_dev, dvdt_gwd_dev, dtdt_gwd_dev, &
! !$acc              dudt_org_dev, dvdt_org_dev, dtdt_org_dev, taugwdx_dev, taugwdy_dev, &
! !$acc              feo_dev, fepo_dev, taubkgx_dev, tauby_dev, feb_dev, fepb_dev, &
! !$acc              utbsrc_dev, vtbsrc_dev, ttbsrc_dev) &
! !$acc       create(src_level, tend_level, ttgw, utgw, vtgw, &
! !$acc              zi, ni, nm, rhoi, tau0x, tau0y, &
! !$acc              ti, ubi, ubm, xv, yv, kvtt, &
! !$acc              utosrc, vtosrc, ttosrc, maxq0, hdepth, &
! !$acc              flx_heat, &
! !$acc              pint_dev_ff, t_dev_ff, u_dev_ff, v_dev_ff, &
! !$acc              ht_dpc_dev_ff, sgh_dev_ff, pref_dev_ff, pmid_dev_ff, &
! !$acc              pdel_dev_ff, rpdel_dev_ff, lnpint_dev_ff, zm_dev_ff, &
! !$acc              rlat_dev_ff, t_gwt_dc_ff, u_gwt_dc_ff, v_gwt_dc_ff, &
! !$acc              t_gwt_org_ff, u_gwt_org_ff, v_gwt_org_ff, &
! !$acc              t_gwt_ff, u_gwt_ff, v_gwt_ff)
...
!          pmid_dev_ff(i,k)   =  pmid_dev(i,k)
!          pdel_dev_ff(i,k)   =  pdel_dev(i,k)
!          rpdel_dev_ff(i,k)  =  rpdel_dev(i,k)

!          ! Met profiles u,v,t,zm
!          !-----------------------
!          t_dev_ff(i,k)      =  t_dev(i,k)
!          u_dev_ff(i,k)      =  u_dev(i,k)
!          v_dev_ff(i,k)      =  v_dev(i,k)
!          zm_dev_ff(i,k)     =  zm_dev(i,k)

!          ! Heating 
!          !----------
!          ht_dpc_dev_ff(i,k)  =  ht_dpc_dev(i,k)
!       enddo
!    enddo
...
!       do i = 1, pcols
!          kvtt(i,k)  = 0._r8
!          pint_dev_ff(i,k)   =  pint_dev(i,k)
!          lnpint_dev_ff(i,k) =  lnpint_dev(i,k)
!       enddo
!    enddo
! !$acc end parallel
...
!    do k = 1,pver+1
!       pref_dev_ff(k)   =  pref_dev(k)
!    enddo

! !$acc loop gang vector 
!    do i = 1,pcols
!       rlat_dev_ff(i)   =  rlat_dev(i)
!       ! SGH
!       !----------
!       sgh_dev_ff(i)  =  sgh_dev(i)
...
!    do k=2,pver 
!       do i = 1,pcols
!        zi(i,k)  =  0.5 * ( zm_dev_ff(i,k-1) + zm_dev_ff(i,k) )  
!       end do
!    enddo
! !$acc end parallel
...
! !$acc loop gang vector
!    do i = 1, pcols
!       zi(i,1) = zi(i,2) + 0.5*( zm_dev_ff(i,1) - zm_dev_ff(i,2)  )
!       zi(i,pver+1) = 0.0
!    enddo
! !$acc end parallel
...
!    do k = 1, pver
!       do i = 1, pcols
!          u_gwt_ff(i,k) = u_gwt_ff(i,k) + u_gwt_dc_ff(i,k)
!          v_gwt_ff(i,k) = v_gwt_ff(i,k) + v_gwt_dc_ff(i,k)
!          t_gwt_ff(i,k) = t_gwt_ff(i,k) + t_gwt_dc_ff(i,k)
!       enddo
!    enddo
! !$acc end parallel
...
!    do k = 1, pver
!       do i = 1, pcols
!          u_gwt_ff(i,k) = u_gwt_ff(i,k) + u_gwt_org_ff(i,k)
!          v_gwt_ff(i,k) = v_gwt_ff(i,k) + v_gwt_org_ff(i,k)
!          t_gwt_ff(i,k) = t_gwt_ff(i,k) + t_gwt_org_ff(i,k)
!       enddo
!    enddo
...
!    do k = 1,pver
!       do i = 1,pcols
!          dudt_gwd_dev(i,k) = REAL( u_gwt_ff(i,k))  !zonal wind tendency at layer 
!          dvdt_gwd_dev(i,k) = REAL( v_gwt_ff(i,k))  !meridional wind tendency at layer 
!          dtdt_gwd_dev(i,k) = REAL( t_gwt_ff(i,k))  !temperature tendency at layer
!          dudt_org_dev(i,k) = REAL( u_gwt_org_ff(i,k))  !zonal wind tendency at layer due to orography GWD
!          dvdt_org_dev(i,k) = REAL( v_gwt_org_ff(i,k))  !meridional wind tendency at layer  due to orography GWD
!          dtdt_org_dev(i,k) = REAL( t_gwt_org_ff(i,k))  !temperature tendency at layer  due to orography GWD

!          utbsrc_dev(i,k)   = 0.0  !dU/dt below background launch level
!          vtbsrc_dev(i,k)   = 0.0  !dV/dt below background launch level
!          ttbsrc_dev(i,k)   = 0.0  !dT/dt below background launch level
!       enddo
!    enddo
...
!    do k = 1, pver+1
!       do i = 1,pcols
!          tauox_dev(i,k)  = 0.0  !zonal      orographic gravity wave stress
!          tauoy_dev(i,k)  = 0.0  !meridional orographic gravity wave stress
!          feo_dev  (i,k)  = 0.0  !energy flux of orographic gravity waves
!          fepo_dev (i,k)  = 0.0  !pseudoenergy flux of orographic gravity waves
!          taubx_dev(i,k)  = 0.0  !zonal      background gravity wave stress
!          tauby_dev(i,k)  = 0.0  !meridional background gravity wave stress
!          feb_dev  (i,k)  = 0.0  !energy flux of background gravity waves
!          fepb_dev (i,k)  = 0.0  !pseudoenergy flux of background gravity waves
!       enddo
!    enddo
! !$acc end parallel
...
! !$acc loop gang vector
!    do i = 1,pcols
!      taugwdx_dev(i)         = 0.0  !zonal      gravity wave surface    stress
!      taugwdy_dev(i)         = 0.0  !meridional gravity wave surface    stress
!      taubkgx_dev(i)         = 0.0  !zonal      gravity wave background stress
!      taubkgy_dev(i)         = 0.0  !meridional gravity wave background stress
!    enddo
! !$acc end parallel
