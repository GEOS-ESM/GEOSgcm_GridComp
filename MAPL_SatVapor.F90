...
!
    ...
    public set_EST_arrays ! Sets size of EST arrays
!
! !PUBLIC DATA MEMBERS:
!
...
! Tables and other Global variables
  ...
  logical              :: MXRT       = .false.
  integer              :: TYPE       =  1
  logical, public      :: TableReady = .false.

  integer              :: DEGSUBS    =  DEFAULT_SUBS ! subdivisions per deg K
  integer              :: TABLESIZE  =  nint(TMAXTBL-TMINTBL)*DEFAULT_SUBS + 1
  real(kind=REAL64)    :: DELTA_T    =  1.0 / DEFAULT_SUBS

  real(kind=REAL64)    :: ESTFRZ
  real(kind=REAL64)    :: ESTLQU

  real(kind=REAL64), allocatable       :: ESTBLE(:)
  real(kind=REAL64), allocatable       :: ESTBLW(:)

  integer,   parameter :: WATER   = 1
  integer,   parameter :: ICE     = 2

  real(kind=REAL64) :: TMIN(2) = (/ TMINLQU, TMINICE /)
  real(kind=REAL64) :: TMAX(2) = (/ TMAXLQU, TMAXICE /) 

!$acc declare copyin(UTBL,MXRT,TYPE,TableReady,DEGSUBS,TABLESIZE,DELTA_T,ESTFRZ,ESTLQU,ESTBLE,ESTBLW,TMIN,TMAX)
...
!BOPI
  ...
  subroutine set_EST_arrays

    allocate(ESTBLE(TABLESIZE))
    allocate(ESTBLW(TABLESIZE))
!!!$acc enter data create(ESTBLE,ESTBLW)
...
!$acc loop gang vector private(T)
      do I=1,TABLESIZE
...
!!!         ESTBLE(I) = QSATD0(T,OverIce=.true. )
         ESTBLW(I) = QSATD0_1(T,OverIce=.false.)
         ESTBLE(I) = QSATD0_1(T,OverIce=.true. )
...
#include "qsatice.H"
  ...
  recursive function QSATD0_1(TL,PL,DQ,OverIce) result(QS)
!$acc routine seq
    real(kind=REAL64),              intent(IN) :: TL
    real(kind=REAL64), optional,    intent(IN) :: PL
    real(kind=REAL64), optional,    intent(OUT):: DQ
