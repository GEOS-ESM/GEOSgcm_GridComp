...
!$acc               z_slope_liq, z_slope_ice, use_ccn, use_ppm, mono_prof, mp_print, tables_are_initialized)
    ...
    subroutine sedi_heat_3d (is, ie, js, je, ktop, kbot, dm, m1, dz, tz, qv, ql, qr, qi, qs, qg, cw)
            ...
            !$acc data present(dm, m1, dz, qv, ql, qr, qi, qs, qg, tz) copyin(cw)

            k = ktop
            !$acc parallel loop gang vector collapse(2) private(cvn) copyin(k)
            do j = js, je
                do i = is, ie
            ...
            !$acc loop seq
            do k = ktop + 1, kbot
                !$acc loop gang vector collapse(2) private(dgz, cvn) 
                do j = js, je
                    do i = is, ie
    ...
    subroutine warm_rain (dt, ktop, kbot, dp, dz, tz, qv, ql, qr, qi, qs, qg, qa, &
        ...
        if (irain_f /= 0) then
            ...
            call linear_prof (kbot - ktop + 1, ql , dl , z_slope_liq, h_var)
            do k = ktop, kbot
                qc0 = fac_rc * ccn (k)
    ...
    subroutine warm_rain_3d (dt, is, ie, js, je, ktop, kbot, dp, dz, tz, qv, ql, qr, qi, qs, qg, qa, &
        eis, &
        den, denfac, ccn, c_praut, vtr, r1, evap1, m1_rain, w1, h_var)
    
        integer, intent (in) :: ktop, kbot, is, ie, js, je
        ...
        !$acc data create(dl, dm, revap, isubl, qadum, ze, zt, no_fall) copyin(zs, dt5, dt) &
        !$acc      present(h_var, dp, dz, den, denfac, ccn, c_praut, eis, tz, vtr, &
        !$acc              qv, ql, qr, qi, qs, qg, qa, evap1, m1_rain, w1, r1)
        ...
        call check_column_3d (qr, no_fall)
        ...
        if (irain_f /= 0) then
            ...
            ! no subgrid varaibility
            ! -----------------------------------------------------------------------
            !$acc parallel loop gang vector collapse(3) private(fac_rc, qc0, qc, dq, sink)
            do k = ktop, kbot
                do j = js, je
                    do i = is, ie
    ...
    subroutine revap_racc_3d (is, ie, js, je, ktop, kbot, dt, tz, qv, ql, qr, qi, qs, qg, qa, revap, den, denfac, h_var)
        ...
        !$acc               firstprivate(TOT_PREC_LS, AREA_LS_PRC) &
        !$acc               private(fac_revp, lhl, q_liq, q_sol, cvm, lcpk, tin, qpz, &
        !$acc                       qsat, dqh, dqv, q_minus, q_plus, dq, qden, t2, evap, &
        !$acc                       sink, dqsdt)
        do k = ktop, kbot
            do j = js, je
                do i = is, ie
    ...
    subroutine linear_prof_3d (im, jm, km, q, dm, z_var, h_var)
        ...
        if (z_var) then
            ...
            ! use twice the strength of the positive definiteness limiter (lin et al 1994)
            ! -----------------------------------------------------------------------
            !$acc parallel loop gang vector collapse(3) private(dq, dq_p1)
            do k = 2, km - 1
                do j = 1, jm
                    do i = 1, im
...
!*****
    ...
    subroutine icloud_3d (is, ie, js, je, ktop, kbot, tzk, p1, qvk, qlk, qrk, qik, qsk, qgk, dp1, &
        den, denfac, vts, vtg, vtr, qak, dts, subl1, h_var, ccn, cnv_fraction, srf_type)
        implicit none
        ...
        !$acc data create(di, cvm, q_liq, q_sol) copyin(rdts, fac_i2s, fac_g2v, fac_imlt, fac_frz, dts) &
        !$acc      present(p1, dp1, den, denfac, vts, vtg, vtr, tzk, qvk, qlk, qrk, qik, qsk, qgk, qak, &
        !$acc              subl1, cnv_fraction, srf_type, h_var, ccn)
    ...
    ! Note : If 'sink' gets added as a private variable, the code will not verify
    !*****
        !$acc parallel loop gang vector collapse(3) private(lhi, icpk, melt, tmp, sink, qi_crt)
        do k = ktop, kbot
            do j = js, je
                do i = is, ie
        ...
        do k = ktop, kbot
                ...
                do i = is, ie
                    ...
                    if (tc .ge. 0.) then
                        ...
                        if (qs > qpmin) then
                            ...
                            if (qr > qpmin) then
                                psacr = min (acr3d (vts (i,j,k), vtr (i,j,k), qr, qs, csacr, acco (1, 2), &
                                    den (i,j,k)), qr * rdts)
                            ...
                            psmlt = max (0., smlt (tc, dqs0, qs * den (i,j,k), psacw, psacr, csmlt, &
                                den (i,j,k), denfac (i,j,k)))
                            sink = min (qs, dts * (psmlt + pracs), tc / icpk)
                            qs = qs - sink
                            ! sjl, 20170321:
                            tmp = min (sink, dim (qs_mlt, ql)) ! max ql due to snow melt
                            ...
                            ! new total condensate / old condensate 
                            qak(i,j,k) = max(0.0,min(1.,qak(i,j,k) * max(qi+ql+tmp,0.0  ) / &
                                                                max(qi+ql    ,qcmin) ) )
                        ...
                        if (qg > qpmin .and. tc > 0.) then
                            ...
                            if (qr > qpmin) &
                                pgacr = min (acr3d (vtg (i,j,k), vtr (i,j,k), qr, qg, cgacr, acco (1, 3), &
                                    den (i,j,k)), rdts * qr)
            ...
            ! GEOS ! WMP impose CALIPSO ice polynomial from 0 C to -40 C on qi0_crt  
                            ...
                            ! new total condensate / old condensate 
                            qak(i,j,k) = max(0.0,min(1.,qak(i,j,k) * max(qi+ql-sink+tmp,0.0  ) / &
                                                            max(qi+ql         ,qcmin) ) )
        ...
        call subgrid_z_proc_3d (is, ie, js, je, ktop, kbot, p1, den, denfac, dts, tzk, qvk, &
            qlk, qrk, qik, qsk, qgk, qak, subl1, h_var, ccn, cnv_fraction, srf_type)
    ...
    subroutine subgrid_z_proc_3d (is, ie, js, je, ktop, kbot, p1, den, denfac, dts, tz, qv, &
        ql, qr, qi, qs, qg, qa, subl1, h_var, ccn, cnv_fraction, srf_type)
        implicit none
        ...
        !$acc data copyin(fac_v2l, fac_l2v, fac_i2v, fac_s2v, fac_v2s, fac_g2v, fac_v2g, fac_frz, dts) &
        !$acc      present(p1, den, denfac, cnv_fraction, srf_type, h_var, ccn, &
        !$acc              tz, qv, ql, qr, qi, qs, qg, qa, subl1)

        !$acc parallel loop gang vector collapse(3) &
        !$acc          private(lhl, lhi, q_liq, q_sol, cvm, rh_adj, rh_rain, &
        !$acc                  sink, lcpk, icpk, tcpk, tcp3, qpz, rh, tin, qsw, dq0, &
        !$acc                  factor, evap, dtmp, tc, qsi, dq, pidep, ifrac, &
        !$acc                  qi_crt, qden, tmp, tsq, pssub, pgsub, q_cond, qstar, &
        !$acc                  rqi, q_plus, q_minus, dqsdt, dwsdt)
        do k = ktop, kbot
            do j = js, je
                do i = is, ie
                    lhl = lv00 + d0_vap * tz (i,j,k)
    ...
    subroutine terminal_fall_3d (dtm, is, ie, js, je, ktop, kbot, tz, qv, ql, qr, qg, qs, qi, dz, dp, &
        ...
        !$acc data create(ze , zt, icpk, cvm, m1, dm, k0, no_fall) copyin(zs, fac_imlt, dtm) &
        !$acc      present(vtg, vts, vti, den, dp, dz, qv, ql, qr, qg, qs, qi, tz, m1_sol, w1, &
        !$acc              r1, g1, s1, i1)

        ! -----------------------------------------------------------------------
        ! define heat capacity and latend heat coefficient
        ! -----------------------------------------------------------------------
        !$acc parallel loop gang vector collapse(3) private(lhi, q_liq, q_sol)
        do k = ktop, kbot
            do j = js, je
                do i = is, ie
                    m1_sol (i,j,k) = 0.
        ...
        ! melting of cloud_ice (before fall) :
        ! -----------------------------------------------------------------------
        !$acc parallel loop gang vector collapse(3) private(tc, q_liq, q_sol, lhi, sink, tmp)
        do k = ktop, kbot
            do j = js, je
                do i = is, ie
                    if(k.ge.k0(i,j)) then
        ...
        ! update capacity heat and latend heat coefficient
        ! -----------------------------------------------------------------------
        !$acc parallel loop gang vector collapse(3) private(lhi)
        ! do k = k0, kbot
        do k = kbot, kbot
            do j = js, je
                do i = is, ie
        ...
        do j = js, je
            do i = is, ie
                if (vi_fac < 1.e-5 .or. no_fall(i,j)) then
                    ...
                    if (k0(i,j) < kbot) then
                        !$acc loop seq private(exit_flag)
                        do k = kbot - 1, k0(i,j), - 1
                            if (qi (i,j,k) > qcmin) then
                                exit_flag = .true.
                                !$acc loop seq private(dtime, sink, tmp)
                                do m = k + 1, kbot
                ...
                if (no_fall(i,j)) then
                    ...
                    if (k0(i,j) < kbot) then
                        ...
                        do k = kbot - 1, k0(i,j), - 1
                            if (qs (i,j,k) > qpmin) then
                                exit_flag = .true.
                                !$acc loop seq private(dtime, sink)
                                do m = k + 1, kbot
                                    if (zt (i,j,k + 1) >= ze (i,j,m) .and. exit_flag) exit_flag = .false.
                                    if(exit_flag) then
    ...
    subroutine implicit_fall_3d (dt, is, ie, js, je, ktop, kbot, ze, vt, dp, q, precip, m1, no_fall)
        ...
        do k = ktop + 1, kbot
            ...
            do j = js, je
                do i = is, ie
                    if(.not.no_fall(i,j)) qm (i,j,k) = (q (i,j,k) + (dt * vt (i,j,k-1)) * qm (i,j,k - 1)) / ((ze (i,j,k) - ze (i,j,k + 1)) + (dt * vt (i,j,k)))
                enddo
            enddo
        enddo
        !$acc end parallel
        ...
        do k = ktop, kbot
            do j = js, je
                do i = is, ie
                    if(.not.no_fall(i,j)) qm (i,j,k) = qm (i,j,k) * (ze (i,j,k) - ze (i,j,k + 1))
                enddo
            enddo
        enddo
        !$acc end parallel loop
        ...
        do k = ktop + 1, kbot
            ...
            do j = js, je
                do i = is, ie
                    if(.not.no_fall(i,j)) m1 (i,j,k) = m1 (i,j,k - 1) + q (i,j,k) - qm (i,j,k)
                enddo
            enddo
        enddo
        !$acc end parallel
        ...
        do j = js, je
            do i = is, ie
                if(.not.no_fall(i,j)) precip(i,j) = m1 (i,j,kbot)
            enddo
        enddo
        !$acc end kernels
        ! -----------------------------------------------------------------------
        ...
        do k = ktop, kbot
            do j = js, je
                do i = is, ie
                    if(.not.no_fall(i,j)) q (i,j,k) = qm (i,j,k) / dp (i,j,k)
                enddo
            enddo
        enddo
        !$acc end parallel loop
...
!$acc routine seq
    ...
    subroutine neg_adj (pt, dp, qv, ql, qr, qi, qs, qg)
        ...
        !$acc data present(dp, pt, qv, ql, qr, qi, qs, qg)
        
        ! -----------------------------------------------------------------------
        ! define heat capacity and latent heat coefficient
        ! -----------------------------------------------------------------------
        !$acc parallel loop gang vector collapse(3) private(cvm, lcpk, icpk)
        do k = ktop, kbot
            do j = js, je
                do i = is, ie
        ...
        ! -----------------------------------------------------------------------
        !$acc parallel loop gang vector collapse(2) private(dq)
        do j = js, je
            do i = is, ie
...
!$acc       create(tz, h_var1d, qvz, qlz, qrz, qiz, qsz, qgz, qaz, &
!$acc         dp1, dz1, w1, vtiz, vtsz, vtgz, vtrz, &
...
!$acc         r1, s1, i1, g1)
        ...
        do n = 1, ntimes
            ...
            ! dry air density
            ! -----------------------------------------------------------------------
            !$acc parallel loop gang vector collapse(3) private(t0)
            do k = ktop, kbot
                do j = js, je
                    do i = is, ie  
                        if (p_nonhydro) then
