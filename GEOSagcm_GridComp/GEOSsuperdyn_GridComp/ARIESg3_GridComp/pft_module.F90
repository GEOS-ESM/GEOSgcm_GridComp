...
#if !defined ( ALGEBRAIC_FILTER )
      if(s(j) > D1_01) then
#else
      if(s(j) > D1_01 .and. s(j) <= D4_0) then
...
!************
      do j=js2g0,jn2g0
         ...
         if( sc(j) > D1_0 ) then
#else
         if(sc(j) > D1_0 .and. sc(j) <= D2_0) then
            sc(j) =  D1_0 +  (sc(j)-D1_0)/(sc(j)+D1_0)
         elseif(sc(j) > D2_0 .and. sc(j) <= D4_0) then
            sc(j) =  D1_0 +  sc(j)/(D8_0-sc(j))
         elseif(sc(j) > D4_0 ) then
#endif
...
!************
      do j=js2g0,jn1g1
         ...
         if( se(j) > D1_0 ) then
#else
         if(se(j) > D1_0 .and. se(j) <= D2_0 ) then
            se(j) =  D1_0 +  (se(j)-D1_0)/(se(j)+D1_0)
         elseif(se(j) > D2_0 .and. se(j) <= D4_0) then
            se(j) =  D1_0 +  se(j)/(D8_0-se(j))
         elseif(se(j) > D4_0 ) then
#endif
! FFT
            do i=1,im/2
               phi = dl * i
