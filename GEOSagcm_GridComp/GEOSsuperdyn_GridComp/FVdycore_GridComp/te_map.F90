...
! !INTERFACE:
   ...
   subroutine te_map(grid,    consv,  convt,   ps,      oma,             &
                     pe,      delp,    pkz,    pk,      mdt,             &
                     nx,      u,       v,      pt,      tracer,          &
                     hs,      cp,      akap,   kord,    peln,            &
                     te0,     te,      dz,     mfx,     mfy,             &
                     te_method, dthconsv, dtmp )
!
! !USES:

   use shr_kind_mod,  only : r8 => shr_kind_r8
   use dynamics_vars, only : T_FVDYCORE_GRID, T_TRACERS
   use mapz_module,   only : map1_cubic_te, map1_ppm, mapn_ppm_tracer

#if defined( SPMD )
   use parutilitiesmodule, only: sumop,  bcstop, parcollective
...
! tracers including specific humidity
   type (t_tracers), intent(inout) :: tracer(grid%ntotq)     ! Array of pointers to individual tracers
...
! !OUTPUT PARAMETERS:
   ...
   real(r8) oma (grid%ifirstxy:grid%ilastxy,grid%jfirstxy:grid%jlastxy,grid%km)    ! vertical press. velocity (pascal/sec)
   ...
   real(r8) dthconsv(grid%ifirstxy:grid%ilastxy,grid%jfirstxy:grid%jlastxy,grid%km) ! THV Increment due to Consv=TRUE
...
! LT 06.12.13 : Compute A-Grid Kinetic Energy
! CC 07.01.29 : Additions for proper calculation of OMGA
! LT 07.06.04 : Removed OMEGA calculations (moved to FV_Wrapper) except for remapping
...
! Nontrivial x decomposition
      if (itot /= im) then
        ...
        call mp_send3d( commglobal, dest, src, im, km+1, jm,             &
                        ifirst, ilast, 1, km+1, jfirst, jlast,      &
                        ilast, ilast, 1, km+1, jfirst, jlast, pe )
      endif
      call mp_send3d( commglobal, iam+nprxy_x, iam-nprxy_x, im, km+1, jm,&
                      ifirst, ilast, 1, km+1, jfirst, jlast,             &
...
! Compute cp*T + KE
      ...
      do 1000 k=1,km
        ...
        do j=js2g0,jlast
           do i=ifirst,ilast
#ifdef DGRID
              u2(i,j) = u(i,j,k)**2  ! D-Grid KE
#else
              u2(i,j) = u(i,j,k)     ! A-Grid KE
#endif
           enddo
        enddo
#if defined( SPMD )
...
#else
              u2(i,jlast+1) = unorth(i,k)     ! fill ghost zone
#endif
           enddo
        endif
#endif
        ...
        do j=js2g0,jn2g0
          do i=ifirst,ilast
            ...
            te(i,j,k) = D0_25 * ( u2(i,j) + u2(i,j+1)          &
                      ...
                      + t2(i,j)*pkz(i,j,k)
#else
            te(i,j,k) = D0_125*( (u2(i,j)+u2(i,j+1))**2          &
                               + (v2(i,j)+v2(i+1,j))**2 )        &
                      + t2(i,j)*pkz(i,j,k)
#endif
          enddo
...
1000  continue
      ...
      if (itot /= im) then
        call mp_recv3d( commglobal, src, im, km+1, jm,                      &
                        ifirst-1, ifirst-1, 1, km+1, jfirst, jlast,         &
                        ifirst-1, ifirst-1, 1, km+1, jfirst, jlast, pewest )   
      endif
      call mp_recv3d( commglobal, iam-nprxy_x, im, km+1, jm,                &
...
! pe1(i1-1,1:km+1) must be ghosted
          ...
          do k=grid%ks+2,km+1
! pe1(i1-1,grid%ks+2:km+1) must be ghosted
            bkh = D0_5*grid%bk(k)
            pe3(i1,k) = grid%ak(k) + bkh*(pe1(i1,km+1)+pe1w(km+1))
            do i=i1+1,i2
              pe3(i,k) = grid%ak(k) + bkh*(pe1(i,km+1)+pe1(i-1,km+1))
            enddo
          enddo
...
#if defined( SPMD )
! Kludge to fix non-reproducibility on discover (LLT 6/4/2009)
! ------------------------------------------------------------
       call parcollective( commglobal,bcstop,dtmp )
#endif
       if( diag ) then
           write(6,1001) grid%iam,te0,te1,sum,dtmp
 1001      format(1x,'IAM: ',i2,' TE0: ',g20.10,' TE1: ',g20.10,' sum: ',g20.10,' DelT: ',g20.10)
       endif
        ...
        do j=js2g0,jn2g0
          do i=ifirst,ilast
            ...
            te(i,j,k) = te(i,j,k) - D0_25 * ( u2(i,j) + u2(i,j+1)     &
                                            + v2(i,j) + v2(i+1,j) )
#else
            te(i,j,k) = te(i,j,k) - D0_125*( (u2(i,j)+u2(i,j+1))**2     &
                                           + (v2(i,j)+v2(i+1,j))**2 )
#endif
          enddo
        enddo
