...
! !INPUT/OUTPUT PARAMETERS:
   ...
   type(T_TRACERS), intent(inout):: tracer(grid%ntotq)
...
!   WP  07.04.19:  Modified interface to tp2c to support new filtering options 
...
! Temporary r8 array for Q
   ...
   integer :: nq                            ! # of advected tracers
...
!!!    if (max_nsplt /= 1) write(*,*) 'trac2d: max_nsplt,k_courant = ', max_nsplt,k_courant
!!!    print *, "max_nsplt", max_nsplt, "k_cour", k_courant, "nsplt", nsplt(:)
...
!CSD$ END PARALLEL DO
 ...
 call FVbarrierclock(grid,'sync_trac2d_tracer')
...
!
        if ( iq < nq ) then
          ...
          call mp_send4d_ns( commglobal, im, jm, km, 1, jfirst, jlast, kfirst, &
                             kend, ng, ng, q_r8(1,jfirst-ng,kfirst,nxt) )
        endif
        call FVstopclock(grid,'----TRAC2D_TRACER_COMM')
#else
!
...
!
        if ( tracer(iq)%is_r4 ) then
          q_r8(1:im,jfirst:jlast,kfirst:kend,cur) = &
                tracer(iq)%content_r4(1:im,jfirst:jlast,kfirst:kend)
        else
          q_r8(1:im,jfirst:jlast,kfirst:kend,cur) = &
                tracer(iq)%content(1:im,jfirst:jlast,kfirst:kend)
        endif
#endif
        ...
        do 5000 k=kfirst,kend
           if ( it <= nsplt(k) ) then
              ...
              if ( tracer(iq)%is_r4 ) then
                 do j=jfirst,jlast
                    do i=1,im
                       tracer(iq)%content_r4(i,j,k) = q_r8(i,j,k,cur) / dp2(i,j,k)
                    enddo
                 enddo
              else
                 do j=jfirst,jlast
                    do i=1,im
                       tracer(iq)%content(i,j,k) = q_r8(i,j,k,cur) / dp2(i,j,k)
                    enddo
                 enddo
              endif
           endif
