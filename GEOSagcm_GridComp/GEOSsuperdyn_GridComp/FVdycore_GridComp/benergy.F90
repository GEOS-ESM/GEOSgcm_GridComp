...
! Local
      ...
      integer   :: i, j, k, js1g0, js2g0, jn1g0, jn1g1, jn2g0, jtot
...
! Check the poles for consistent values
      ...
      if ( jlastxy /= jm ) then    ! Pull information out of northern halo
         do i=ifirstxy,ilastxy
#ifdef DGRID
            u2(i,jlastxy+1) = unorth(i,k)**2
#else
            u2(i,jlastxy+1) = unorth(i,k)
#endif
         enddo
      endif
...
#else
         v2(ilastxy+1,j) = veast(j,k)     ! eastern halo
#endif
      enddo
      ...
      if ( qqq%is_r4 ) then
         do j=jfirstxy,jlastxy
            do i=ifirstxy,ilastxy
               tm(i,j) = t3(i,j,k)*(D1_0+r_vir*qqq%content_r4(i,j,k))
            enddo
         enddo
      else
         do j=jfirstxy,jlastxy
            do i=ifirstxy,ilastxy
               tm(i,j) = t3(i,j,k)*(D1_0+r_vir*qqq%content(i,j,k))
            enddo
         enddo
      endif
      ...
      if ( jfirstxy == 1 ) then
        do i=ifirstxy,ilastxy
#ifdef DGRID
           te_sp(i,k) = u2(i,2) + v2(i,2)
#else
           te_sp(i,k) = D0_5*u2(i,2)**2
#endif
        enddo
      ...
      if ( jlastxy == jm ) then
        do i=ifirstxy,ilastxy
#ifdef DGRID
           te_np(i,k)=      u2(i,jm) + v2(i,jm-1)
#else
           te_np(i,k)= D0_5*u2(i,jm)**2
#endif
        enddo
...
! SP
        tte(1) = pe(ifirstxy,1,km+1)*phis(ifirstxy,1) - pe(ifirstxy,1,1)*gztop(ifirstxy)
        do k=1,km
...
!       gztop(:) should all have identical values   WS 2006.06.22: this checks out
! NP
        tte(jm) = pe(ifirstxy,jm,km+1)*phis(ifirstxy,jm) - pe(ifirstxy,jm,1)*gztop(ifirstxy)
        do k=1,km
...
! Interior
        ...
        do i=ifirstxy,ilastxy
           bte(i,j) = pe(i,j,km+1)*phis(i,j) - pe(i,j,1)*gztop(i)
        enddo
...
! print *, "myidxy_x/y:", myidxy_x, myidxy_y, "The total energy is", te0
