...
! the D-grid during the last small timestep in cd_core.
...
! !INPUT PARAMETERS:
      ...
      real(r8), intent(in) :: akap, cp
      real(r8), intent(in) :: hs(grid%ifirstxy:grid%ilastxy,grid%jfirstxy:grid%jlastxy)
      real(r8), intent(in) :: pt(grid%ifirstxy:grid%ilastxy,grid%jfirstxy:grid%jlastxy,grid%km)
      real(r8), intent(in) :: delp(grid%ifirstxy:grid%ilastxy,grid%jfirstxy:grid%jlastxy,grid%km)
...
! !INTERFACE:
      ...
      use mod_comm, only : numpro, blockdescriptor, get_partneroffset,  &
                           commglobal, mp_swapirr, max_nparcels
#endif
...
! !INPUT PARAMETERS:
      ...
      real(r8)    akap, cp
      real(r8), intent(in)::    hs(1:grid%im,grid%jfirst:grid%jlast)
...
#if defined(SPMD)
!
! data structures for mp_swapirr
!
...
! allocate for either method (safety)
        ...
        call get_partneroffset(commglobal, sendbl1, recvbl1)
        call get_partneroffset(commglobal, sendbl2, recvbl2)
...
# if !defined (PAREXCH)
        call mp_swapirr(commglobal, sendbl1, recvbl1, inbuf8, outbuf8)
# else
...
# if !defined (PAREXCH)
        call mp_swapirr(commglobal, sendbl2, recvbl2, inbuf8, outbuf8)
# else
