...
! !LOCAL VARIABLES:
      ...
      integer i, k, kk, kl, k0(i1:i2,0:kn+1)
...
! !INPUT PARAMETERS:
      ...
      real(r8), intent(in) :: pe2(ifirst:ilast,kn+1) ! pressure at layer edges 
                                               ...
                                               ! in the new vertical coordinate
! !INPUT/OUTPUT PARAMETERS:
      type (t_tracers), intent(inout)::  tracer(nq) ! Field output
...
!dir$ prefervector
    ...
    do iq=1,nq
      if ( tracer(iq)%is_r4) then
        do k=1,km
          do i=i1,i2
            q4(1,i,k) = tracer(iq)%content_r4(i,j,k)
          enddo
        enddo
      else
        do k=1,km
          do i=i1,i2
            q4(1,i,k) = tracer(iq)%content(i,j,k)
          enddo
        enddo
      endif
...
! Check to see if pe2(i,k+1) and pe2(i,k) are in same pe1 interval
              if (k0(i,k+1) == k0(i,k)) then
                 pr = (pe2(i,k+1)-pe1(i,kk)) / dp1(i,kk)
                 tracer(iq)%content_r4(i,j,k) = q4(2,i,kk)          &
...
! Now consider contribution between pe1(i,k0(i,k+1)) and pe2(i,k+1)
                 ...
                 qsum = qsum + delp*(q4(2,i,kl)+D0_5*esl*                          &
                    (q4(3,i,kl)-q4(2,i,kl)+q4(4,i,kl)*(D1_0-r23*esl)))
                 tracer(iq)%content_r4(i,j,k) = qsum / ( pe2(i,k+1) - pe2(i,k) )
              endif
           enddo
        enddo
...
! Check to see if pe2(i,k+1) and pe2(i,k) are in same pe1 interval
              if (k0(i,k+1) == k0(i,k)) then
                 pr = (pe2(i,k+1)-pe1(i,kk)) / dp1(i,kk)
                 tracer(iq)%content(i,j,k) = q4(2,i,kk)          &
                            + D0_5*(q4(4,i,kk)+q4(3,i,kk)-q4(2,i,kk))     &
                            *(pr+pl)-q4(4,i,kk)*r3*(pr*(pr+pl)+pl**2)
              else
! Consider contribution between pe2(i,k) and pe1(i,kk+1)
...
! Now consider contribution between pe1(i,k0(i,k+1)) and pe2(i,k+1)
                 ...
                 qsum = qsum + delp*(q4(2,i,kl)+D0_5*esl*                          &
                    (q4(3,i,kl)-q4(2,i,kl)+q4(4,i,kl)*(D1_0-r23*esl)))
                 tracer(iq)%content(i,j,k) = qsum / ( pe2(i,k+1) - pe2(i,k) )
              endif
           enddo
        enddo
