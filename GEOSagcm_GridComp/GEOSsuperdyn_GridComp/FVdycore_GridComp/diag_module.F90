...
!   09.04.01  Sawyer  Upgraded to PILGRIM from cam3_6_33
!
! EOP
!-----------------------------------------------------------------------
  private
...
#if defined( SPMD )
  use mod_comm, only: commglobal, mp_send3d, mp_recv3d, mp_swapirr
#endif
...
! Local 
  ...
  if ( grid%twod_decomp == 1 ) then
    ...
    call mp_swapirr( commglobal, grid%ikj_xy_to_yz%SendDesc,   &
...
!
  call mp_send3d(commglobal, dest, src, im, km+1, jm,          &
                 1, im, kfirst, klast+1, jfirst, jlast, 1,     &
                 im, kfirst, klast+1, jlast, jlast, pe)
  ...
  call mp_send3d(commglobal, src, dest, im, km+1, jm,          &
                 1, im, kfirst, klast+1, jfirst, jlast,        &
                 1, im, kfirst, klast+1, jfirst, jfirst, pe)
  call mp_recv3d(commglobal, dest, im, km+1, jm,               &
...
!
    call mp_swapirr(commglobal, grid%ikj_yz_to_xy%SendDesc,   &
