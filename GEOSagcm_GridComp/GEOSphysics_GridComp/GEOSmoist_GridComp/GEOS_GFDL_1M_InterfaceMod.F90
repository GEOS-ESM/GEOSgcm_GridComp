...
! ------------------------------
    ...
    if (DEBUG_TQ_ERRORS) then
         do L = 1, LM
           do J = 1, JM
             ...
             if ( (       T(I,J,L) > 333.0) .OR. (       T(I,J,L) /=        T(I,J,L)) .OR. &
                   ...
                   (QGRAUPEL(I,J,L) < 0.0  ) .OR. (QGRAUPEL(I,J,L) /= QGRAUPEL(I,J,L)) ) then
                 print *, "T or Q  spike detected : ", T(I,J,L)
                 print *, "    GFDL DTDTmic Temp Increment : ", DTDTmic(I,J,L) * DT_MOIST
                 print *, "  Latitude       =", LATS(I,J)*180.0/MAPL_PI
                 print *, "  Longitude      =", LONS(I,J)*180.0/MAPL_PI
                 print *, "  Pressure (mb)  =", PLmb(I,J,L)
                 print *, "                        CLLS=",   CLLS(I,J,L), "   CLCN=",    CLCN(I,J,L)
     ...
     ! ------------------------------
         if (REPORT_GFDL_1M_NEGATIVES) then
           call FILLQ2ZERO( Q       , MASS, DT=DT_MOIST, DQDT=  DQVDT_FILL, WARNING_LABEL="QV   After GFDL_1M Cloud Microphysics", VM=VMG, RC=STATUS); VERIFY_(STATUS)
           call FILLQ2ZERO( QLLS    , MASS, DT=DT_MOIST, DQDT=DQLLSDT_FILL, WARNING_LABEL="QLLS After GFDL_1M Cloud Microphysics", VM=VMG, RC=STATUS); VERIFY_(STATUS)
           call FILLQ2ZERO( QLCN    , MASS, DT=DT_MOIST, DQDT=DQLCNDT_FILL, WARNING_LABEL="QLCN After GFDL_1M Cloud Microphysics", VM=VMG, RC=STATUS); VERIFY_(STATUS)
           call FILLQ2ZERO( QILS    , MASS, DT=DT_MOIST, DQDT=DQILSDT_FILL, WARNING_LABEL="QILS After GFDL_1M Cloud Microphysics", VM=VMG, RC=STATUS); VERIFY_(STATUS)
           call FILLQ2ZERO( QICN    , MASS, DT=DT_MOIST, DQDT=DQICNDT_FILL, WARNING_LABEL="QICN After GFDL_1M Cloud Microphysics", VM=VMG, RC=STATUS); VERIFY_(STATUS)
           call FILLQ2ZERO( QRAIN   , MASS, DT=DT_MOIST, DQDT=  DQRDT_FILL, WARNING_LABEL="QR   After GFDL_1M Cloud Microphysics", VM=VMG, RC=STATUS); VERIFY_(STATUS)
           call FILLQ2ZERO( QSNOW   , MASS, DT=DT_MOIST, DQDT=  DQSDT_FILL, WARNING_LABEL="QS   After GFDL_1M Cloud Microphysics", VM=VMG, RC=STATUS); VERIFY_(STATUS)
           call FILLQ2ZERO( QGRAUPEL, MASS, DT=DT_MOIST, DQDT=  DQGDT_FILL, WARNING_LABEL="QG   After GFDL_1M Cloud Microphysics", VM=VMG, RC=STATUS); VERIFY_(STATUS)
         else
           call FILLQ2ZERO( Q       , MASS, DT=DT_MOIST, DQDT=  DQVDT_FILL, VM=VMG, RC=STATUS); VERIFY_(STATUS)
           call FILLQ2ZERO( QLLS    , MASS, DT=DT_MOIST, DQDT=DQLLSDT_FILL, VM=VMG, RC=STATUS); VERIFY_(STATUS)
           call FILLQ2ZERO( QLCN    , MASS, DT=DT_MOIST, DQDT=DQLCNDT_FILL, VM=VMG, RC=STATUS); VERIFY_(STATUS)
