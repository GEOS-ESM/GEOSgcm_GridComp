...
!!  public :: fast_sat_adj
    ...
    real   , parameter :: es_table_tmin = tice - 160.
    real (kind = r8), parameter :: delt = 0.1
    real (kind = r8), parameter :: rdelt = 1.0/delt
    ...
    integer :: vdiffflag = 2 ! wind difference scheme in accretion
    ! 1: Wisner et al. (1972)
    ! 2: Mizuno (1990)
    ! 3: Murakami (1990)
    ...
    logical :: do_cld_adj = .true. ! do cloud fraction adjustment

    logical :: do_ref = .false. ! do radar calculations
    ...
    real :: f_dq_p = 3.0 ! cloud fraction adjustment for supersaturation
    ...
    real :: rermin = 10.0, rermax = 10000.0 ! minimum and maximum effective radius for rain (micron)
    ...
    real (kind = r8) :: lv00, li00, li20, cpaut0, ql_exponent, ccn_exponent 
...
!!  ntimes = max (ntimes, ceiling (dtm / min (0.5*dtm, mp_time)))
    ...
    if (.not. exists) then
        ...
        if(ios /= 0) stop 'open namelist file gfdl_mp:input.nml failed, bailing out...'
        rewind (nlunit, iostat=ios)
        if(ios /= 0) stop 'rewind namelist file gfdl_mp:input.nml failed, bailing out...'
        ! Read Main namelist
        read (nlunit,gfdl_mp_nml,iostat=ios)
        if(ios /= 0) stop 'read namelist gfdl_mp:input.nml:gfdl_mp_nml failed, bailing out...'
        close(nlunit, iostat=ios)
        if(ios /= 0) stop 'close namelist file gfdl_mp:input.nml failed, bailing out...'
    endif
#endif
...
!        ua, va, delz, delp, dtm, rhcrit, hs, cnv_frc, eis, area, srf_type, water, rain, ice, snow, graupel, &
...
subroutine gfdl_mp_driver (qv, ql, qr, qi, qs, qg, qa, zet, qnl, qni, pt, wa, &
    ...
    integer, intent (in) :: is, ie, ks, ke, ktop
...
!   real, intent (inout), dimension (is:, ks:) :: q_con, cappa
    ...
    logical :: consv_te = .false.
    logical :: do_inline_mp = .false.
    logical :: last_step = .true.
    logical :: use_cond = .true.
    logical :: moist_kappa = .true.
    real, dimension (is:ie) :: mppcw, mppew, mppe1, mpper, mppdi
    real, dimension (is:ie) :: mppd1, mppds, mppdg, mppsi, mpps1
    real, dimension (is:ie) :: mppss, mppsg, mppfw, mppfr, mppar
    real, dimension (is:ie) :: mppas, mppag, mpprs, mpprg, mppxr
    real, dimension (is:ie) :: mppxs, mppxg, mppmi, mppms, mppmg
    real, dimension (is:ie) :: mppm1, mppm2, mppm3
...
subroutine setup_mp
    ...
    ! -----------------------------------------------------------------------
    select case (c_paut_scheme)
    case (1)
        ! Hong et al. (2004) Formulation
        ! DOI: 10.1175/1520-0493(2004)132<0103:ARATIM>2.0.CO;2
        ql_exponent  = 7.0 / 3.0
        ccn_exponent = -1.0 / 3.0
        aone   = (2.0 / 9.0) * ((3.0 / 4.0) ** (4.0 / 3.0)) / (pi ** (1.0 / 3.0))
        cpaut0 = c_paut * aone * grav / visd
    case (2)
        ! Khairoutdinov and Kogan (2000) Formulation
        ! DOI: 10.1175/1520-0493(2000)128<0229:ANCPPI>2.0.CO;2
        ql_exponent  = 2.47
        ccn_exponent = -1.79
        ! Empirical KK2000 pre-factor in SI units. 
        ! NOTE: If your ccn input is in #/cm3, multiply this constant by 1.0e6**(-1.79) 
        ! inside your localized c_praut evaluation if necessary.
        cpaut0 = c_paut * 1350.0
    case default
        stop 'c_paut_scheme invalid'
    end select
...
subroutine mpdrv (hydrostatic, ua, va, wa, delp, pt, qv, ql, qr, qi, qs, qg, qa, &
    ...
    real, intent (  out), dimension (:, :) :: te, adj_vmr
    ...
    do i = is, ie
        ...
        ! -----------------------------------------------------------------------
        ! diagnosed convective fraction and surface type (input)
        ! -----------------------------------------------------------------------
        ...
        if (consv_checker) then
            call mtetw (ks, ke, qv (i, ks:), ql (i, ks:), qr (i, ks:), qi (i, ks:), &
                qs (i, ks:), qg (i, ks:), tz, ua (i, ks:), va (i, ks:), wa (i, ks:), &
                delp (i, :), dte (i), 0.0, water (i), rain (i), ice (i), &
                snow (i), graupel (i), 0.0, 0.0, dtm, te_beg_m (i, ks:), &
        ...
        if (prog_cin) then
            do k = ks, ke
              ! qni import from GEOS has units # / m^3
                cin (k) = qni (i, k) / den (k)
            enddo
        else
            cin (k) = 0.
        ...
        if (do_mp_full) then
            ...
            call mp_full (ks, ke, ntimes, tz, qaz, qvz, qlz, qrz, qiz, qsz, qgz, dp, dz, &
                ...
                prefluxr (i, ks:), prefluxi (i, ks:), prefluxs (i, ks:), prefluxg (i, ks:), &
        ...
        if (consv_checker) then
            call mtetw (ks, ke, qvz, qlz, qrz, qiz, qsz, qgz, tz, u, v, w, &
                dp, dte (i), 0.0, water (i), rain (i), ice (i), snow (i), &
                graupel (i), 0.0, 0.0, dtm, te_end_m (i, :), tw_end_m (i, :), &
        ...
        if (consv_te) then
            if (hydrostatic) then
                do k = ks, ke
                    te (i, k) = te (i, k) + c_air * tz (k) * dp (k)
                enddo
            else
                do k = ks, ke
                    te (i, k) = te (i, k) + mte (qvz (k), qlz (k), qrz (k), qiz (k), &
                        qsz (k), qgz (k), tz (k), dp (k), .true.) * grav
                enddo
            endif
        endif
...
subroutine neg_adj (ks, ke, tz, dp, qa, qv, ql, qr, qi, qs, qg, mppcw, mppfr, convt)
...
subroutine mp_full (ks, ke, ntimes, tz, qa, qv, ql, qr, qi, qs, qg, dp, dz, u, v, w, &
    ...
    do n = 1, ntimes
        ...
        call warm_rain (dts, ks, ke, dp, dz, tz, qa, qv, ql, qr, qi, qs, qg, den, &
            denfac, vtw, vtr, ccn, h_var, revap, mpper, mppar, mppxr, convt)
        ...
        if (do_subgrid_proc) then
            ...
            call subgrid_z_proc (ks, ke, den, denfac, dts, h_var, tz, qa, qv, ql, qr, &
                qi, qs, qg, dp, ccn, cin, mppcw, mppew, mppe1, mppdi, mppd1, mppds, &
                mppdg, mppsi, mpps1, mppss, mppsg, mppfw, rsubl, convt, last_step)
...
subroutine mp_fast (ks, ke, tz, qa, qv, ql, qr, qi, qs, qg, dtm, dp, den, ccn, &
    ...
    real, intent (inout), dimension (ks:ke) :: rsubl
    ...
    if (.not. do_warm_rain_mp .and. fast_fr_mlt) then
        ...
        call pcomp (ks, ke, dtm, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, lcpk, icpk, &
    ...
    if (cond_evap) then
        do n = 1, nconds
            call pcond_pevap (ks, ke, dtm, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, &
    ...
    if (.not. do_warm_rain_mp .and. fast_fr_mlt) then
        ...
        call pwbf (ks, ke, dtm, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, &
            lcpk, icpk, tcpk, tcp3, ccn, cin, mppfw, convt)

        ! -----------------------------------------------------------------------
        ! Bigg freezing mechanism
        ! -----------------------------------------------------------------------

        call pbigg (ks, ke, dtm, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, ccn, &
            lcpk, icpk, tcpk, tcp3, mppfw, convt)
        ...
        call pgfr_simp (ks, ke, dtm, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, &
        ...
        call psmlt_simp (ks, ke, dtm, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, &
    ...
    call praut_simp (ks, ke, dtm, dp, tz, qa, qv, ql, qr, qi, qs, qg, mppar, convt)
    ...
    if (.not. do_warm_rain_mp .and. fast_dep_sub) then
        ...
        call pidep_pisub (ks, ke, dtm, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, den, &
            lcpk, icpk, tcpk, tcp3, cin, mppdi, mppsi, rsubl, convt)

        ! -----------------------------------------------------------------------
        ! cloud ice to snow autoconversion
        ! -----------------------------------------------------------------------

        call psaut_simp (ks, ke, dtm, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, mppas, convt)

        ! -----------------------------------------------------------------------
        ! snow deposition and sublimation
        ! -----------------------------------------------------------------------

        call psdep_pssub (ks, ke, dtm, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, den, &
        ...
        call pgdep_pgsub (ks, ke, dtm, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, den, &
...
subroutine sedimentation (dts, ks, ke, tz, qv, ql, qr, qi, qs, qg, dz, dp, &
    ...
    if (do_psd_ice_fall) then
        call term_rsg (ks, ke, qi, den, denfac, 0.5*(vi_fac_cnv+vi_fac_lsc), blini, mui, tvai, tvbi, vi_min, vi_max, const_vi, vti)
    else
        call term_ice (ks, ke, tz, qi, den, vi_fac_cnv, vi_fac_lsc, vi_min, vi_max, const_vi, vti)
    endif
    ...
    if (do_hail) then
        call term_rsg (ks, ke, qg, den, denfac, vh_fac, blinh, muh, tvah, tvbh, vh_min, vh_max, const_vg, vtg)
    else
    ...
    if (do_psd_water_fall) then
        ...
        call term_rsg (ks, ke, ql, den, denfac, vw_fac, blinw, muw, tvaw, tvbw, vw_min, vw_max, const_vw, vtw)
    ...
    call term_rsg (ks, ke, qr, den, denfac, vr_fac, blinr, mur, tvar, tvbr, vr_min, vr_max, const_vr, vtr)
...
subroutine term_ice (ks, ke, tz, q, den, v_fac_cnv, v_fac_lsc, v_min, v_max, const_v, vt)
    ...
    real, intent (in) :: v_fac_cnv, v_fac_lsc, v_min, v_max
            ...
            if (q (k) .lt. qfmin) then
                ...
                select case (ifflag)
                    ...
                    case (1)
                        ...
                        vt (k) = 0.01 * (v_fac_lsc*viLSC*(1.0-cnv_fraction) + v_fac_cnv*viCNV*(cnv_fraction))
                ...
                if (do_ice_pres_scaling) then
                    ...
                    DIAM = 2.0*LDRADIUS4(pl/100.0,tmp,q(k),zero,zero,2,cnv_fraction)*1.e6 ! microns
                    lnP = log(pl/100.0)                                    
                    C0 = -1.04 + 0.298*lnP
                    C1 =  0.67 - 0.097*lnP
...
subroutine term_rsg (ks, ke, q, den, denfac, v_fac, blin, mu, tva, tvb, v_min, v_max, const_v, vt)
    ...
    real, intent (in) :: v_fac, v_min, blin, v_max, mu
...
subroutine warm_rain (dts, ks, ke, dp, dz, tz, qa, qv, ql, qr, qi, qs, qg, &
        den, denfac, vtw, vtr, ccn, h_var, revap, mpper, mppar, mppxr, convt)
    ...
    real, intent (in), dimension (ks:ke) :: h_var, dp, dz, den, denfac, vtw, vtr

    real, intent (inout), dimension (ks:ke) :: qa, qv, ql, qr, qi, qs, qg, ccn

    real (kind = r8), intent (inout), dimension (ks:ke) :: tz

    real, intent (inout), dimension (ks:ke) :: revap
    ...
    call prevp (ks, ke, dts, dp, tz, qa, qv, ql, qr, qi, qs, qg, den, denfac, &
        h_var, revap, mpper, convt)

    ! -----------------------------------------------------------------------
    ! rain accretion with cloud water
    ! -----------------------------------------------------------------------

    call pracw (ks, ke, dts, dp, tz, qa, qv, ql, qr, qi, qs, qg, den, denfac, vtw, &
        vtr, mppxr, convt)
    ...
    call praut (ks, ke, dts, dp, tz, qa, qv, ql, qr, qi, qs, qg, den, ccn, h_var, &
        mppar, convt)
...
subroutine prevp (ks, ke, dts, dp, tz, qa, qv, ql, qr, qi, qs, qg, den, denfac, &
        h_var, revap, mpper, convt)
    ...
    real, intent (in), dimension (ks:ke) :: h_var, den, denfac, dp

    real (kind = r8), intent (inout), dimension (ks:ke) :: tz

    real, intent (inout), dimension (ks:ke) :: qa, qv, qr, ql, qi, qs, qg
    ...
    do k = ks, ke
      ...
      if (tz (k) .gt. t_wfr .and. qr (k) .gt. qpmin) then
        ...
        if (dqv .gt. 0.0 .and. qsat .gt. q_minus) then
            ...
            ! Absolute physics thermodynamic boundary limit
            max_evap_limit = dqv / (1.0 + lcpk (k) * dqdt)
            ...
            sink = min (qr (k), dts * fac_revp * sink, max_evap_limit)
            ...
            if (use_enhanced_dry_evap) then
                ...
                tmp = min (qr (k), tmp)
                ! Safety Bound: Protect the enhanced value from over-saturating the cell
                tmp  = min (tmp, max_evap_limit)
...
subroutine pracw (ks, ke, dts, dp, tz, qa, qv, ql, qr, qi, qs, qg, den, denfac, vtw, &
        vtr, mppxr, convt)
...
subroutine praut (ks, ke, dts, dp, tz, qak, qvk, qlk, qrk, qik, qsk, qgk, den, ccn, h_var, &
        mppar, convt)
    ...
    real, intent (in), dimension (ks:ke) :: h_var, den, dp

    real, intent (inout), dimension (ks:ke) :: qak, qvk, qlk, qrk, qik, qsk, qgk, ccn
...
subroutine ice_cloud (ks, ke, dp, tz, qa, qv, ql, qr, qi, qs, qg, den, denfac, vtw, &
    ...
    if (.not. do_warm_rain_mp) then
        ...
        call psmlt (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, denfac, &
        ...
        call pgmlt (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, denfac, &
        ...
        call psaci (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, denfac, vti, vts, &
            mppxs, convt)
        ...
        call psaut (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, di, mppas, convt)

        ! -----------------------------------------------------------------------
        ! graupel accretion with cloud ice
        ! -----------------------------------------------------------------------

        call pgaci (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, denfac, vti, vtg, &
            mppxg, convt)
        ...
        call psacr_pgfr (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, &
        ...
        call pgacs (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, vts, vtg, mppxg, convt)

        ! -----------------------------------------------------------------------
        ! snow to graupel autoconversion
        ! -----------------------------------------------------------------------

        call pgaut (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, mppag, convt)
...
subroutine psmlt (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, denfac, &
        vtw, vtr, vts, lcpk, icpk, tcpk, tcp3, mppms, convt)
...
subroutine pgmlt (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, denfac, &
        vtw, vtr, vtg, lcpk, icpk, tcpk, tcp3, mppmg, convt)
...
subroutine psaci (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, denfac, vti, &
        vts, mppxs, convt)
...
subroutine psaut (ks, ke, dts, qak, qvk, qlk, qrk, qik, qsk, qgk, dp, tz, den, dik, mppas, convt)
    ...
    real, intent (inout), dimension (ks:ke) :: qak, qvk, qlk, qrk, qik, qsk, qgk, dik
...
subroutine pgaci (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, denfac, vti, &
        vtg, mppxg, convt)
...
subroutine psacr_pgfr (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, &
        denfac, vtr, vts, lcpk, icpk, tcpk, tcp3, mppfr, mpprs, convt)
...
subroutine pgacs (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, vts, vtg, &
        mppxg, convt)
...
subroutine pgaut (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, mppag, convt)
...
subroutine subgrid_z_proc (ks, ke, den, denfac, dts, h_var, tz, qa, qv, ql, qr, &
        qi, qs, qg, dp, ccn, cin, mppcw, mppew, mppe1, mppdi, mppd1, mppds, &
        mppdg, mppsi, mpps1, mppss, mppsg, mppfw, rsubl, convt, last_step)
    ...
    if (.not. do_warm_rain_mp) then
        ...
        call pcomp (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, lcpk, icpk, &
            tcpk, tcp3, mppfw, convt)
        ...
        call pwbf (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, lcpk, &
            icpk, tcpk, tcp3, ccn, cin, mppfw, convt)

        ! -----------------------------------------------------------------------
        ! Bigg freezing mechanism
        ! -----------------------------------------------------------------------

        call pbigg (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, ccn, &
            lcpk, icpk, tcpk, tcp3, mppfw, convt)
        ...
        call pidep_pisub (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, den, &
            lcpk, icpk, tcpk, tcp3, cin, mppdi, mppsi, rsubl, convt)
        ...
        call psdep_pssub (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, den, &
            denfac, lcpk, icpk, tcpk, tcp3, mppds, mppss, convt)
        ...
        call pgdep_pgsub (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, den, &
            denfac, lcpk, icpk, tcpk, tcp3, mppdg, mppsg, convt)
...
subroutine pinst (ks, ke, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, dts, den, &
        lcpk, icpk, tcpk, tcp3, h_var, mppe1, mppd1, mpps1, convt)
    ...
    do k = ks, ke
        ...
        ! -----------------------------------------------------------------------
        ! Polar Stratospheric Ice Cloud (PSC Type II) Proxy
        ...
        ! -----------------------------------------------------------------------
        ! Your original extreme freeze-dry sink (left untouched)
        ! -----------------------------------------------------------------------
        ...
        ! Only allow standard tropospheric clearing if we are well outside the PSC zone
        if (tin .gt. t_sub + 6. .and. tz(k) .ge. t_psc) then
...
subroutine pcond_pevap (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, &
...
subroutine pcomp (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, lcpk, icpk, &
        tcpk, tcp3, mppfw, convt)
...
subroutine pbigg (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, den, ccn, &
        lcpk, icpk, tcpk, tcp3, mppfw, convt)
...
subroutine pidep_pisub (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, &
        den, lcpk, icpk, tcpk, tcp3, cin, mppdi, mppsi, rsubl, convt)
    ...
    real, intent (inout), dimension (ks:ke) :: qa, qv, ql, qr, qi, qs, qg, cin
    real, intent (inout), dimension (ks:ke) :: lcpk, icpk, tcpk, tcp3
    ...
    do k = ks, ke
        ...
        if (tz (k) .lt. tice) then
            ...
            qsi = iqs (tin, den (k), dqdt)
            dq = qv (k) - qsi
            ! Potential deposition/sublimation mass limit
            tmp = min(qi (k), dq / (1.0 + tcpk (k) * dqdt))

            ! -----------------------------------------------------------------
            ! 1. Ice Crystal Concentration (cin) Evaluation
            ! -----------------------------------------------------------------
            if (qi (k) .gt. qcmin) then
                if (do_psd_ice_num) then
                    ...
                    cin (k) = cin (k) / den (k)
                elseif (.not. prog_cin) then
                    select case (inflag)
                        case (1)
                            ! Uses ice mass relationship
                            ! Simplified from: exp(0.75 * log(x)) -> x**0.75
                            cin (k) = 5.38e7 * (qi (k) * den (k))**0.75
                        case (2)
                            ! Uses temperature dependence (Fletcher-type curve)
                            ! Simplified from: exp(-2.80) * 1000.0 -> 60.8101
                            cin (k) = 60.8101 * exp (0.262 * (tice - tz (k)))
                        case (3)
                            ! Uses ice supersaturation dependence
                            ! Simplified from: exp(-0.639) * 1000.0 -> 527.819
                            cin (k) = 527.819 * exp (12.96 * (qv (k) / qsi - 1.0))
                        case (4)
                            ! Steeper temperature dependence
                            ! Simplified from: 5.e-3 * 1000.0 -> 5.0
                            cin (k) = 5.0 * exp (0.304 * (tice - tz (k)))
                        case (5)
                            ! Very steep temperature dependence (Meyers-type approximation)
                            ! Simplified from: 1.e-5 * 1000.0 -> 0.01
                            cin (k) = 0.01 * exp (0.5 * (tice - tz (k)))
                    end select
                endif
                pidep = dts * dq * 4.0 * 11.9 * sqrt(qi (k) * den (k) * cin (k)) / &
                       (qsi * den (k) * (tcpk (k) * cvm (k)) ** 2 / (tcond * rvgas * tz (k) ** 2) + &
                      1. / vdifu)
            endif

            ! -----------------------------------------------------------------
            ! 2. Deposition vs Sublimation Branching
            ! -----------------------------------------------------------------
            if (dq .gt. 0.) then
                ...
                rsubl (k) = rsubl (k) - sink * dp (k) * convt
            endif
...
subroutine psdep_pssub (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, &
...
subroutine pgdep_pgsub (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, tz, dp, cvm, te8, &
...
subroutine cloud_fraction (ks, ke, pz, den, qv, ql, qr, qi, qs, qg, qa, tz, gsize, h_var)
    ...
    real, intent (in) :: gsize

    real, intent (in), dimension (ks:ke) :: pz, den, h_var
...
subroutine linear_prof (km, q, dm, z_var, h_var)
    ...
    if (z_var) then
        ...
        ! -----------------------------------------------------------------------
        do k = 1, km
            dm (k) = max (dm (k), 0.0, h_var (k) * q (k))
        enddo
    else
        do k = 1, km
            dm (k) = max (0.0, h_var (k) * q (k))
        enddo
    endif
...
subroutine pgfr_simp (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, &
        lcpk, icpk, tcpk, tcp3, mppfr, convt)
...
subroutine psmlt_simp (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, cvm, te8, &
        lcpk, icpk, tcpk, tcp3, mppms, convt)
...
subroutine praut_simp (ks, ke, dts, dp, tz, qa, qv, ql, qr, qi, qs, qg, mppar, convt)
...
subroutine psaut_simp (ks, ke, dts, qa, qv, ql, qr, qi, qs, qg, dp, tz, den, mppas, convt)
...
subroutine qs_init
    ...
    if (.not. tables_are_initialized) then
        ...
        if (.not. allocated (table0)) allocate (table0 (es_table_length))
        if (.not. allocated (table1)) allocate (table1 (es_table_length))
        if (.not. allocated (table2)) allocate (table2 (es_table_length))
        if (.not. allocated (table3)) allocate (table3 (es_table_length))
        if (.not. allocated (table4)) allocate (table4 (es_table_length))

        if (.not. allocated (des0)) allocate (des0 (es_table_length))
        if (.not. allocated (des1)) allocate (des1 (es_table_length))
        if (.not. allocated (des2)) allocate (des2 (es_table_length))
        if (.not. allocated (des3)) allocate (des3 (es_table_length))
        if (.not. allocated (des4)) allocate (des4 (es_table_length))

        call qs_table0 (es_table_length)
        call qs_table1 (es_table_length)
        call qs_table2 (es_table_length)
        call qs_table3 (es_table_length)
        call qs_table4 (es_table_length)
        ...
        enddo
        des0 (es_table_length) = des0 (es_table_length - 1)
        des1 (es_table_length) = des1 (es_table_length - 1)
        des2 (es_table_length) = des2 (es_table_length - 1)
        des3 (es_table_length) = des3 (es_table_length - 1)
        des4 (es_table_length) = des4 (es_table_length - 1)
...
subroutine qs_table0 (n)
    ...
    real (kind = r8) :: tem, fac0, fac1, fac2
...
function es_core (tk, table, des)
    ...
    real, intent (in), dimension (es_table_length) :: table, des
    ...
    ! Ensures it is >= 1 AND it+1 <= es_table_length
