...
def check_config(
    ...
    if cumulus_parameterization_config.SGS_W_TIMESCALE != 1:
        ndsl_log.warning(
            " GF2020-->CumulusParameterization-->DiurnalCycle initalized with "
            "untested SGS_W_TIMESCALE option. Running untested code... proceed with caution"
        )

    if cumulus_parameterization_config.AEROEVAP != 1:
        ndsl_log.warning(
            " GF2020-->CumulusParameterization-->DowndraftWindshear initalized with "
    ...
    if cumulus_parameterization_config.USE_TRACER_EVAPORATION == 0:
        ndsl_log.warning(
            " GF2020-->CumulusParameterization-->AtmosphericComposition-->downdraft_chemistry initalized "
    ...
    if cumulus_parameterization_config.USE_TRACER_SCAVENGE != True:
        raise NotImplementedError(
            "[NDSL] GF2020-->CumulusParameterization-->AtmosphericComposition initalized with"
