    ...
    with computation(FORWARD), interval(0, -1):
        if error_code[0, 0][plume] == 0:
            if K <= cloud_top_level[0, 0][plume]:
                ...
                dts = dts - (((del_u_cloud_ensemble * u) + (del_v_cloud_ensemble * v)) * dp / constants.MAPL_GRAV)

                fpi = fpi + (sqrt((del_u_cloud_ensemble * del_u_cloud_ensemble) + (del_v_cloud_ensemble * del_v_cloud_ensemble)) * dp)

    with computation(PARALLEL), interval(...):
        if error_code[0, 0][plume] == 0:
            if fpi > 0.0:
                if K <= cloud_top_level[0, 0][plume]:
                    fp = sqrt((del_u_cloud_ensemble * del_u_cloud_ensemble + del_v_cloud_ensemble * del_v_cloud_ensemble)) / fpi

                    del_t_cloud_ensemble = del_t_cloud_ensemble + (fp * dts * constants.MAPL_GRAV / cumulus_parameterization_constants.CP)
