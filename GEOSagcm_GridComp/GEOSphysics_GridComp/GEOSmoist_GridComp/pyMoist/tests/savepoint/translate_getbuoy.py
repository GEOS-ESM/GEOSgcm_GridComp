...
class TranslateGetbuoy(TranslateFortranData2Py):
    ...
    def compute(self, inputs):
        pifc0_below = self.quantity_factory._numpy.empty((1, 1, 143), dtype=Float)
        pifc0_below[0, 0, :] = inputs["pifc0_below"]
        thv0bot = self.quantity_factory._numpy.empty((1, 1, 143), dtype=Float)
        thv0bot[0, 0, :] = inputs["thv0bot_buoy"]
        pifc0_above = self.quantity_factory._numpy.empty((1, 1, 143), dtype=Float)
        pifc0_above[0, 0, :] = inputs["pifc0_above"]
        thv0top = self.quantity_factory._numpy.empty((1, 1, 143), dtype=Float)
        thv0top[0, 0, :] = inputs["thv0top_buoy"]
        thvubot = self.quantity_factory._numpy.empty((1, 1, 143), dtype=Float)
        thvubot[0, 0, :] = inputs["thvubot_buoy"]
        thvutop = self.quantity_factory._numpy.empty((1, 1, 143), dtype=Float)
        thvutop[0, 0, :] = inputs["thvutop_buoy"]
        cin_in = self.quantity_factory._numpy.empty((1, 1, 143), dtype=Float)
        cin_in[0, 0, :] = inputs["cin_in"]
        plfc_in = self.quantity_factory._numpy.empty((1, 1, 143), dtype=Float)
        plfc_in[0, 0, :] = inputs["plfc_in"]
        


        plfc = self.quantity_factory._numpy.empty((1, 1, 143), dtype=Float)
        cin = self.quantity_factory._numpy.empty((1, 1, 143), dtype=Float)
