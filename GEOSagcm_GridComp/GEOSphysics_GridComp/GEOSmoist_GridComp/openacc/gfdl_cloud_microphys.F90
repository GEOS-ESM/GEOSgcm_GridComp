...
module gfdl2_cloud_microphys_mod
  ...
  use fms_mod, only: &
       ...
       check_nml_error, close_file, file_exist, fms_init
  ...
  !$acc declare create(missing_value, cracs, csacr, cgacr, cgacs, csacw, craci, &
  !$acc                csaci, cgacw, cgaci, cracw, acco, cssub, cgsub, crevp, &
  !$acc                cgfr, csmlt, cgmlt, es0, ces0, pie, rgrav, c_air, c_vap, &
  !$acc                lati, latv, lats, lat2, lcp, icp, tcp, d0_vap, lv00, &
  !$acc                icloud_f, irain_f, de_ice, sedi_transport, do_sedi_w, &
  !$acc                do_sedi_heat, prog_ccn, do_bigg, do_evap, do_subl, &
  !$acc                do_qa, preciprad, fix_negative, do_setup, p_nonhydro, &
  !$acc                table, table2, table3, tablew, des, des2, des3, desw, &
  !$acc                dt_fr, p_min, cld_min, tice, log_10, tice0, t_wfr, &
  !$acc                t_min, t_sub, mp_time, rh_inc, rh_inr, rh_ins, tau_r2g, &
  !$acc                tau_smlt, tau_g2r, tau_imlt, tau_i2s, tau_l2r, tau_v2l, &
  !$acc                tau_l2v, tau_i2v, tau_s2v, tau_v2s, tau_g2v, tau_v2g, &
  !$acc                tau_revp, tau_frz, dw_land, dw_ocean, ccn_o, ccn_l, &
  !$acc                rthreshu, rthreshs, sat_adj0, qc_crt, qi_lim, ql_mlt, &
  !$acc                qs_mlt, ql_gen, qi_gen, ql0_max, qi0_max, qi0_crt, &
  !$acc                qr0_crt, qs0_crt, c_paut, c_psaci, c_piacr, c_cracw, &
  !$acc                c_pgacs, c_pgaci, alin, clin, const_vi, const_vs, &
  !$acc                const_vg, const_vr, vi_fac, vs_fac, vg_fac, vr_fac, &
  !$acc                vi_max, vs_max, vg_max, vr_max, fast_sat_adj, &
  !$acc                z_slope_liq, z_slope_ice, use_ccn, use_ppm, mono_prof, &
  !$acc                mp_print, tables_are_initialized)
...
contains
  ...
  subroutine gfdl_cloud_microphys_driver (                               &
    ...
    ! -----------------------------------------------------------------------
    !$acc data  copyin(uin, vin, delp, pt, qv, ql, qr, qi, qs, qg, qa, qn, dz, &
    !$acc              area, land, cnv_fraction, srf_type, eis, rhcrit, ntimes) &
    !$acc      copyout(m2_rain, m2_sol, revap, isubl, w_var, vt_r, vt_s, vt_g, &
    !$acc              vt_i, qn2) &
    !$acc         copy(w, rain, snow, graupel, ice, cond, udt, vdt, pt_dt, &
    !$acc              qv_dt, ql_dt, qr_dt, qi_dt, qs_dt, qg_dt, qa_dt)
    call mpdrv (                                                            &
  ...
  ! -----------------------------------------------------------------------
  subroutine mpdrv ( &
       hydrostatic, uin, vin, w, delp, pt, qv, ql, qr, qi, qs, &
    ...
    !$acc     present(area1, land, cnv_fraction, srf_type, eis, rhcrit, uin, &
    !$acc             vin, delp, pt, dz, qv, qi, ql, qr, qs, qg, qa, qn, u_dt, &
    !$acc             v_dt, w, pt_dt, qa_dt, qv_dt, ql_dt, qr_dt, qi_dt, qs_dt, &
    !$acc             qg_dt, revap, isubl, rain, snow, ice, graupel, cond, &
    !$acc             w_var, vt_r, vt_s, vt_g, vt_i, qn2, m2_rain, m2_sol)
