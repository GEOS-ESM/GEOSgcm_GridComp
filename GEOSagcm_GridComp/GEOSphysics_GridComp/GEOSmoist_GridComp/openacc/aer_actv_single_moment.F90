...
!>----------------------------------------------------------------------------------------------------------------------
      ...
      ! pchakrab: integer, intent(inout) :: aero_aci
...
!$acc data present(AeroProps,NWFA,ple,t,q,sh,evap,zle,plo,qicn,qils,qlcn,qlls,tke, vvel,NACTL,NACTI,FRLAND) &
...
!$acc end parallel loop
      ...
      ! pchakrab: WHAT HAPPENED TO THIS BLOCK
      ! DO j=1,JM
      !   Do i=1,IM 
      !        k            = kpbli(i,j)
      !        tk           = T(i,j,k)              ! K
