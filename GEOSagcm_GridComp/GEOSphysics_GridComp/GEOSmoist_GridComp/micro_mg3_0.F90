...
!!!DONIFF   
   ...
   real(r8), parameter :: tmelt_in    = MAPL_TICE
...
!     nccons_in, nicons_in, ncnst_in, ninst_in, ngcons_in, ngnst_in, errstring)
  ...
  integer   :: kind         ! Kind used for reals
...
!====================================================================================
!====================================================================================
!!"Interface" for mg_micro.
!!Substeps MG2 and Macro-scale condensation and cloud fraction. 
! NO changes to prognostic variables are done here
...
!                             errstring, & ! Below arguments are "optional" (pass null pointers to omit).
      ...
      integer :: num_steps_micro, N_MICRO, K, nbincontactdustx   
       ...
       real, dimension(1, 1:LM) :: QRAIN_tmp, QSNOW_tmp, QGR_tmp, NRAIN_tmp, NSNOW_tmp, NGR_tmp, QLTOT_tmp, &    
            ...
            real :: DT_AUX, DT_MOIST ! "real" versions of the time steps
!!!!!!!!!!!!!!Initialize
      ...
      npccninr8  = max(npccninr8*cldfr8 - ncr8, 0.0)/DT_R8 
      naair8     = max(naair8*cldfr8 -  nir8, 0.0)/DT_R8 ! only for cirrus
       
    
     call accum_mg_tend(0) !initialize accum values

     DT_MOIST  =  DT_r8
     num_steps_micro = INT(DT_R8/DT_MICRO) + 1
...
!                             errstring, & ! Below arguments are "optional" (pass null pointers to omit).
           ...
           ! Substepping update
                  ...
                  ! prognostic vars
                          ...
                          QGR_tmp(1, 1:LM) = max( QGR_tmp(1, 1:LM) +    REAL(qgrtendr8(1, 1:LM)*DT_R8), 0.0) ! grid average 
                          NRAIN_tmp(1, 1:LM)  = max(NRAIN_tmp(1, 1:LM)  + REAL(nrtendr8(1, 1:LM)*DT_R8), 0.0)
                          NSNOW_tmp(1, 1:LM)  = max(NSNOW_tmp (1, 1:LM) + REAL(nstendr8(1, 1:LM)*DT_R8), 0.0) 
                          NGR_tmp(1, 1:LM)  = max(NGR_tmp(1, 1:LM)  + REAL(ngrtendr8(1, 1:LM)*DT_R8), 0.0)          
                          QLTOT_tmp(1, 1:LM)= max(QLTOT_tmp(1, 1:LM)  + REAL(qctendr8(1,1:LM)) * DT_R8, 0.0)
                          QITOT_tmp(1, 1:LM)= max(QITOT_tmp(1, 1:LM)  + REAL(qitendr8(1,1:LM)) * DT_R8, 0.0)    
                          Q1_tmp(1, 1:LM)  = MAX(Q1_tmp (1, 1:LM)  + REAL(qvlatr8(1,1:LM)) * DT_R8, 0.0)
                          TEMP_tmp(1, 1:LM)= TEMP_tmp(1, 1:LM)  + REAL(tlatr8(1,1:LM)) * DT_R8 / (MAPL_CP) 
...
!                   elsewhere                        
 ...
 !                 end where 
            ...
            call accum_mg_tend(2) ! output averages
            DT_R8 = DT_MOIST ! restore value   

!!!!!!!!!!!!!!! note: dxout, nxout and qxout variables are not averaged
 contains 
 
 subroutine accum_mg_tend(stage)

 integer, intent(in) :: stage

    if (stage .eq.0) then  !initialize
           
       qcsinksum_rate1ordr8_accum = 0.0 
       tlatr8_accum = 0.0 
       qvlatr8_accum = 0.0 
       qctendr8_accum = 0.0 
       qitendr8_accum = 0.0 
       ...
       precir8_accum = 0.0 
       nevaprr8_accum = 0.0 
       evapsnowr8_accum = 0.0                                                    
       prainr8_accum = 0.0 
       prodsnowr8_accum = 0.0 
       cmeoutr8_accum = 0.0 
       deffir8_accum = 0.0 
       pgamradr8_accum = 0.0 
       lamcradr8_accum = 0.0 
       ...
       reff_rainr8_accum = 0.0 
       reff_snowr8_accum = 0.0
       reff_graur8_accum = 0.0
       ...
       umrr8_accum = 0.0 
       umsr8_accum = 0.0 
       qcsedtenr8_accum = 0.0 
       qisedtenr8_accum = 0.0 
       qrsedtenr8_accum = 0.0 
       qgsedtenr8_accum = 0.0
       qssedtenr8_accum = 0.0 
       praor8_accum = 0.0 
       prcor8_accum = 0.0 
       mnuccctotr8_accum = 0.0 
       mnucctor8_accum = 0.0 
       msacwior8_accum = 0.0 
       psacwsor8_accum = 0.0 
       bergsor8_accum = 0.0 
       ...
       prcior8_accum = 0.0 
       praior8_accum = 0.0 
       qirestotr8_accum = 0.0 
       mnuccctotr8_accum = 0.0 
       pracstotr8_accum = 0.0 
       meltsdtr8_accum = 0.0 
       frzrdtr8_accum = 0.0 
       mnuccdor8_accum = 0.0 
       nroutr8_accum = 0.0 
       ...
       pracgtotr8_accum = 0.0            
       psacwgtotr8_accum = 0.0          
       pgsacwtotr8_accum = 0.0          
       pgracstotr8_accum = 0.0           
       prdgtotr8_accum = 0.0            
       qmultgtotr8_accum = 0.0           
       qmultrgtotr8_accum = 0.0          
       psacrtotr8_accum = 0.0 
       npracgtotr8_accum = 0.0           
       nscngtotr8_accum = 0.0            
       ngracstotr8_accum = 0.0           
       nmultgtotr8_accum = 0.0           
       nmultrgtotr8_accum = 0.0          
       npsacwgtotr8_accum = 0.0      
                             
                             
    elseif  (stage .eq.1)  then!accumulate
      
       qcsinksum_rate1ordr8_accum = qcsinksum_rate1ordr8_accum  +qcsinksum_rate1ordr8/num_steps_micro
       tlatr8_accum =  tlatr8_accum +  tlatr8/num_steps_micro
       qvlatr8_accum = qvlatr8_accum + qvlatr8/num_steps_micro
       qctendr8_accum = qctendr8_accum + qctendr8/num_steps_micro
       qitendr8_accum = qitendr8_accum + qitendr8/num_steps_micro
       nctendr8_accum = nctendr8_accum + nctendr8/num_steps_micro
       nitendr8_accum =  nitendr8_accum +  nitendr8/num_steps_micro
       qrtendr8_accum = qrtendr8_accum + qrtendr8/num_steps_micro
       qgrtendr8_accum = qgrtendr8_accum + qgrtendr8/num_steps_micro
       qstendr8_accum = qstendr8_accum + qstendr8/num_steps_micro
       nrtendr8_accum = nrtendr8_accum +nrtendr8/num_steps_micro
       nstendr8_accum = nstendr8_accum + nstendr8/num_steps_micro
       ngrtendr8_accum =  ngrtendr8_accum  +  ngrtendr8/num_steps_micro 
        
       effcr8_accum = effcr8_accum  + effcr8/num_steps_micro
       effc_fnr8_accum = effc_fnr8_accum +effc_fnr8/num_steps_micro
       effir8_accum = effir8_accum + effir8/num_steps_micro
       sadicer8_accum = sadicer8_accum + sadicer8/num_steps_micro
       sadsnowr8_accum = sadsnowr8_accum + sadsnowr8/num_steps_micro
       prectr8_accum = prectr8_accum  + prectr8/num_steps_micro
       precir8_accum = precir8_accum  + precir8/num_steps_micro
       nevaprr8_accum = nevaprr8_accum + nevaprr8/num_steps_micro
       evapsnowr8_accum = evapsnowr8_accum + evapsnowr8/num_steps_micro
       prainr8_accum = prainr8_accum + prainr8/num_steps_micro
       prodsnowr8_accum = prodsnowr8_accum + prodsnowr8/num_steps_micro
       cmeoutr8_accum = cmeoutr8_accum + cmeoutr8/num_steps_micro
       deffir8_accum = deffir8_accum + deffir8/num_steps_micro
       pgamradr8_accum = pgamradr8_accum + pgamradr8/num_steps_micro
       lamcradr8_accum = lamcradr8_accum +lamcradr8/num_steps_micro
       qsoutr8_accum = qsoutr8_accum + qsoutr8/num_steps_micro
       dsoutr8_accum = dsoutr8_accum + dsoutr8/num_steps_micro
       lflxr8_accum = lflxr8_accum + lflxr8/num_steps_micro
       iflxr8_accum = iflxr8_accum + iflxr8/num_steps_micro
       rflxr8_accum = rflxr8_accum + rflxr8/num_steps_micro
       sflxr8_accum = sflxr8_accum + sflxr8/num_steps_micro
       gflxr8_accum = gflxr8_accum + gflxr8/num_steps_micro
       
       
       
       reff_rainr8_accum = reff_rainr8_accum  +reff_rainr8/num_steps_micro
       reff_snowr8_accum =  reff_snowr8_accum  +   reff_snowr8/num_steps_micro
       reff_graur8_accum =   reff_graur8_accum +  reff_graur8_accum/num_steps_micro
       
       qcsevapr8_accum = qcsevapr8_accum  + qcsevapr8/num_steps_micro
       qisevapr8_accum = qisevapr8_accum  + qisevapr8/num_steps_micro 
       qvresr8_accum = qvresr8_accum  + qvresr8/num_steps_micro
       cmeioutr8_accum = cmeioutr8_accum +cmeioutr8/num_steps_micro
       vtrmcr8_accum = vtrmcr8_accum + vtrmcr8/num_steps_micro
       vtrmir8_accum = vtrmir8_accum + vtrmir8/num_steps_micro
       umrr8_accum = umrr8_accum + umrr8/num_steps_micro
       umsr8_accum = umsr8_accum + umsr8/num_steps_micro
       qcsedtenr8_accum = qcsedtenr8_accum + qcsedtenr8/num_steps_micro
       qisedtenr8_accum = qisedtenr8_accum + qisedtenr8/num_steps_micro
       qrsedtenr8_accum =  qrsedtenr8_accum + qrsedtenr8/num_steps_micro
       qssedtenr8_accum = qssedtenr8_accum + qssedtenr8/num_steps_micro
       praor8_accum = praor8_accum + praor8/num_steps_micro
       prcor8_accum = prcor8_accum + prcor8/num_steps_micro
       mnuccctotr8_accum = mnuccctotr8_accum + mnuccctotr8/num_steps_micro
       mnucctor8_accum = mnucctor8_accum + mnucctor8/num_steps_micro
       msacwior8_accum = msacwior8_accum + msacwior8/num_steps_micro
       psacwsor8_accum = psacwsor8_accum + psacwsor8/num_steps_micro
       bergsor8_accum = bergsor8_accum + bergsor8/num_steps_micro
       bergor8_accum = bergor8_accum + bergor8/num_steps_micro
       meltor8_accum = meltor8_accum + meltor8/num_steps_micro
       homoor8_accum = homoor8_accum + homoor8/num_steps_micro
       qcresor8_accum = qcresor8_accum + qcresor8/num_steps_micro
       prcior8_accum = prcior8_accum + prcior8/num_steps_micro
       praior8_accum = praior8_accum + praior8/num_steps_micro
       qirestotr8_accum = qirestotr8_accum +qirestotr8/num_steps_micro
       mnuccctotr8_accum = mnuccctotr8_accum + mnuccctotr8/num_steps_micro
       pracstotr8_accum = pracstotr8_accum  + pracstotr8/num_steps_micro
       meltsdtr8_accum = meltsdtr8_accum +meltsdtr8/num_steps_micro
       frzrdtr8_accum = frzrdtr8_accum + frzrdtr8/num_steps_micro
       mnuccdor8_accum = mnuccdor8_accum + mnuccdor8/num_steps_micro
       nroutr8_accum = nroutr8_accum + nroutr8/num_steps_micro
       nsoutr8_accum = nsoutr8_accum + nsoutr8/num_steps_micro
       reflr8_accum = reflr8_accum + reflr8/num_steps_micro
       areflr8_accum = areflr8_accum + areflr8/num_steps_micro
       areflzr8_accum = areflzr8_accum + areflzr8/num_steps_micro
       freflr8_accum = freflr8_accum + freflr8/num_steps_micro
       csrflr8_accum = csrflr8_accum + csrflr8/num_steps_micro
       acsrflr8_accum = acsrflr8_accum + acsrflr8/num_steps_micro
       fcsrflr8_accum = fcsrflr8_accum + fcsrflr8/num_steps_micro
       rercldr8_accum = rercldr8_accum + rercldr8/num_steps_micro
       ncair8_accum = ncair8_accum +ncair8/num_steps_micro
       ncalr8_accum = ncalr8_accum + ncalr8/num_steps_micro
             
       freqsr8_accum = freqsr8_accum + freqsr8/num_steps_micro
       freqrr8_accum = freqrr8_accum + freqrr8/num_steps_micro
       freqgr8_accum = freqgr8_accum + freqgr8/num_steps_micro
              
       nficer8_accum = nficer8_accum + nficer8/num_steps_micro
       npccnor8_accum = npccnor8_accum+ npccnor8/num_steps_micro
       npsacwsor8_accum = npsacwsor8_accum  +npsacwsor8/num_steps_micro
       npraor8_accum = npraor8_accum+ npraor8/num_steps_micro
       nsubcor8_accum = nsubcor8_accum + nsubcor8/num_steps_micro
       nprc1or8_accum = nprc1or8_accum + nprc1or8/num_steps_micro
       npraior8_accum = npraior8_accum + npraior8/num_steps_micro
       nnucctor8_accum = nnucctor8_accum + nnucctor8/num_steps_micro
       nnucccor8_accum = nnucccor8_accum + nnucccor8/num_steps_micro
       nnuccdor8_accum = nnuccdor8_accum+ nnuccdor8/num_steps_micro
       nsubior8_accum = nsubior8_accum + nsubior8/num_steps_micro
       nprcior8_accum = nprcior8_accum + nprcior8/num_steps_micro
       nsacwior8_accum = nsacwior8_accum + nsacwior8/num_steps_micro
       
       pracgtotr8_accum = pracgtotr8_accum  + pracgtotr8/num_steps_micro             
       psacwgtotr8_accum = psacwgtotr8_accum + psacwgtotr8/num_steps_micro       
       pgsacwtotr8_accum = pgsacwtotr8_accum + pgsacwtotr8/num_steps_micro        
       pgracstotr8_accum = pgracstotr8_accum  + pgracstotr8/num_steps_micro            
       prdgtotr8_accum = prdgtotr8_accum + prdgtotr8/num_steps_micro            
       qmultgtotr8_accum = qmultgtotr8_accum + qmultgtotr8/num_steps_micro          
       qmultrgtotr8_accum = qmultrgtotr8_accum + qmultrgtotr8/num_steps_micro          
       psacrtotr8_accum = psacrtotr8_accum + psacrtotr8/num_steps_micro
       npracgtotr8_accum = npracgtotr8_accum + npracgtotr8/num_steps_micro          
       nscngtotr8_accum = nscngtotr8_accum + nscngtotr8/num_steps_micro          
       ngracstotr8_accum = ngracstotr8_accum + ngracstotr8/num_steps_micro           
       nmultgtotr8_accum = nmultgtotr8_accum + nmultgtotr8/num_steps_micro          
       nmultrgtotr8_accum = nmultrgtotr8_accum + nmultrgtotr8/num_steps_micro       
       npsacwgtotr8_accum = nmultrgtotr8_accum + nmultrgtotr8/num_steps_micro   
       
       
    else  !finish
    
       qcsinksum_rate1ordr8 = qcsinksum_rate1ordr8_accum 
       tlatr8 =  tlatr8_accum 
       qvlatr8 = qvlatr8_accum 
       qctendr8 = qctendr8_accum 
       qitendr8 = qitendr8_accum 
       nctendr8 = nctendr8_accum 
       nitendr8 =  nitendr8_accum
       qrtendr8 = qrtendr8_accum 
       qstendr8 = qstendr8_accum 
       nrtendr8 = nrtendr8_accum 
       nstendr8 = nstendr8_accum 
       ngrtendr8 = ngrtendr8_accum
              
       effcr8 = effcr8_accum 
       effc_fnr8 = effc_fnr8_accum 
       effir8 = effir8_accum
       sadicer8 = sadicer8_accum 
       sadsnowr8 = sadsnowr8_accum 
       prectr8 = prectr8_accum  
       precir8 = precir8_accum  
       nevaprr8 = nevaprr8_accum 
       evapsnowr8 = evapsnowr8_accum 
       prainr8 = prainr8_accum 
       prodsnowr8 = prodsnowr8_accum 
       cmeoutr8 = cmeoutr8_accum 
       deffir8 = deffir8_accum 
       pgamradr8 = pgamradr8_accum 
       lamcradr8 = lamcradr8_accum 
       qsoutr8 = qsoutr8_accum
       dsoutr8 = dsoutr8_accum 
       lflxr8 = lflxr8_accum 
       iflxr8 = iflxr8_accum 
       rflxr8 = rflxr8_accum
       sflxr8 = sflxr8_accum
       ...
       qcsevapr8 = qcsevapr8_accum 
       qisevapr8 = qisevapr8_accum 
       qvresr8 = qvresr8_accum  
       cmeioutr8 = cmeioutr8_accum 
       vtrmcr8 = vtrmcr8_accum 
       vtrmir8 = vtrmir8_accum 
       umrr8 = umrr8_accum 
       umsr8 = umsr8_accum 
       qcsedtenr8 = qcsedtenr8_accum 
       qisedtenr8 = qisedtenr8_accum 
       qrsedtenr8 =  qrsedtenr8_accum
       qssedtenr8 = qssedtenr8_accum 
       praor8 = praor8_accum
       prcor8 = prcor8_accum
       mnuccctotr8 = mnuccctotr8_accum
       mnucctor8 = mnucctor8_accum 
       msacwior8 = msacwior8_accum 
       psacwsor8 = psacwsor8_accum 
       bergsor8 = bergsor8_accum 
       bergor8 = bergor8_accum 
       meltor8 = meltor8_accum 
       homoor8 = homoor8_accum 
       qcresor8 = qcresor8_accum 
       prcior8 = prcior8_accum 
       praior8 = praior8_accum 
       qirestotr8 = qirestotr8_accum
       mnuccctotr8 = mnuccctotr8_accum 
       pracstotr8 = pracstotr8_accum  
       meltsdtr8 = meltsdtr8_accum 
       frzrdtr8 = frzrdtr8_accum 
       mnuccdor8 = mnuccdor8_accum 
       nroutr8 = nroutr8_accum 
       nsoutr8 = nsoutr8_accum 
       reflr8 = reflr8_accum 
       areflr8 = areflr8_accum
       areflzr8 = areflzr8_accum 
       freflr8 = freflr8_accum
       csrflr8 = csrflr8_accum 
       acsrflr8 = acsrflr8_accum 
       fcsrflr8 = fcsrflr8_accum 
       rercldr8 = rercldr8_accum 
       ncair8 = ncair8_accum
       ncalr8 = ncalr8_accum 
       
       drout2r8 = drout2r8_accum 
       dsout2r8 = dsout2r8_accum 
       dgout2r8 = dgout2r8_accum 
              
       freqsr8 = freqsr8_accum 
       freqrr8 = freqrr8_accum
       freqgr8 = freqgr8_accum 
              
       nficer8 = nficer8_accum 
       npccnor8 = npccnor8_accum
       npsacwsor8 = npsacwsor8_accum 
       npraor8 = npraor8_accum
       nsubcor8 = nsubcor8_accum 
       nprc1or8 = nprc1or8_accum
       npraior8 = npraior8_accum
       nnucctor8 = nnucctor8_accum
       nnucccor8 = nnucccor8_accum
       nnuccdor8 = nnuccdor8_accum
       nsubior8 = nsubior8_accum
       nprcior8 = nprcior8_accum
       nsacwior8 = nsacwior8_accum
       
       pracgtotr8   = pracgtotr8_accum          
       psacwgtotr8  = psacwgtotr8_accum      
       pgsacwtotr8  = pgsacwtotr8_accum 
       pgracstotr8  = pgracstotr8_accum  
       prdgtotr8    = prdgtotr8_accum 
       qmultgtotr8  = qmultgtotr8_accum 
       qmultrgtotr8 = qmultrgtotr8_accum 
       psacrtotr8   = psacrtotr8_accum 
       npracgtotr8  = npracgtotr8_accum 
       nscngtotr8   = nscngtotr8_accum 
       ngracstotr8  = ngracstotr8_accum 
       nmultgtotr8  = nmultgtotr8_accum 
       nmultrgtotr8 = nmultrgtotr8_accum 
       npsacwgtotr8 = nmultrgtotr8_accum 
       
    end if 
    
    
    end subroutine accum_mg_tend

end subroutine micro_mg_tend_interface
...
!!!DONIFF
      ...
      real(r8), intent(out) ::  nnuccdo(mgncol,nlev)
      real(r8), intent(out) ::  nnuccco(mgncol,nlev)
      real(r8), intent(out) ::  nsacwio(mgncol,nlev)
      real(r8), intent(out) ::  nsubio(mgncol,nlev)
      real(r8), intent(out) ::  nprcio(mgncol,nlev)
      real(r8), intent(out) ::  npraio(mgncol,nlev)
      

      real(r8), intent(out) ::  npccno(mgncol,nlev)
      real(r8), intent(out) ::  npsacwso(mgncol,nlev)
      real(r8), intent(out) ::  nsubco(mgncol,nlev)
      real(r8), intent(out) ::  nprao(mgncol,nlev)
...
!          ncic(:,k), rho(:,k), relvar(:,k), prc(:,k), nprc(:,k), nprc1(:,k), mgncol)
       ...
       call liu2006_liq_autoconversion(pgam(:,k), microp_uniform, qcic(:,k), & !DONIF
          ncic(:,k), rho(:,k), relvar(:,k), prc(:,k), nprc(:,k), nprc1(:,k), mgncol)

          
          prc(:,k) = prc(:,k)*dcrit
          nprc(:,k) = nprc(:,k)*dcrit
          nprc1(:,k) = nprc1(:,k)*dcrit
...
! donif     
    ...
    ! donif     
         nacon(:,k,nbincontactdust + 1)  = nsoot(:,k)  !Add soot. 
               rndst(:,k,nbincontactdust + 1)  = rnsoot(:,k) 
          mdust = nbincontactdust + 1
           call contact_freezing(microp_uniform, t(:,k), p(:,k), rndst(:,k,:), &
                nacon(:,k,:), pgam(:,k), lamc(:,k), qcic(:,k), ncic(:,k), &
                relvar(:,k), mnucct(:,k), nnucct(:,k), mgncol, mdust)  ! do contact here since it depends on size dist
...
! DONIF additional diagnostics for number (some may be redundant)
               ...
               nprcio(i,k)=nprcio(i,k)+ nprci(i, k)*icldm(i,k)!DONIF
               npraio(i,k)=npraio(i,k)+ nprai(i, k)*icldm(i,k)!DONIF

               npccno(i, k)= npccno(i,k)+ npccn(i, k)!DONIF
               npsacwso(i,k)=npsacwso(i,k)+ npsacws(i, k)*lcldm(i,k)!DONIF
               nsubco(i,k)=nsubco(i,k)+ nsubc(i, k)*lcldm(i,k)!DONIF
               nprao(i,k)=nprao(i,k)+ npra(i, k)*lcldm(i,k)!DONIF
               nprc1o(i,k)=nprc1o(i,k)+ nprc1(i, k)*lcldm(i,k)!DONIF   
...
!          lami(:,k), mgncol)
     ...
     call size_dist_param_ice(mg_ice_props, dumi(:,k), dumni(:,k), & !DONIF
          lami(:,k), mgncol)
  ...
  do k=1,nlev
     do i=1,mgncol
        ...
        if (dumi(i,k).ge.qsmall) then
         ...
         mu_ice =  mui_of_l(lami(i, k))
         aux=gamma(4._r8+bi+mu_ice)/gamma(4._r8+mu_ice)  !DONIF         
         vtrmi(i,k)=  ain(i,k)*aux/(lami(i, k)**bi)
         fi(i,k) = g*rho(i,k)*vtrmi(i,k)
        
               
!        vtrmi(i,k)=min(ain(i,k)*gamma_bi_plus4/(6._r8*lami(i,k)**bi), &
!                1.2_r8*rhof(i,k))
          aux=gamma(1._r8+bi+mu_ice)/gamma(1._r8+mu_ice) !DONIF                
          fni(i,k) = g*rho(i,k)* &
                min(ain(i,k)*aux/(lami(i, k)**bi),1.2_r8*rhof(i,k))
...
!DONIFFFF tune up sedimentation vel!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            fi(i,k)= fi(i,k)*ui_scale
            fni(i,k)= fni(i,k)*ui_scale
     ...
     do k=1,nlev
        do i=1,mgncol
           if (dumi(i,k).ge.qsmall) then
             ...
             ! call size_dist_param_basic(mg_ice_props, dumi(i,k), dumni(i,k), & DONIF
             !      lami(i,k), dumni0)
             
              call size_dist_param_ice(mg_ice_props, dumi(i,k), dumni(i,k), & !DONIF
                   lami(i,k), dumni0)
              ...
              !effi(i,k) = 1.5_r8/lami(i,k)*1.e6_r8
               effi(i,k) = 1.e6_r8*gamma(4._r8+mu_ice)/(2.0*lami(i, k)*gamma(3._r8+mu_ice))  !DONIF
                          
                         
              sadice(i,k) = 2._r8*pi*(lami(i,k)**(-3))*dumni0*rho(i,k)*1.e-2_r8  ! m2/m3 -> cm2/cm3 ! Check DONIF
