...
subroutine MGB2_2M_Initialize (MAPL, RC)
    ...
    call MAPL_GetResource(MAPL, MAX_CNVNDROP, 'MAX_CNVNDROP:', DEFAULT= 5000.e6 ,RC=STATUS) !scaling ice eff radius in cnv
    call MAPL_GetResource(MAPL, MAX_CNVNICE, 'MAX_CNVNICE:', DEFAULT= 5.e6 ,RC=STATUS) !scaling ice eff radius in cnv
    ...
    call MAPL_GetResource(MAPL, WSUB_OPTION,  'WSUB_OPTION:',   DEFAULT= 2 , __RC__) !0- param 1- Use Wsub climatology 2-Wnet
    call MAPL_GetResource(MAPL, USE_NCLOUD_CLIM,  'USE_NCLOUD_CLIM:',   DEFAULT= .FALSE.,    __RC__) !0- param 1- Use Wsub climatology 2-Wnet
    call MAPL_GetResource(MAPL, SECOND_HYSTPDF, 'SECOND_HYSTPDF:', DEFAULT= .FALSE. ,RC=STATUS) !drop vol radius in cnv
    call MAPL_GetResource(MAPL, ITER_METHOD, 'ITER_METHOD:', DEFAULT= 1 ,RC=STATUS) !iteration method in hystpdf
    call MAPL_GetResource(MAPL, DO_UPD_CLD, 'DO_UPD_CLD:', DEFAULT= .TRUE. ,RC=STATUS) !drop vol radius in cnv
    
    call MAPL_GetResource(MAPL, MAKE_SNOW_ICE,  'MAKE_SNOW_ICE:',   DEFAULT= .FALSE.,    __RC__) !treat snow radiatively as ice
    

    call MAPL_GetResource(MAPL, ST_OPTION, 'ST_OPTION:', DEFAULT= 0 ,RC=STATUS) !0 uses LTS, 1 uses EIS for CF correction
