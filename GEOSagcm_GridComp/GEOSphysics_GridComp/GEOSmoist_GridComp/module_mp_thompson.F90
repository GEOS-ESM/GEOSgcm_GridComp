...
!..Write tables with master task after computing them in thompson_init
...
!+---+-----------------------------------------------------------------+
      ...
      ! Standard tables are only written by master task;
      ...
      if (amroot) write (*,*)'creating microphysics lookup tables ... '
      if (amroot) write (*,'(a, f5.2, a, f5.2, a, f5.2, a, f5.2)') &
...
!! further changes by Eidhammer and Kriedenweis
      if (amroot) write(*,*) '  calling table_ccnAct routine'
...
!! between rain/snow and cloud water
      if (amroot) write(*,*) '  creating qc collision eff tables'
      call table_Efrw
      call table_Efsw

!>  - Call table_dropevap() to creat rain drop evaporation table
      if (amroot) write(*,*) '  creating rain evap table'
      call table_dropEvap

!>  - Call qi_aut_qs() to create conversion of some ice mass into snow category
      if (amroot) write(*,*) '  creating ice converting to snow table'
      call qi_aut_qs

      call cpu_time(etime)
      if (amroot) print '("Calculating Thompson tables part 1 took ",f10.3," seconds.")', etime-stime
...
!>  - Call radar_init() to initialize various constants for computing radar reflectivity
      ...
      call radar_init
      call cpu_time(etime)
      if (amroot) print '("Calling radar_init took ",f10.3," seconds.")', etime-stime
...
!>  - Call qr_acr_qg() to create rain collecting graupel & graupel collecting rain table
      if (amroot) write(*,*) '  creating rain collecting graupel table'
      call cpu_time(stime)
      call qr_acr_qg
      call cpu_time(etime)
      if (amroot) print '("Computing rain collecting graupel table took ",f10.3," seconds.")', etime-stime

!>  - Call qr_acr_qs() to create rain collecting snow & snow collecting rain table
      if (amroot) write (*,*) '  creating rain collecting snow table'
      call cpu_time(stime)
      call qr_acr_qs
      call cpu_time(etime)
      if (amroot) print '("Computing rain collecting snow table took ",f10.3," seconds.")', etime-stime

!>  - Call freezeh2o() to create cloud water and rain freezing (Bigg, 1953) table
      if (amroot) write(*,*) '  creating freezing of water drops table'
      call cpu_time(stime)
      call freezeH2O(threads)
      call cpu_time(etime)
      if (amroot) print '("Computing freezing of water drops table took ",f10.3," seconds.")', etime-stime

      call cpu_time(etime)
      if (amroot) print '("Calculating Thompson tables part 2 took ",f10.3," seconds.")', etime-stime

      end if precomputed_tables_2

      endif if_not_iiwarm

      if (amroot) write(*,*) ' ... DONE microphysical lookup tables'
...
!..Subroutine arguments
      ...
      REAL, DIMENSION(:,:), OPTIONAL, INTENT(IN) :: rand_pert
      REAL, DIMENSION(:), OPTIONAL, INTENT(IN) :: spp_prt_list, spp_stddev_cutoff
      CHARACTER(len=3), DIMENSION(:), OPTIONAL, INTENT(IN) :: spp_var_list
...
!.. number tendency (number per kg per second).
         ...
         if (it==ndt) then
           RAINNC(i,j) = pcp_ra(i,j)
           IF ( PRESENT(snownc) ) THEN
              ! Add ice to snow if separate ice not present
              IF ( .NOT.PRESENT(icenc) ) THEN
                 SNOWNC(i,j) = pcp_sn(i,j) + pcp_ic(i,j)
              ELSE
                 SNOWNC(i,j) = pcp_sn(i,j) 
              ENDIF
           ENDIF
           ! Use separate ice if present
           IF ( PRESENT(icenc) ) THEN
              ICENC(i,j) = pcp_ic(i,j)
           ENDIF
           IF ( PRESENT(graupelnc) ) THEN
              GRAUPELNC(i,j) = pcp_gr(i,j)
           ENDIF
           SR(i,j) = (            + pcp_sn(i,j) + pcp_gr(i,j) + pcp_ic(i,j)         ) / &
                     (pcp_ra(i,j) + pcp_sn(i,j) + pcp_gr(i,j) + pcp_ic(i,j) + 1.e-12)
         endif 
...
!> - Call calc_refl10cm()
           ...
           if (diagflag .and. do_radar_ref) then
!
