...
!.. USE GTMP_2_GFCONVPAR, only : GTMP_2_GFCONVPAR_interface
 ...
 INTEGER :: CONVECTION_TRACER  ! = 1/0               !- turn ON/OFF the "convection" tracer

 INTEGER :: USE_SCALE_DEP      != 0/1:  scale dependence flag, default = 1

 INTEGER :: DICYCLE            != 0/1:  diurnal cycle    flag, default = 1

 INTEGER :: DOWNDRAFT          !- 0/1: turn ON/OFF downdrafts, default = 1 

 INTEGER :: CLEV_GRID          !- 0/1/2: interpolation method to define environ state 
                               !-        at the cloud levels (at face layer), default = 0 
 ...
 INTEGER :: SATUR_CALC         !- 0/1: 1=new saturation specific humidity calculation, default = 0
...
!-- General control for the diverse options in GF
 LOGICAL, PARAMETER :: ENTRNEW        = .TRUE.  !- new entr formulation (1/z)
 ...
 !-autonversion formulation: (1,5) Kessler, (2) Berry, (3) NOAA, (4) Sundvisqt, (6) Kessler with analytic solution 
 ...
 !- gradient of updraft (Zhang and Wu (ZW), 2003, JAS)
!REAL, PARAMETER ::    pgcd=1., pgcon=-0.55 , lambau_deep=0.0 !for ZW CMT
 REAL, PARAMETER ::    pgcd=1., pgcon=0.    , lambau_deep=2.0 , lambau_shdn=2.0
...
!CHARACTER(LEN=20),PARAMETER  :: clev_grid = 1
!--for GATE soundings only
!CHARACTER(LEN=20),PARAMETER  :: clev_grid = 0
!-- New interpolation method based on Tiedtke (1989)
!CHARACTER(LEN=20),PARAMETER  :: clev_grid = 2
...
!---------------------------------------------------------------------------------------------------
  ...
  SUBROUTINE GF_GEOS5_INTERFACE(mxp,myp,mzp,mtp,ITRCR,LONS,LATS,DT_MOIST          &
                               ...
                               ,TRACER,FSCAV,CNAMES,QNAMES,DTRDT_GF,TPWI,TPWI_star&
                               ,RSU_CN,REV_CN, PFI_CN, PFL_CN                     &
			       ,VAR3d,VAR2d,ZKBCON,LIGHTN_DENS)
			       !,CNV_TR)
    ...
    REAL   ,DIMENSION(mxp,myp,mzp)   ,INTENT(INOUT):: TH1,Q1,U1,V1,QLCN ,QICN, NCPL, NCPI    &
                                                     ,QLLS,QILS,CLLS
    REAL   ,DIMENSION(mxp,myp,mzp)   ,INTENT(INOUT):: RSU_CN,REV_CN,VAR3d
    REAL   ,DIMENSION(mxp,myp,0:mzp) ,INTENT(INOUT):: PFI_CN, PFL_CN

    REAL   ,DIMENSION(mxp,myp,mzp)                 :: CNV_TR  !temporary
!   REAL   ,DIMENSION(mxp,myp,mzp)   ,INTENT(INOUT):: CNV_TR  
    ...
    REAL,  DIMENSION(mzp , mxp, myp ) ::          &
                                        ...
                                        ,VAR3d_GF   ! dummy 3-d var for output
...
!-initialization
 ...
 !                ,sflux_r(1,1),sflux_t(1,1),topt(1,1),xland(1,1),sfc_press(1,1),dx(1),kpbl(1,1),temp2m(1,1),dt_moist&
 !                )
 !  ENDIF
...
!-----
    ...
    !--- for dummy output
	  ...
	  if(ierr4d(i,j,deep) .ne. 0) cycle
	  DO k=1,mzp
             VAR3d  (i,j,k) = VAR3d_GF(flip(k),i,j)
          ENDDO
         ENDDO
       ENDDO
           ...
           DO j=1,myp
            DO i=1,mxp
              ...
              DO k=1,mzp
                ...
                ! iens =?
                !src_cnvtr =  xmb4d(i,j,iens)* edt4d(i,j,iens)*zdn5d(i,j,flip(k),iens)     
        
                !- downdraft detrainment mass flux [kg m^{-2} s^{-1}]
                ! iens =?
                !src_cnvtr = xmb4d(i,j,iens)* edt4d(i,j,iens)*dd_massdetr5d(i,j,flip(k),iens)
...
!---------------------------------------------------------------------------------------------------
  ...
  !------------------------------------------------------------------------
   ...
   REAL,    DIMENSION(its:ite,jts:jte), INTENT(IN) :: topt ,aot500 ,temp2m ,sfc_press &
                                                     ,sflux_r ,sflux_t ,xland,lons,lats,dx2d,col_sat
   REAL,    DIMENSION(kts:kte,its:ite,jts:jte), INTENT(IN) ::     &
                                                         rthften  &
                                                        ,rqvften  &
                                                        ,rthblten &
...
!
    ...
    REAL,   DIMENSION (its:ite,kts:kte) ::                                      &
			      ...
			      ,buoy_exc2d,revsu_gf_2d,prfil_gf_2d,var3d_gf_2d   &
                              ,temp_new,qv_new
...
!
         ...
         po      (i,k)  = press(kr,i,j)*1.e-2 !mbar/hPa
         temp_old(i,k)  = temp(kr,i,j)        ! K

         qv_old  (i,k)  = rvap     (kr,i,j) ! @ begin of the timestep (kg/kg)
     ...
     if(use_gate) then
	  ...
	  CHEM_NAME_MASK (:) = 1 
          ...
          DO i=1,mtp
              ...
              FSCAV(i) = 0.  !km^-1
              Hcts(i)%hstar  = 8.300e+4! 2.4E+3 !59.
	      Hcts(i)%dhr    = 7400.   !5000.  !4200.
	      Hcts(i)%ak0    = 0.0
	      Hcts(i)%dak    = 0.0
...
!--temp
!
!              se_chem(1:mtp,i,kts:kte) = 0.
!              DO k=kts,ktf 
!	       po       (1,k) = 0.5*(ppres(1,k)+ppres(1,k+1))
!               if(po(1,k) > 500. .or. po(1,k)<300.) cycle
!               se_chem(1:mtp,i,k)=1.
!              ENDDO
!              se_chem=0.1*se_chem
!--temp
       ...
       do i=its,itf
         do k=kts,kte-1
           !po       (i,k) = ppres(jlx,k)
           po       (i,k) = 0.5*(ppres(jlx,k)+ppres(jlx,k+1))
        ...
        if(.not. use_gate) then 
          ...
          do k=kts,ktf
            do i=its,itf
              ...
              temp_new(i,k)=max(200.     ,temp_new(i,k))
                qv_new(i,k)=max(smallerQV,  qv_new(i,k))
             enddo
           enddo
        endif
     ...
     DO i = its,itf
      ...
      DO k = kts,kte
            ...
            !---these arrays are only for the deep plume mode	    
             ...
             VAR3d_GF (kr,i,j)= var3d_gf_2d(i,k)               !-- vertical velocity of the deep plume
      ENDDO
     ENDDO
     IF(MOMENTUM) THEN
      DO i = its,itf
...
!---------------------------------------------------------------------------------------------------
  ...
  ! pre    = output precip
     REAL,    DIMENSION (its:ite,kts:kte) ,INTENT (INOUT)   ::       &
        outu,outv,outt,outq,outqc,outbuoy,revsu_gf,prfil_gf,var3d_gf  
  ...
  !
     REAL,    DIMENSION (its:ite,kts:kte)       ,INTENT (IN   )    ::   &
        dhdt,rho,t,po,us,vs,tn,dm2d,buoy_exc 
     REAL,    DIMENSION (its:ite,kts:kte,1:ens4),INTENT (INOUT)    ::   &
        omeg
     REAL,    DIMENSION (its:ite,kts:kte)       ,INTENT (INOUT)    ::   &
         q,qo
     REAL,    DIMENSION (its:ite)               ,INTENT (IN   )    ::   &
        ccn,Z1,PSUR,xland,xlons,xlats, h_sfc_flux,le_sfc_flux,tsur,dx,col_sat 
  ...
  ! edt     = epsilon
     ...
     real, dimension (its:ite)         :: aa0_bl,aa1_bl,tau_bl,tau_ecmwf,wmean,aa1_fa,aa1_tmp,hkbo_x &
                                         ,aa2,aa3,cin0,cin1,edtmax,edtmin,aa1_lift
...
!
     ...
     real,    dimension (its:ite) :: p_cwv_ave,cape
...
!     real, dimension (mtp,its:ite,kts:kte),intent (in)   ::   se_chem
    ...
    !-- debug/diag
     ...
     real :: s1,s2,q1,q2,rzenv,factor,CWV,entr_threshold
     integer :: status
...
!
      if(cumulus == 'deep'   ) then; cap_maxs=150. ; cap_max_inc=20. ; endif
      if(cumulus == 'mid'    ) then; cap_maxs=150. ; cap_max_inc=10. ; endif
      ...
      do i=its,itf
        ...
        xland1 (i) = xland(i) ! 1.
        cap_max(i) = cap_maxs
        lambau_dp (i) = lambau_deep
        lambau_dn (i) = lambau_shdn
        ierrc  (i) = "ierrtxt"
        aa0    (i) = 0.0
        aa1    (i) = 0.0
        aa2    (i) = 0.0
...
!
      z_detr=1000. !P9_P6
      if(cumulus == 'deep'                         ) z_detr= 1000. !500. DNDP
...
!
      call cup_env_clev(t,qes,q,he,hes,z,po,qes_cup,q_cup,he_cup, &
           ...
           ierr,z1,itf,ktf,its,ite, kts,kte, clev_grid)
...
!
       DO i=its,itf
        ...
        if(ierr(i) == 0)then
           ...
           call get_cloud_bc(kts,kte,po(i,kts:kte),q_cup (i,kts:kte),rlll,k22(i),x_add)
           x_add = float(use_excess)*ztexec(i)
           call get_cloud_bc(kts,kte,po(i,kts:kte),t_cup (i,kts:kte),tlll,k22(i),x_add)
           call get_cloud_bc(kts,kte,po(i,kts:kte),p_cup (i,kts:kte),plll,k22(i))
           ...
           if(dzlcl >= 0.) then ! LCL found (if dzlcl<0 => not found)
             call get_cloud_bc(kts,kte,po(i,kts:kte),z_cup (i,kts:kte),zlll,k22(i))
loop0:       do k=kts,ktf
...
!
      do i=its,itf
       if(ierr(I) /= 0)cycle
       x_add = float(use_excess)*(xlv*zqexec(i)+cp*ztexec(i)) +  x_add_buoy(i)
       call get_cloud_bc(kts,kte,po(i,kts:kte),he_cup (i,kts:kte),hkb (i),k22(i),x_add)
       call get_cloud_bc(kts,kte,po(i,kts:kte),heo_cup(i,kts:kte),hkbo(i),k22(i),x_add)
      enddo
!
...
!
        CALL cup_cloud_limits(cumulus,ierrc,ierr,cap_max_increment,cap_max,entr_rate  &
                             ...
                             ,use_excess,zqexec,ztexec,x_add_buoy,itf,ktf,its,ite, kts,kte)
...
!
      CALL get_lateral_massflux(itf,ktf, its,ite, kts,kte                            &
                               ...
                               ,cumulus,kbcon,k22,up_massentru,up_massdetru,lambau_dp)
!--- start_level
      ...
      do i=its,itf
       IF(ierr(i).eq.0)THEN
         do k=kts,start_level(i)
            ...
            call get_cloud_bc(kts,kte,po(i,kts:kte),u_cup(i,kts:kte),uc(i,k),k22(i))
            call get_cloud_bc(kts,kte,po(i,kts:kte),v_cup(i,kts:kte),vc(i,k),k22(i))
         enddo
       ENDIF
      enddo
!
...
!
     call get_bouyancy(itf,ktf, its,ite, kts,kte,ierr,klcl,kbcon,ktop,hco,heo_cup,heso_cup,dbyo,zo_cup)
...
IF(CLOUDMP==1) then
     call cup_up_moisture(cumulus,klcl,ierr,zo_cup,qco,qrco,pwo,pwavo,hco,tempco,xland,  &
           ...
           ZQEXEC,use_excess,ccn,rho,up_massentr,up_massdetr,psum,psumh,c1d,x_add_buoy,vvel2d,&
           1,itf,ktf,ipr,jpr,its,ite, kts,kte)

ELSEIF(CLOUDMP==2) then

      call cup_up_moisture2(cumulus,klcl,ierr,zo_cup,qco,qrco,pwo,pwavo,hco,tempco,xland, &
           po,p_cup,kbcon,ktop,cd,dbyo,clw_all, &
           t_cup,qo,GAMMAo_cup,zuo,qeso_cup,k22,qo_cup,        &
           ZQEXEC,use_excess,ccn,rho,up_massentr,up_massdetr,psum,psumh,c1d,x_add_buoy, &
           vvel2d,vvel1d,zws,entr_rate_2d,p_liq_ice,&
           1,itf,ktf,ipr,jpr,its,ite, kts,kte)

ELSEIF(CLOUDMP==3) then
      !.. do i=its,itf
      !.. qco(i,:) = qo_cup(i,:)
      !.. tempco(i,:) = t_cup(i,:)
      !.. vvel2d(i,:) = 0.
      ...
      !.. x_add =float(use_excess)*zqexec(i)
      !.. call get_cloud_bc(kts,kte,po(i,kts:kte),qo_cup (i,kts:kte),qco(i,kts),k22(i),x_add)
      !.. do k=kts,start_level(i)
...
!--- Get bouyancy of updrafts
!
     call get_bouyancy(itf,ktf, its,ite, kts,kte,ierr,klcl,kbcon,ktop,hc, he_cup, hes_cup,  dby, z_cup)
     call get_bouyancy(itf,ktf, its,ite, kts,kte,ierr,klcl,kbcon,ktop,hco,heo_cup,heso_cup,dbyo,zo_cup)
...
!
       call cup_up_rain(cumulus,klcl,kbcon,ktop,k22,ierr,xland         &
                       ...
                       ,zuo,up_massentr,up_massdetr,vvel2d             & 
                       ,qrr                                            & 
...
!
       ...
       do i=its,itf
            ...
            !--for future test
            !--option 1
            !hcdo(i,jmin(i))=cp*t_wetbulb(i)+xlv*q_wetbulb(i)+zo_cup(i,jmin(i))*g ; i_wb=1
            !--option 2
            !hcdo(i,jmin(i))=0.5*(cp*t_wetbulb(i)+xlv*q_wetbulb(i)+zo_cup(i,jmin(i))*g + hc(i,jmin(i))); i_wb=1
...
!
      call cup_dd_moisture(cumulus,ierrc,zdo,hcdo,heso_cup,qcdo,qeso_cup, &
           ...
           pwevo,bu,qrcdo,qo,heo,t_cup,1,t_wetbulb,q_wetbulb,qco, &
	   itf,ktf,its,ite, kts,kte)
!
!
      ...
      IF(SGS_W_TIMESCALE == 0) THEN 
       ...
       ELSE
         ...
         DO i=its,itf
            ...
            wmean(i) = 2.+ min(max(vvel1d(i),1.),15.)            
            !- time-scale cape removal from Bechtold et al. 2008
            tau_ecmwf(i)=( zo_cup(i,ktop(i))- zo_cup(i,kbcon(i)) ) / wmean(i)
      
            tau_ecmwf(i)= tau_ecmwf(i) * (1. + 1.66 * (dx(i)/(125*1000.)))! dx must be in meters

            if(cumulus=='deep') tau_ecmwf(i)= min(max(1200.,tau_ecmwf(i)),tau_deep)
            if(cumulus=='mid' ) tau_ecmwf(i)= min(max(1200.,tau_ecmwf(i)),tau_mid )
         ENDDO
      ENDIF
...
PHY2:  DO step=fa,fa
        ...
        do i=its,itf
             ...
             call get_cloud_bc(kts,kte,po(i,kts:kte),heo_cup_x(i,kts:kte),hkbo_x(i),k22(i),x_add)
        enddo
        do i=its,itf
             hco_x (i,:)=0.
             IF(ierr(i) /= 0)cycle
      ...
      do i=its,itf
         ...
         do k=kts,ktop(i)
            ...
            if(abs(totmas).gt.1.e-6)then
               ...
               write(0,123)k,subin,subdown,detup,entup, detdo,entdo,entupk,detupk
      ...
      do i=its,itf
        ...
        if(ierr(i).eq.0)then
         do k=kts,ktop(i)
            ...
            detup=up_massdetro(i,k)
            if(.not. USE_C1D  .or. cumulus == 'mid' .or. cumulus == 'shallow') then
...
!        XT(I,K)=((1./cp)*DELLAH(i,k)-(xlv/cp)*(DELLAQ(i,k)+DELLAQC(i,k)))*MBDT(i)+TN(I,K)
      ...
      do i=its,itf
         ...
         !-ztexec and zqexec
         call get_cloud_bc(kts,kte,po(i,kts:kte),DELLAH (i,kts:kte),DELLAH_aver,k22(i))
...
!                                   ------ ice content --------
        ...
        enddo
      enddo
      call get_bouyancy(itf,ktf, its,ite, kts,kte,ierr,klcl,kbcon,ktop,xhc,xhe_cup,xhes_cup,xdby,xz_cup)
!
...
!
      ...
      call cup_forcing_ens_3d_shal(itf,ktf,its,ite,kts,kte,dtime,ichoice     &
				  ,ierrc,ierr,kpbl,kbcon,k22,ktop 	     &
...
!- begin: for GATE soundings-------------------------------------------
   if(use_gate .or. wrtgrads) then
    ...
    if(cumulus == 'shallow') then ; cty='2'; nvarbegin = 51 ;endif
    if(cumulus == 'mid'    ) then ; cty='3'; nvarbegin =101 ;endif
...
!------------------------------------------------------------------------------------
   ...
   SUBROUTINE cup_dd_moisture(cumulus,ierrc,zd,hcd,hes_cup,qcd,qes_cup,    &
              ...
              gamma_cup,pwev,bu,qrcd, q,he,t_cup,iloop,t_wetbulb,q_wetbulb,qco, &
              itf,ktf,its,ite, kts,kte                              )
...
!       IF(Q(I,K).GT.QES(I,K))Q(I,K)=QES(I,K)
	   ...
	   qes(i,k) = min(0.5, max(1.E-08,qes(i,k)))
...
!------------------------------------------------------------------------------------
   SUBROUTINE cup_env_clev(t,qes,q,he,hes,z,p,qes_cup,q_cup,he_cup, &
             ...
             ,clev_grid )
     implicit none
     integer ,intent (in   )              :: itf,ktf, its,ite, kts,kte,clev_grid
  !
  ! ierr error value, maybe modified in this routine
  ! q           = environmental mixing ratio
...
!
	 ...
	 do i=its,itf
            ...
            u_cup  (i,k) = c1*us (i,k) - c2*u_cup(i,k+1)                                        
            v_cup  (i,k) = c1*vs (i,k) - c2*v_cup(i,k+1)                                        
...
!------------------------------------------------------------------------------------
   SUBROUTINE cup_up_moisture(name,klcl,ierr,z_cup,qc,qrc,pw,pwav,hc,tempc,xland,   &
              ...
              up_massentr,up_massdetr,psum,psumh,c1d,x_add_buoy,vvel2d, &
...
!
  ...
  !
     real,    dimension (its:ite,kts:kte)                              &
        ,intent (in   )                   ::                           &
        t_cup,p_cup,rho,q,zu,gamma_cup,qe_cup,hc,po,                   &
        up_massentr,up_massdetr,dby,qes_cup,z_cup,cd,c1d,vvel2d
     real,    dimension (its:ite)                                      &
        ,intent (in   )                   ::                           &
        zqexec,xland,x_add_buoy
     integer, dimension (its:ite)                                      &
        ,intent (in   )                   ::                           &
        kbcon,ktop,k22,klcl
!
...
!--- no precip for small clouds
        ...
        do i=its,itf
          if(ierr(i).eq.0)then
            call get_cloud_bc(kts,kte,po(i,kts:kte),qe_cup (i,kts:kte),qaver,k22(i))

!           qaver = qaver + float(use_excess)*zqexec(i)
            qaver = qaver + float(use_excess)*zqexec(i) + 0.5*x_add_buoy(i)/xlv
        ...
        DO 100 i=its,itf
          ...
          DO k=start_level(i)+1,ktop(i) + 1     !k22(i)+1,ktop(i)
            ...
            IF (autoconv == 1 ) then
                ...
                !--- new 
                !aux = 1.25*atan((p_cup(i,k)/1000.)**0.5)
                !aux = max(aux,0.5)
                !PW (i,k)= cx0*max(0.,QRC(I,K) -qrc_crit*aux)! units kg[rain]/kg[air]
                !--- new
            ...
            ELSEIF (autoconv == 5 ) then
                ...
                if(clw_all(i,k) <= min_liq) then
                  ...
                  cx0     = (c1d(i,k)+c0)*dz
                  qrc(i,k)= (clw_all(i,k)-min_liq)/(1.+cx0)
                  !--normalized pw
                  pw (i,k)= cx0*qrc(i,k)*zu(i,k)! units kg[rain]/kg[air]
                  qrc(i,k)= qrc(i,k) + min_liq
                endif
                !
                !print*,"5mass=",pw(i,k)/(1.e-12+zu(i,k))+qrc(i,k),clw_all(i,k)
            ...
            ELSEIF (autoconv == 3 ) then
                ...
                if(clw_all(i,k) <= min_liq) then
                   ...
                   !print*,"aux=",aux,t_cup(i,k)
                   cx0 = aux*(c1d(i,k)+c0)*dz
                   qrc(i,k)=(clw_all(i,k)-min_liq)*exp(-cx0)+min_liq
                   pw (i,k)= clw_all(i,k) - qrc(i,k)
                   pw (i,k)= pw(i,k)*zu(i,k)
                endif
               ...
               !min_liq  = (xland(i)*0.3+(1.-xland(i))*0.5)*1.e-3
                ...
                if(qrc(i,k) > min_liq ) then 
                 ...
                 !if(tempc(i,k) < T_BF) cbf=1.+0.5*sqrt (min(T_BF-tempc(i,k),T_BF-T_ICE))
                  cx0 = (c1d(i,k)+c0)*cbf 
                  qrc_crit_BF=min_liq/cbf
                
                  !cx0 = 1.4e-4*cbf/(0.75*vvel2d(i,k))
                  step = cx0*dz
                  DO n=int(rk),1,-1
...
!------------------------------------------------------------------------------------
  ...
  SUBROUTINE cup_forcing_ens_3d_shal(itf,ktf,its,ite,kts,kte,dtime,ichoice     &
      ...
      integer ,dimension (its:ite)          ,intent (in) :: kpbl,kbcon,k22,ktop
      real                                  ,intent (in) :: dtime
...
!------------------------------------------------------------------------------------
  ...
  SUBROUTINE get_lateral_massflux(itf,ktf, its,ite, kts,kte                             &
     ...
     real :: dz,zuo_star, mass1,mass2,dp,rho
     logical, parameter :: SMOOTH = .FALSE.
     integer, parameter :: mass_U_option = 1  
    !real   , parameter :: fx = 0.5 ! new
     real   , parameter :: fx = 0.0 ! original
     !---
     if(draft == 'deep' ) then
      incr1=1
      incr2=1
     ...
     do i=its,itf
        ...
        !- mass entrainment and detrainment are defined on model levels
         ...
         do k=kts+1,maxloc(zuo(i,:),1)
           ...
           zuo_star= zuo(i,k-1)*(1.-fx) + zuo(i,k)*fx 
           up_massdetro(i,k-1)=cd(i,k-1)*dz*zuo_star 
...
!if(k<3)print*,"KX=",k-1, up_massdetro(i,k-1),zuo(i,k),zuo(i,k-1),up_massentro(i,k-1),zuo(i,k)-zuo(i,k-1)-up_massentro(i,k-1)+up_massdetro(i,k-1)
           if(zuo_star.gt.0.) then
                cd(i,k-1)       =up_massdetro(i,k-1)/(dz*zuo_star)
             entr_rate_2d(i,k-1)=up_massentro(i,k-1)/(dz*zuo_star)
           endif
        ...
        !---smoothing the transition zone (from maxloc(zu)-1 to maxloc(zu)+1)
          do k=maxloc(zuo(i,:),1)-1,maxloc(zuo(i,:),1)+1
           dz=zo_cup(i,k)-zo_cup(i,k-1)
           zuo_star= zuo(i,k-1)*(1.-fx) + zuo(i,k)*fx 
           up_massentro(i,k-1)=0.5*(entr_rate_2d(i,k-1)*dz*zuo_star+up_massentro(i,k-2))
           ...
           if(zuo_star.gt.0.)cd(i,k-1)=up_massdetro(i,k-1)/(dz*zuo_star)
          enddo
          ...
          do k=maxloc(zuo(i,:),1),maxloc(zuo(i,:),1)+2
           ...
           up_massdetro(i,k-1)=0.5*(cd(i,k-1)*dz*zuo_star+up_massdetro(i,k-2))
           up_massentro(i,k-1)=zuo(i,k)-zuo(i,k-1)+up_massdetro(i,k-1)
         ...
         !=> above location of maximum value zu -> change detrainment
           ...
           up_massentro(i,k-1)=entr_rate_2d(i,k-1)*dz*zuo_star
    ...
    DO i=its,itf
	    ...
	    rho= -dp/dz/g
            ...
            !print*,"masscons=",-rho*g*(zuo(i,k)-zuo(i,k-1))/dp, (zuo(i,k)-zuo(i,k-1))/dz,( up_massentro(i,k-1)-up_massdetro(i,k-1))/dz,rho
    ...
    !real, parameter :: fx = 0.5 ! new
     ...
     do i=its,itf
        ...
        do ki=jmin(i)   ,maxloc(zdo(i,:),1),-1
          ...
          dzo=zo_cup(i,ki+1)-zo_cup(i,ki)
          dd_massdetro(i,ki)=cdd(i,ki)*dzo* (zdo(i,ki+1)*(1.-fx) +zdo(i,ki)*fx)
!XXX
        ...
        do ki=maxloc(zdo(i,:),1)-1,kts,-1
          ...
          dzo=zo_cup(i,ki+1)-zo_cup(i,ki)
          dd_massentro(i,ki)=mentrd_rate_2d(i,ki)*dzo*(zdo(i,ki+1)*(1.-fx) +zdo(i,ki)*fx)
!XXX
...
!------------------------------------------------------------------------------------
 ...
 SUBROUTINE rates_up_pdf(name,ktop,ierr,p_cup,entr_rate_2d,hkbo,heo,heso_cup,z_cup, &
     ...
     dbythresh=1.   ! 0.95  
                    ! the range of this parameter is 0-1, higher => lower
...
!------------------------------------------------------------------------------------
  ...
  INTEGER :: k1
  real :: lev_start,g_alpha2,g_a,y1,x1,g_b,a,b,alpha2,csum,zubeg,wgty,hei_down
...
! if(draft == "deep_up" .and. xland <= 0.90) itest=20 !land
 ...
 !---------------------------------------------------------
  ...
  ELSEIF(itest==20 .and. draft == "deep_up") then       !--- land/ocean
      ...
      !-ref  pmaxzu= 714.1000     beta=  1.739130    

      tunning = 0.5 !max(0.1, min(1.,float(JL)/160.))
      
      !--tunning parameter goes from 0 to 1 = rainfall increases with tunning 
      tunning =  min(1.,max(tunning,0.1))  
      pmaxzu=psur*(tunning)+(1.-tunning)*0.5*(psur+po_cup(kt))
  ...
  !---------------------------------------------------------
  !---------------------------------------------------------
  ELSEIF(itest==12 .and. draft == "deep_up") then       !--- land
      !- kb cannot be at 1st level
      !kb_adj=max(kb,2)
      pmaxzu=psur-px*(psur-po_cup(kt))
      kb_adj=minloc(abs(po_cup(kts:kt)-pmaxzu),1)
     ...
     IF(do_smooth) then
      ...
      zu(kts)=0.
  !---------------------------------------------------------
  ELSEIF(itest==11 .and. draft == "deep_up") then     !--- ocean
 ...
 ELSEIF(draft == "shallow_up") then
  ...
  ELSEIF(draft == "DOWN" ) then
     ...
     !IF(cumulus == 'mid' ) beta =2.5 ! P9_P6
      IF(cumulus == 'deep') beta =2.5
     !IF(cumulus == 'deep') beta =6.5 ! DNDP
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@                      target 
!      if(xland  < 0.90 ) then !- over land
!        hei_down=0.35  ! tested
!       !kb_adj=kb+2
!       !kb_adj=min(kb_adj,kt)
!
...
!------------------------------------------------------------------------------------
  ...
  SUBROUTINE cup_kbcon(NAME,ierrc,cap_inc,iloop,k22,kbcon,he_cup,hes_cup, &
              hkb,ierr,kbmax,po,p_cup,cap_max,                               &
...
!------------------------------------------------------------------------------------
 SUBROUTINE get_cloud_bc(kts,mzp,po,array,x_aver,k22,add,maxi)
    implicit none
    integer, intent(in)     :: kts,mzp,k22
    real   , intent(in)     :: array(kts:mzp),po(kts:mzp)
    ...
    real,    parameter :: max_dp_layer= 52. !mbar  for deep/congestus
   !real,    parameter :: max_dp_layer= 32. !mbar  for shallow
    real :: count,dp,dp_layer
    ...
    !order_aver = 4    !=> meth 1: average between k22, k22-1, k22-2 ...
                       !=> meth 2: average between ... k22+1,k22, k22-1 ...
    ...
    if(meth == 1) then 
      ...
      ! from         k22-3           =>            k22+3
      do i = max(kts+1,k22-(order_aver-1)),k22+(order_aver-1)
        dp= -(po(i+1)-po(i))
       !if(dp_layer+dp > max_dp_layer) exit ! use "order_aver = 6" ,see above  
...
!------------------------------------------------------------------------------------
  ...
  SUBROUTINE cup_cloud_limits(name,ierrc,ierr,cap_inc,cap_max_in,entr_rate                 &
                             ,heo_cup,heso_cup,qo_cup,qeso_cup,po,po_cup,z_cup,heo,hkbo &
                             ,entr_rate_2d,hcot,k22,kbmax,klcl,kbcon,ktop               &
                             ,use_excess,zqexec,ztexec,  x_add_buoy                     &
                             ,itf,ktf,its,ite, kts,kte)
     ...
     real,    dimension (its:ite), intent (in) :: entr_rate

     real,    dimension (its:ite,kts:kte)  ,intent (in   )     ::    &
        heo_cup,heso_cup,po_cup,z_cup,heo,qo_cup,qeso_cup,po
     real,    dimension (its:ite)          ,intent (in   )     ::    &
        cap_max_in,cap_inc
     real,    dimension (its:ite)          ,intent (in   )     ::    &
...
!------------------------------------------------------------------------------------
  SUBROUTINE get_incloud_sc_chem_up(cumulus,fscav,mtp,se,se_cup,sc_up,pw_up,tot_pw_up_chem&
     ...
     integer                               ,intent (in)  :: itf,ktf, its,ite, kts,kte
     integer                               ,intent (in)  :: mtp
     integer, dimension (its:ite)          ,intent (in)  :: ierr ,kbcon,ktop,k22,klcl
     character *(*)                        ,intent (in)  :: cumulus
     real, dimension (mtp)                 ,intent (in)  :: FSCAV
     ...
     DO i=its,itf
      ...
      !start_level(i) = klcl(i)
       ...
       do ispc=1,mtp
             call get_cloud_bc(kts,kte,po(i,kts:kte),se_cup(ispc,i,kts:kte),sc_b(ispc,i),k22(i))
       enddo
       do k=kts,start_level(i)
            sc_up   (:,i,k) = sc_b  (:,i)
...
!---------------------------------------------------------------------------------------------------
   ...
   SUBROUTINE cup_up_rain(cumulus,klcl,kbcon,ktop,k22,ierr,xland        &
     ...
     real,    dimension(its:ite,kts:kte),intent (in)  ::   &
             ...
             ,up_massentr,up_massdetr,vvel2d,tempco
    ...
    !-- locals
     ...
     integer, dimension(its:ite) :: start_level
     real :: dz,z1,zrnew,zc,zd,zint,z2,zrold,denom,fall_fact,wup
     ...
     do i=its,itf
	  ...
	  !--- fallout of rain
	   fall_fact = 21.28*( p_liq_ice(k) + 0.5*(1.-p_liq_ice(k) ))
	   wup       = min(15.,max(2.,vvel2d(i,k)))
!	   wup       = 10.
	   z2	     = fall_fact/wup ! - rainfall velocity = fall_fact* qrr(i,k)**0.2

	   !--exact solution
	   if(qrr(i,k) + pw(k)>0.) then 
	    zd= z2* (qrr(i,k) + pw(k))**0.2 != V/W_up, V1=21.28 * qrr**0.2 , V=V1 (T>0) or 0.5V1 (T<0.)
	    zint= EXP(-zd)
	    zrold=qrr(i,k)
	    zc=pw(k)
 	  ...
 	  !--solution with rk3
	   !z1= qrr(i,k)
	   !z1= qrr(i,k) +  1./3.*(pw(k) - z2*(z1)**0.2 * z1)
	   !z1= qrr(i,k) +  0.500*(pw(k) - z2*(z1)**0.2 * z1)
	   !z1= qrr(i,k) +  1.000*(pw(k) - z2*(z1)**0.2 * z1)
	   !qrr(i,k)=z1    
	   !---
	   !--- solution 2
	   !qrr(i,k)= qrr(i,k) + pw(k) - z2*(qrr(i,k) + pw(k))**0.2 * qrr(i,k)
	   
	   !print*,"rr3=",k,qrr(i,k),21.18*qrr(i,k)**0.2,wup
	  ENDDO
     ENDDO
...
!---------------------------------------------------------------------------------------------------
!---------------------------------------------------------------------------------------------------
   SUBROUTINE cup_up_moisture2(name,klcl,ierr,z_cup,qc,qrc,pw,pwav,hc,tempc,xland,   &
              po,p_cup,kbcon,ktop,cd,dby,clw_all,              &
              t_cup,q,GAMMA_cup,zu,qes_cup,k22,qe_cup,         &
              ZQEXEC,use_excess,ccn,rho,                       &
...
!
  ...
  !
     real,    dimension (its:ite,kts:kte)                              &
        ,intent (in   )                   ::                           &
        t_cup,p_cup,rho,q,zu,gamma_cup,qe_cup,hc,po,                   &
        up_massentr,up_massdetr,dby,qes_cup,z_cup,cd,c1d,p_liq_ice
     real,    dimension (its:ite)                                      &
        ,intent (in   )                   ::                           &
        zqexec,xland,x_add_buoy
     integer, dimension (its:ite)                                      &
...
!
     ...
     integer, dimension (its:ite)  ::     start_level
     real delt
     real                          ::  BU,dw2,dw1,kx,dz1m,Tv,Tve,qvapor
...
!           qaver = qaver + float(use_excess)*zqexec(i)
        ...
        DO 100 i=its,itf
          ...
          DO k=start_level(i)+1,ktop(i) + 1     !k22(i)+1,ktop(i)
            ...
            !-- updraft temp
            tempc(i,k) = (1./cp)*(hc(i,k)-g*z_cup(i,k)-xlv* ( qc(i,k)-clw_all(i,k))) 

            !------- vertical velocity
            Tve=  t_cup(i,k)*(1.+( q(i,k)/eps)                 /(1.+ q (i,k)))

            Tv =  tempc(i,k)*(1.+( (qc(i,k)-clw_all(i,k)) /eps)/(1.+ (qc(i,k)-clw_all(i,k)))) 

            
            cx0     = (c1d(i,k)+c0)*DZ
                        
            BU = g* ( (Tv-Tve)/Tve - clw_all(i,k)/(1.+cx0))

            dw1 = 2./(f*(1.+gam)) * BU * dz

           !kx  = (1.+beta*C_d)*max(entr_rate_2d(i,k-1),cd(i,k-1))*dz
            kx  =               max(entr_rate_2d(i,k-1),cd(i,k-1))*dz
            dw2 =  (vvel2d(i,k-1)) -2.*kx * (vvel2d(i,k-1))
            
            vvel2d(i,k)=(dw1+dw2)/(1.+kx)

            
            if( vvel2d(i,k)< 0.) vvel2d(i,k) = 0.5 * vvel2d(i,k-1)          
            ...
            ELSEIF (autoconv == 4 ) then
                ...
                if(qrc(i,k) > min_liq ) then 
                  ...
                  !cx0 = (c1d(i,k)+c0)*cbf
                  !cx0 = (c1d(i,k)+c0)*cbf/(0.75*sqrt(2.+min(10.,vvel2d(i,k))))
                  !cx0 =  1.4e-3*cbf/(0.75*sqrt(2.+min(10.,vvel2d(i,k))))
                  cx0 =  1.4e-3*cbf/(0.75*sqrt(2.+min(15.,vvel2d(i,k))))
                  
                  qrc_crit_BF=min_liq/cbf
                  print*,"cx=",k,cx0,sqrt(vvel2d(i,k))!,c1d(i,k)
                  step = cx0*dz
                  DO n=int(rk),1,-1
                    aux     = qrc(i,k)/qrc_crit_BF
                    pw (i,k)= auto_rk(n,step,aux,xexp,qrc(i,k))
 ...
 100     CONTINUE
   ...
   END SUBROUTINE cup_up_moisture2
