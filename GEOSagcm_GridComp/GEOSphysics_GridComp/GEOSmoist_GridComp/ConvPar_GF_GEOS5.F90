...
!.. USE GTMP_2_GFCONVPAR, only : GTMP_2_GFCONVPAR_interface
 ...
 PUBLIC  gf_geos5_interface, maxiens, icumulus_gf, closure_choice, deep, shal, mid &
	...
	,use_smooth_prof, evap_fix,output_sound
...
!
      ...
      ELSE
       ...
       if(cumulus == 'shallow') then; cap_maxs=75.  ; cap_max_inc=10. ; endif
      ENDIF
!
...
!------------------------------------------------------------------------------------
 SUBROUTINE SOUND(part,cumulus,int_time,dtime,ens4,itf,ktf,its,ite, kts,kte,xlats,xlons,jcol,whoami_all  &
       ...
       real :: X_time
       character(200) :: lixo
       real,    dimension (its:ite) :: X_stochastic_sig
	...
	ELSE
	 ...
	 IF(part == 1) THEN
	    ...
	    read(15,*) lixo
	    read(15,*) X_kte,z1(i),psur(i),tsur(i),xland(i)
            !-- check
