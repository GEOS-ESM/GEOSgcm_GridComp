...
#include "MAPL_Generic.h"
   ...
   real, parameter :: NN_MIN_ICE  =  100.0e6
   real, parameter :: NN_MAX_ICE  =  500.0e6
...
CONTAINS
   ...
   SUBROUTINE Aer_Activation(MAPL, IM,JM,LM, q, t, plo, ple, tke, vvel, FRLAND, &
      ...
      allocate( sig0(IM,JM,n_modes), source=0.0, __STAT__)
      allocate(   rg(IM,JM,n_modes), source=0.0, __STAT__)
      allocate(   ni(IM,JM,n_modes), source=0.0, __STAT__)
   ...
   subroutine GetActFrac(im, nmodes,xnap,rg,sigmag,bibar,tkelvin,ptot,wupdraft,nact)
      ...
      do n=1, nmodes
         do i = 1, im
            if ( (wupdraft(i) > 1.e-2) .and. (bibar(i,n) > 0.4) .and. (gamma(i) > 1.e-12) .and. &
               ...
               smax(i) = smax(i) + (f1 * (zeta(i) / eta)**1.5 + &
                                    f2 * (sm(i,n)**2 / (eta + 3.0*zeta(i)))**0.75) / sm(i,n)**2
             end if
          end do
      end do
      ! Finalize smax
      ...
      do n=1, nmodes
         do i = 1, im
            if (smax(i) > 0.0 .and. sm(i,n) > 0.0) then
               ac = rg(i,n) * ( sm(i,n)/smax(i) )**r23                   ! [um]
               u = log(ac/rg(i,n)) / ( sqrt2 * xlogsigm(i,n) )        ! [1]
               fracactn(i,n) = 0.5 * (1.0 - erf(u))                        ! [1]
