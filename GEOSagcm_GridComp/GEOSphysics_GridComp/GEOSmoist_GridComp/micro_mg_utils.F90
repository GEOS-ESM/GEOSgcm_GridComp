...
elemental subroutine size_dist_param_liq_line(props, qcic, ncic, rho, pgam, lamc)
 ...
 !DONIF (2017) relative dispersion from Yangang Liu et al 2008 Environ. Res. Lett. 3 045021 doi:10.1088/1748-9326/3/4/045021
                  !***************************    
        if ((ncic .gt. 1.0) .and. (qcic .gt. 1.0e-11))  then 
            ...
            pgam=sqrt(xs)
            pgam=1._r8/(pgam**2)-1._r8
            pgam=min(max(pgam,2._r8), 15._r8)
      end if
...
subroutine size_dist_param_liq_vect(props, qcic, ncic, rho, pgam, lamc, mgncol)
 ...
 !DONIF (2017) relative dispersion from Yangang Liu et al 2008 Environ. Res. Lett. 3 045021 doi:10.1088/1748-9326/3/4/045021
                  !***************************    
        if ((ncic(i) .gt. 1.0) .and. (qcic(i) .gt. 1.0e-11))  then 
            ...
            pgam(i)=sqrt(xs)
            pgam(i)=1._r8/(pgam(i)**2)-1._r8
            pgam(i)=min(max(pgam(i),2._r8), 15._r8)
      end if
...
! Basic routine for getting ice distribution parameters.
...
!        call size_dist_param_basic(mg_ice_props, qiic, niic, lami, n0i)
        ...
        call size_dist_param_ice(mg_ice_props, qiic, niic, lami, n0i) !DONIF account fot mu_i > 0

        !Get depletion timescale=1/eps
       ! epsi = 2._r8*pi*n0i*rho(i)*Dv(i)/(lami*lami)

        mui  = mui_of_l(lami)
        epsi = (mui+2._r8)*(mui+1._r8)*pi*niic*rho(i)*Dv(i)/lami !DONIF
...
subroutine liu2006_liq_autoconversion(pgam, microp_uniform, qcic, &
  ...
  real(r8), parameter :: miu_disp = 2.0
 
 
  ! Take variance into account, or use uniform value.
  if (.not. microp_uniform) then
     prc_coef(:) = var_coef(relvar(:), 2.00_r8) !assume 2 for now
  else
     prc_coef(:) = 1._r8
  end if
  ...
  do i=1,mgncol
     if (qcic(i) >= icsmall) then
           ...
           xs=1._r8/(1._r8+ pgam(i)) !returs e^2
           beta6=(1._r8+3.0*xs)*(1._r8+4.0*xs)*(1._r8+5.0*xs) / (1._r8+ xs)/(1._r8+2.0*xs)
           LW= 1.0e-3_r8*qcic(i)*rho(i)
           NW = ncic(i)*rho(i)*1.e-6_r8
           
          xs =  1.03e16*(LW*LW)/(NW*SQRT(NW))
          xs=min(20.0, xs)    
          prc(i) =  1.1e10*beta6*LW*LW*LW*(1.0 - exp(-(xs**miu_disp))) /NW                         
          prc(i)=prc_coef(i)*prc(i)*1.0e3/rho(i) ! return to SI units and add enhacement factor
         ...
         nprc1(i) = ncic(i)*prc(i)/qcic(i)/(1_r8+xs)  
  ...
  enddo
end subroutine liu2006_liq_autoconversion
...
! Modified by Donifan Barahona to account for mu_i > 0
subroutine ice_autoconversion(t, qiic, lami, niic, dcs, prci, nprci, mgncol, ac_time)
  ...
  do i=1,mgncol
     if (t(i) <= tmelt .and. qiic(i) >= qsmall) then
      ...
      !  nprci(i) = n0i(i)/(lami(i)*ac_time)*exp(-d_rat)
           ...
           nprci(i) = (niic(i)/ac_time)*(1._r8- gammp(mu_ice, d_rat))         
           prci(i) = (qiic(i)/ac_time) *(1._r8 - gammp(mu_ice + 3._r8, d_rat))  
...
subroutine contact_freezing (microp_uniform, t, p, rndst, nacon, &
  ...
  real(r8) :: frz_eff(size(rndst,2)),  area(size(rndst,2))

  ! Coefficients not used for subcolumns
  real(r8) :: dum, dum1, taux, nssoot, nsdust
  ...
  do i = 1,mgncol
     ...
     if (qcic(i) >= qsmall .and. t(i) < 269.15_r8) then
        ...
        frz_eff(mdust) =  nssoot         
        area = 12.5664*rndst(i, :)*rndst (i, :)       
        frz_eff = dot_product(frz_eff, area)
      ...
      ! DONIF
        contact_factor = dot_product(ndfaer,nacon(i,:)*frz_eff) * pi * &
             ncic(i) * (pgam(i) + 1._r8) / lamc(i) ! 
      
      
!        contact_factor = dot_product(ndfaer,nacon(i,:)*tcnt) * pi * &
!             ncic(i) * (pgam(i) + 1._r8) / lamc(i) ! this expression is missing the ice nucleation efficiency
...
!cccccccccccccccccccccDONIFccccccccccccccccccccccccccccccccccccccccccccccccc
         ...
         !**********************************
pure   FUNCTION mui_of_l(lambda_)
