...
!!$       CURAINMOVE_DEV, CUSNOWMOVE_DEV, &
       ...
       ANV_SDQV3, ANV_SDQVT1, N_WARM, N_ICE, N_ANVIL, &
...
contains
  ...
  call ESMF_ConfigGetAttribute( CF, MGVERSION, Label="MGVERSION:",  default=0.0, RC=STATUS)
    ...
    if(adjustl(CLDMICRO)=="2MOMENT") then    
      ...
      if (MGVERSION .gt. 1.0) then          
        ...
        call ESMF_ConfigGetAttribute( CF, DOGRAUPEL, Label="DOGRAUPEL:",  default=0.0, RC=STATUS)
        if (DOGRAUPEL .gt. 0.0) FRIENDLIES_NGRAUPEL = 'DYNAMICS:TURBULENCE'               
        if (DOGRAUPEL .gt. 0.0) FRIENDLIES_QGRAUPEL = 'DYNAMICS:TURBULENCE'  
      end if 
    endif
  ...
  if (DOSHLW /= 0) then
    ...
    call MAPL_AddImportSpec(GC,                             &
         SHORT_NAME = 'FRACI',                                    &
         LONG_NAME  = 'areal_seaice_fraction',                       &
         UNITS      = '1',                                     &
         DIMS       = MAPL_DimsHorzOnly,                           &
         VLOCATION  = MAPL_VLocationNone,                        &
         AVERAGING_INTERVAL = AVRGNINT,                            &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine Initialize ( GC, IMPORT, EXPORT, CLOCK, RC )
    ...
    real DCS, QCVAR_, WBFFACTOR, MGVERSION, NC_CST, NI_CST, NG_CST, DOGRAUPEL
    ...
    if(adjustl(CLDMICRO)=="2MOMENT") then
       ...
       call MAPL_GetResource(MAPL, MGVERSION, 'MGVERSION:',   DEFAULT= 0.0 , RC=STATUS )
      ...
      call MAPL_GetResource(MAPL, WBFFACTOR,   'WBFFACTOR:', DEFAULT= 1.0 ,RC=STATUS) !variance of the QL distribution     
       ...
       call MAPL_GetResource(MAPL, DOGRAUPEL,  'DOGRAUPEL:', DEFAULT= 1.0 ,RC=STATUS) !graupel is prognostic in mg3  if greather than zero)       
       ...
       if (DOGRAUPEL .gt. 0.0) do_graupel = .true.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine MOIST_DRIVER(IM,JM,LM, RC)
      ...
      real, pointer, dimension(:,:  ) :: PTYPE,FRZR,ICE,SNR,PRECU,PRELS,TS,SNOMAS,FRLANDICE,FRLAND,FROCEAN,FRACI
...
!!$      real, dimension(IM,JM,LM) :: CUSNOWMOVE_X 
      ...
      ! Combined LAND+SEA ICE
      real   , dimension(IM,JM) :: FRLNDSEAICE
...
!
      ...
      call MAPL_GetResource(STATE, MGVERSION, 'MGVERSION:',   DEFAULT= 0.0 , RC=STATUS )
      ...
      call MAPL_GetResource(STATE, TS_AUTO_ICE,    'TS_AUTO_ICE:',    DEFAULT= 1.0, RC=STATUS) !Ice autoconversion time scale
      ...
      call MAPL_GetResource(STATE, DT_MICRO,          'DT_MICRO:',        DEFAULT= 300.0,   RC=STATUS)    ! time step of the microphysics substepping (s) (MG2) (5 min)
      ...
      call MAPL_GetResource(STATE, CLDPARAMS%DISP_FACTOR_LIQ,         'DISP_FACTOR_LIQ:',     DEFAULT= 50.0,   RC=STATUS) ! Scales the droplet/ice crystal number in convective detrainment 
...
!-srf - placed here before the cumulus parameterizations are called.
      ...
      ! Combine LAND+SEA ice
      DO J=1, JM
        DO I=1, IM
          FRLNDSEAICE(I,J)=MIN(1.0,MAX(FRLANDICE(I,J),FRACI(I,J)))
        END DO
      END DO
...
!-srf-gf-scheme
          ...
          do J=1,JM
           do I=1,IM
            IFRC = ICE_FRACTION( TEMP(I,J,K), CNV_FRACTION(I,J), &
                                 SNOMAS(I,J), FRLNDSEAICE(I,J), FRLAND(I,J) )
     ...
     ! Meltfrz
        CALL MELTFRZ_ALL ( IM, JM, LM, DT_MOIST, TEMP, Q1, &
                           QLLS, QLCN, QILS, QICN, CNV_FRACTION, &
                           SNOMAS, FRLNDSEAICE, FRLAND)
...
!      if(associated(TVQX1))  TVQX1     = SUM( (  Q1 +  QLLS + QLCN + QILS + QICN + CNV_PRC3)*MASS + CNV_DQLDT*DT_MOIST , 3 )
      ...
      call MAPL_GetResource( STATE, CLDPARAMS%ANV_TO_LS_CU, 'ANV_TO_LS_CU:', DEFAULT= 10800.  )
      call MAPL_GetResource( STATE, CLDPARAMS%ANV_TO_LS_ST, 'ANV_TO_LS_ST:', DEFAULT= 300.  )
     ...
     !   DZ = ( ZLE(:,:,1:LM)-ZLE(:,:,0:LM-1) )
        ...
        ! Fill CNV_FICE,CNV_NICE,CNV_NDROP assume still 1-moment
         do K=1,LM
          do J=1,JM
           do I=1,IM
             CNV_FICE(I,J,K) = ICE_FRACTION( TEMP(I,J,K), CNV_FRACTION(I,J), SNOMAS(I,J), FRLNDSEAICE(I,J), FRLAND(I,J) )
           enddo
          enddo
         enddo
         IF(ADJUSTL(CONVPAR_OPTION) == 'GF') THEN
  ...
  call  macro_cloud (                    &
      ...
      !  endif ! DOCLDMACRO
       ...
       if(associated(TVQX1)) TVQX1    = SUM( ( Q1 +  QLLS + QLCN + QILS + QICN )*MASS , 3 )  + TPREC*DT_MOIST
         ...
         !================This is the implementation of the Morrison and Gettelman (2008, 2015) microphysics =====================
         !==================================================================================================================
        ...
        scale_ri =  1.0
         ...
         QCVAR_EXP = 2.0
           ...
           CFX =  CLLS +CLCN
         where ((CLCN .gt. 0.0) .and. (CLLS .gt. 0.0) .and. (RAD_CF .gt. 0.0))
            CFX  =  ((((1.0-FQA)**2.0)/CLLS)  + ((FQA**2.0)/CLCN))*(CLCN+CLLS)/RAD_CF        
          end where  
            CFX =  min(max(CFX, 0.0), 1.0)     
         ...
         do J=1,JM
            ...
            ! use effective concept from Gerard et al., MWR, 2009. 
             cldfr8(1,1:LM)      = CFX(I,J,1:LM) !Assume minimum overlap 
...
!                             errstring, & ! Below arguments are "optional" (pass null pointers to omit).
        ...
        IF (MGVERSION > 1.0) then 
              ...
              if (DOGRAUPEL .gt. 0.0) then                   
               ...
               QRN(I,J,1:LM)    = max(REAL(qrout2r8(1, 1:LM)), 0.0) ! grid average 
               QSN(I,J,1:LM)    = max(REAL(qsout2r8(1, 1:LM)), 0.0)   
    ...
    !update water tracers
         ...
         QICN=QI_TOT*FQA
         QILS=QI_TOT-QICN
         PFI_AN_X =  PFI_LS_X*FQA 
         PFL_AN_X =  PFL_LS_X*FQA 
         PFI_LS_X = PFI_LS_X - PFI_AN_X
         PFL_LS_X = PFL_LS_X - PFL_AN_X
         QTOT= QICN+QILS+QLCN+QLLS
               ...
               if (FRLAND(I, J) .lt. 0.1) then 
                  ...
                  DO K=LM-1, 2, -1
                     if ((RAD_CF(I, J, K) .gt. 0.01) .and. (RAD_CF(I, J, K) .lt. 0.99)) then  
                           ...
                           USURF =  usurf+ (1.0-USURF)*CNV_FRACTION(I, J) !only non-comvective
...
!      if(associated(TVQX2)) TVQX2    = SUM( ( Q1 +  QLLS + QLCN + QILS + QICN )*MASS , 3 )  + TPREC*DT_MOIST
      ...
      if(adjustl(CLDMICRO) .ne. "2MOMENT") then 
...
!Find cloud base  for a given cloud fraction 
  ...
  subroutine VertInterp(v2,v3,ple,pp,rc)
   ...
   subroutine MELTFRZ_ALL  ( IM, JM, LM, Dt, TE, QV, QLLS, QLCN, QILS, QICN, &
                             CNV_FRACTION, SNOMAS, FRLNDSEAICE, FRLAND)

      integer,                   intent(in)    :: IM, JM, LM
      real, dimension(IM,JM,LM), intent(inout) :: TE,QV,QLLS,QLCN,QILS,QICN
      real,                      intent(in)    :: Dt
      real, dimension(IM,JM),    intent(in)    :: CNV_FRACTION, SNOMAS, FRLNDSEAICE, FRLAND
      real                   :: fQi,dQil,DQmax, QLTOT, QITOT, FQAi, FQAl
      integer                :: i, j, k, n
    ...
    ! store condensate fractions for later
      FQAi = 0.0
      QITOT = QILS(i,j,k)+QICN(i,j,k)
      if (QITOT .gt. 0.0) then
         FQAi = QICN(i,j,k)/QITOT
      end if
      FQAl = 0.0
      QLTOT = QLLS(i,j,k)+QLCN(i,j,k)
      if (QLTOT .gt. 0.0) then
         FQAl = QLCN(i,j,k)/QLTOT
      end if

      convergence: do n=1,MaxIterations

      ! melt ice using ICE_FRACTION
      fQi = ice_fraction( TE(i,j,k), CNV_FRACTION(i,j), SNOMAS(i,j), FRLNDSEAICE(i,j), FRLAND(i,j) )
      if ( fQi < 1.0 ) then
    ...
    ! Parse into new LS and CN partitions based on previous fractions (FQA)
      QILS(i,j,k) = QITOT*(1.0-FQAi)
      QICN(i,j,k) = QITOT*FQAi
      QLLS(i,j,k) = QLTOT*(1.0-FQAl)
      QLCN(i,j,k) = QLTOT*FQAl
