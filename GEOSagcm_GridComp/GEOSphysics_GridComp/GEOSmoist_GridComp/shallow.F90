module shallow
   ...
   real*8, parameter :: xlv   = MAPL_ALHL             ! Latent heat of vaporization
   real*8, parameter :: xlf   = MAPL_ALHF             ! Latent heat of fusion
   real*8, parameter :: xls   = MAPL_ALHS             ! Latent heat sublimation
   real*8, parameter :: cp    = MAPL_CP               ! Specific heat of dry air
   real*8, parameter :: zvir  = 0.609_r8              ! r_H2O/r_air-1
   real*8, parameter :: r     = MAPL_RGAS             ! Gas constant for dry air
   real*8, parameter :: g     = MAPL_GRAV             ! Acceleration due to gravity
   real*8, parameter :: ep2   = MAPL_H2OMW/MAPL_AIRMW ! Ratio of molc wgt H2O to dry air
   real*8, parameter :: p00   = 1e5_r8                ! Reference pressure
   real*8, parameter :: rovcp = MAPL_RGAS/MAPL_CP     ! Gas constant over specific heat

contains

   real*8 function exnerfn(pressure)
      real*8, intent(in)              :: pressure   ! in Pa
      exnerfn = (pressure/p00)**rovcp
      return
   end function exnerfn


   subroutine compute_uwshcu_inv(idim, k0, ncnst, dt,pifc0_inv, zifc0_inv,  &
         exnifc0_inv, pmid0_inv, zmid0_inv, exnmid0_inv, dp0_inv,    &
         u0_inv, v0_inv, qv0_inv, ql0_inv, &
         qi0_inv, th0_inv, kpbl_inv,   &
         tke_inv, cush, umf_inv,   &
         qvten_inv, qlten_inv, qiten_inv, thten_inv,   &
         uten_inv, vten_inv, qrten_inv, qsten_inv,     &
         cufrc_inv, qcu_inv, qlu_inv, qiu_inv, cbmf,      &
         qc_inv, cnt_inv, cnb_inv, cin, plcl, plfc, pinv,prel,pbup, &
         wlcl, qtsrc, thlsrc, thvlsrc, tkeavg, cldtop, &
         wu_inv, qtu_inv, thlu_inv, thvu_inv, uu_inv, vu_inv,  &
         fer_inv, fdr_inv, xc_inv, qldet_inv, qidet_inv, qlsub_inv, &
         qisub_inv, ndrop_inv, nice_inv, thlsrc_pert, tr0_inv,shlwparams)
...
!      real*8, intent(in)    :: dpdry0_inv(idim,k0)      !  Environmental dry layer pressure thickness [ Pa ]
...
!      real, intent(out)   :: trten_inv(idim,k0,ncnst) !  Tendency of tracers [ #/s, kg/kg/s ]
  ...
  !----- Internal variables -----
      real*8              :: pifc0(idim,0:k0)           !  Environmental pressure at the interfaces [ Pa ]
      real*8              :: zifc0(idim,0:k0)           !  Environmental height at the interfaces   [ m ]
      real*8              :: exnifc0(idim,0:k0)
      real*8              :: pmid0(idim,k0)              !  Environmental pressure at the layer mid-point [ Pa ]
      real*8              :: zmid0(idim,k0)              !  Environmental height at the layer mid-point [ m ]
      real*8              :: exnmid0(idim,k0)
      real*8              :: dp0(idim,k0)             !  Environmental layer pressure thickness [ Pa ] > 0.
!      real*8                :: dpdry0(idim,k0)          !  Environmental dry layer pressure thickness [ Pa ]
      real*8              :: u0(idim,k0)              !  Environmental zonal wind [ m/s ]
      real*8                :: v0(idim,k0)              !  Environmental meridional wind [ m/s ]
                                                        ! at the previous time step [ fraction ]
      real*8              :: qv0(idim,k0)             !  Environmental water vapor specific humidity [ kg/kg ]
      real*8              :: ql0(idim,k0)             !  Environmental liquid water specific humidity [ kg/kg ]
      real*8              :: qi0(idim,k0)             !  Environmental ice specific humidity [ kg/kg ]
      real*8              :: th0(idim,k0)              !  Environmental temperature [ K ]
      real*8              :: tke(idim,0:k0)
      real*8                :: tr0(idim,k0,ncnst)       !  Environmental tracers [ #, kg/kg ]
      real*8              :: umf(idim,0:k0)           !  Updraft mass flux at the interfaces [ kg/m2/s ]
      real*8                :: qvten(idim,k0)           !  Tendency of water vapor specific humidity [ kg/kg/s ]
      real*8                :: qlten(idim,k0)           !  Tendency of liquid water specific humidity [ kg/kg/s ]
      real*8                :: qiten(idim,k0)           !  tendency of ice specific humidity [ kg/kg/s ]
      real*8                :: sten(idim,k0)            !  Tendency of static energy [ J/kg/s ]
      real*8                :: uten(idim,k0)            !  Tendency of zonal wind [ m/s2 ]
      real*8                :: vten(idim,k0)            !  Tendency of meridional wind [ m/s2 ]
      real*8                :: trten(idim,k0,ncnst)     !  Tendency of tracers [ #/s, kg/kg/s ]
      real*8                :: qrten(idim,k0)           !  Tendency of rain water specific humidity [ kg/kg/s ]
      real*8                :: qsten(idim,k0)           !  Tendency of snow speficif humidity [ kg/kg/s ]
      real*8              :: cufrc(idim,k0)           !  Shallow cumulus cloud fraction at the layer mid-point [ fraction ]
      real*8              :: qcu(idim,k0)             !  Condensate water specific humidity within cumulus updraft
                                                        ! at the layer mid-point [ kg/kg ]
      real*8              :: qlu(idim,k0)             !  Liquid water specific humidity within cumulus updraft
                                                        ! at the layer mid-point [ kg/kg ]
      real*8              :: qiu(idim,k0)             !  Ice specific humidity within cumulus updraft
                                                        ! at the layer mid-point [ kg/kg ]
      real*8              :: qc(idim,k0)              !  Tendency of cumulus condensate detrained into the environment [ kg/kg/s ]
      real             :: cnt(idim)                 !  Cumulus top  interface index, cnt = kpen [ no ]
      real             :: cnb(idim)                 !  Cumulus base interface index, cnb = krel - 1 [ no ] 
      real*8                :: wu(idim,0:k0)
      real*8                :: qtu(idim,0:k0)
      real*8                :: thlu(idim,0:k0)
      real*8                :: thvu(idim,0:k0)
      real*8                :: uu(idim,0:k0)
      real*8                :: vu(idim,0:k0)
      real*8                :: fer(idim,k0)
      real*8                :: fdr(idim,k0)
      real*8                :: xc(idim,k0)
      real*8                :: qldet(idim,k0)
      real*8                :: qidet(idim,k0)
      real*8                :: qlsub(idim,k0)
      real*8                :: qisub(idim,k0)
      real*8                :: ndrop(idim,k0)
      real*8                :: nice(idim,k0)
...
!      print *,'pifc0_inv=',pifc0_inv
!      print *,'pifc0=',pifc0

      pifc0(:idim,k0) = pmid0(:idim,k0)+(pmid0(:idim,k0)-pifc0(:idim,k0-1))
      zifc0(:idim,k0) = zmid0(:idim,k0)+(zmid0(:idim,k0)-zifc0(:idim,k0-1))
      exnifc0(:idim,k0) = exnifc0(:idim,k0-1)
      tke(:idim,k0) = tke(:idim,k0-1)
      kpbl = int(kpbl_inv)
!      print *,'pifc0=',pifc0

!      print *,"pmid0=",pmid0
!      print *,"exnmid0=",exnmid0
      ...
      call compute_uwshcu( idim,k0, dt, ncnst,pifc0, zifc0, &
           ...
           qrten, qsten, cufrc, qcu, qlu, qiu, cbmf, qc,    &
           cnt, cnb, cin, plcl, plfc, pinv, prel, pbup,     &
           wlcl, qtsrc, thlsrc, thvlsrc, tkeavg, cldtop,    &
           wu, qtu, thlu, thvu, uu, vu, fer, fdr, xc, qldet,& 
           qidet, qlsub, qisub, ndrop, nice, thlsrc_pert,   & 
           trten, shlwparams )
   ...
   subroutine compute_uwshcu(idim, k0, dt,ncnst, pifc0_in,zifc0_in,&
         exnifc0_in, pmid0_in, zmid0_in, exnmid0_in, dp0_in,       &
         u0_in, v0_in, qv0_in, ql0_in, qi0_in, th0_in, tr0_inout,  &
         kpbl_in, tke_in, cush_inout, umf_out,                     &
         qvten_out, qlten_out,                                     &
         qiten_out, sten_out, uten_out, vten_out, qrten_out,       &
         qsten_out, cufrc_out,                                     &
         qcu_out, qlu_out, qiu_out, cbmf_out, qc_out,              & 
         cnt_out, cnb_out, cinh_out, plcl_out, plfc_out, pinv_out, &
         prel_out, pbup_out, wlcl_out, qtsrc_out, thlsrc_out, thvlsrc_out,   &
         tkeavg_out, cldhgt_out, wu_out, qtu_out, thlu_out,        &
         thvu_out, uu_out, vu_out, fer_out, fdr_out, xc_out,       & 
         qldet_out, qidet_out, qlsub_out, qisub_out, ndrop_out,    &
         nice_out, thlsrc_pert, trten_out, shlwparams )  
    ...
    ! ------------------------------------------------------------ !
      ...
      real*8, intent(in) :: pifc0_in(   idim,0:k0 )    ! Environmental pressure at interfaces [Pa]
      real*8, intent(in) :: zifc0_in(   idim,0:k0 )    ! Environmental height at interfaces [m]
      real*8, intent(in) :: exnifc0_in( idim,0:k0 )    ! Exner function at interfaces
      real*8, intent(in) :: pmid0_in(    idim,k0   )    ! Environmental pressure at midpoints [Pa]
      real*8, intent(in) :: zmid0_in(    idim,k0   )    ! Environmental height at midpoints [m]
      real*8, intent(in) :: exnmid0_in(  idim,k0   )    ! Exner function at midpoints
      real*8, intent(in) :: dp0_in( idim,k0 )      ! Environmental layer pressure thickness
      real*8, intent(in) :: u0_in ( idim,k0 )      ! Environmental zonal wind [m/s]
      real*8, intent(in) :: v0_in ( idim,k0 )      ! Environmental meridional wind [m/s]
      real*8, intent(in) :: qv0_in( idim,k0 )      ! Environmental specific humidity
      real*8, intent(in) :: ql0_in( idim,k0 )      ! Environmental liquid water specific humidity
      real*8, intent(in) :: qi0_in( idim,k0 )      ! Environmental ice specific humidity
      real*8, intent(in) :: th0_in ( idim,k0 )     ! Environmental potential temperature [K]
      real*8, intent(inout) :: tr0_inout(idim,k0,ncnst)          !  Environmental tracers [ #, kg/kg ]
      real*8, intent(in) :: tke_in( idim,0:k0 )    ! Turbulent kinetic energy at interfaces
      integer, intent(in) :: kpbl_in( idim )         ! Boundary layer top layer index
      real, intent(in) :: thlsrc_pert
      real, intent(inout) :: cush_inout( idim )   ! Convective scale height [m]

      real*8, intent(out)   :: umf_out(idim,0:k0)             !  Updraft mass flux at the interfaces [ kg/m2/s ]
      real*8, intent(out)   :: qvten_out(idim,k0)             !  Tendency of water vapor specific humidity [ kg/kg/s ]
      real*8, intent(out)   :: qlten_out(idim,k0)             !  Tendency of liquid water specific humidity [ kg/kg/s ]
      real*8, intent(out)   :: qiten_out(idim,k0)             !  Tendency of ice specific humidity [ kg/kg/s ]
      real*8, intent(out)   :: sten_out(idim,k0)              !  Tendency of dry static energy [ J/kg/s ]
      real*8, intent(out)   :: uten_out(idim,k0)              !  Tendency of zonal wind [ m/s2 ]
      real*8, intent(out)   :: vten_out(idim,k0)              !  Tendency of meridional wind [ m/s2 ]
      real*8, intent(out)   :: trten_out(idim,k0,ncnst)       !  Tendency of tracers [ #/s, kg/kg/s ]
      real*8, intent(out)   :: qrten_out(idim,k0)             !  Tendency of rain water specific humidity [ kg/kg/s ]
      real*8, intent(out)   :: qsten_out(idim,k0)             !  Tendency of snow specific humidity [ kg/kg/s ]

      real*8, intent(out)   ::  wu_out(idim,0:k0)             !  Updraft vertical velocity
                                                            ! ( defined from the release level to 'kpen-1' interface )
      real*8, intent(out)   ::  qtu_out(idim,0:k0)            !  Updraft qt [ kg/kg ]
      real*8, intent(out)   ::  thlu_out(idim,0:k0)           !  Updraft thl [ K ]
      real*8, intent(out)   ::  thvu_out(idim,0:k0)           !  Updraft thv [ K ]
      real*8, intent(out)   ::  uu_out(idim,0:k0)             !  Updraft zonal wind [ m/s ] 
      real*8, intent(out)   ::  vu_out(idim,0:k0)             !  Updraft meridional wind [ m/s ]

      real*8, intent(out)   :: cufrc_out(idim,k0)             !  Shallow cumulus cloud fraction at the layer mid-point [ fraction ]
      real*8, intent(out)   :: qcu_out(idim,k0)               !  Condensate water specific humidity within cumulus updraft [ kg/kg ]
      real*8, intent(out)   :: qlu_out(idim,k0)               !  Liquid water specific humidity within cumulus updraft [ kg/kg ]
      real*8, intent(out)   :: qiu_out(idim,k0)               !  Ice specific humidity within cumulus updraft [ kg/kg ]
      real, intent(out)   :: cbmf_out(idim)                 ! Cloud base mass flux [kg/m2/s]
      real*8, intent(out)   :: qc_out(idim,k0)                !  Tendency of detrained cumulus condensate
     ...
     real*8, intent(out)      :: qldet_out(idim,k0)
     real*8, intent(out)      :: qidet_out(idim,k0)
     real*8, intent(out)      :: qlsub_out(idim,k0)
     real*8, intent(out)      :: qisub_out(idim,k0)
     real*8, intent(out)      :: ndrop_out(idim,k0)
     real*8, intent(out)      :: nice_out(idim,k0)
    ...
    real            cldhgt_out(idim)
    real*8          qtten_out(idim,k0)             !  Tendency of qt [ kg/kg/s ]
    real*8          slten_out(idim,k0)             !  Tendency of sl [ J/kg/s ]
    real*8          ufrc_out(idim,0:k0)            !  Updraft fractional area at the interfaces [ fraction ]
!    real*8          uflx_out(idim,0:k0)            !  Updraft/pen.entrainment zonal momentum flux [ m/s/m2/s ]
!    real*8          vflx_out(idim,0:k0)            !  Updraft/pen.entrainment meridional momentum flux [ m/s/m2/s ]
    real*8          fer_out(idim,k0)               !  Fractional lateral entrainment rate [ 1/Pa ]
    real*8          fdr_out(idim,k0)               !  Fractional lateral detrainment rate [ 1/Pa ]
    real*8          xc_out(idim,k0)
!    real*8          cinh_out(idim)                  !  Convective INhibition upto LFC (CIN) [ J/kg ]
    real*8          trflx_out(idim,0:k0,ncnst)     !  Updraft/pen.entrainment tracer flux [ #/m2/s, kg/kg/m2/s ] 
      ...
      real*8 :: exnmid0(k0)
      real*8 :: exnifc0(0:k0)
      real*8 :: sstr0(k0,ncnst)

   ! 2-1. For preventing negative condensate at the provisional time step

    real*8    qv0_star(k0)                                 !  Environmental water vapor specific humidity [ kg/kg ]
    real*8    ql0_star(k0)                                 !  Environmental liquid water specific humidity [ kg/kg ]
    real*8    qi0_star(k0)                                 !  Environmental ice specific humidity [ kg/kg ]
    real*8    s0_star(k0)                                  !  Environmental dry static energy [ J/kg ]


   ! 3. Variables associated with cumulus convection

    real*8    umf(0:k0)                                    !  Updraft mass
    real*8    emf(0:k0)                                    !  Penetrative
    real*8    qvten(k0)                                    !  Tendency of
    real*8    qlten(k0)                                    !  Tendency of
    real*8    qiten(k0)                                    !  Tendency of
    real*8    sten(k0)                                     !  Tendency of
    real*8    uten(k0)                                     !  Tendency of
    real*8    vten(k0)                                     !  Tendency of
    real*8    qrten(k0)                                    !  Tendency of
    real*8    qsten(k0)                                    !  Tendency of
    real*8    slflx(0:k0)                                  !

    real*8    qtflx(0:k0)                                  !
    real*8    uflx(0:k0)                                   !
    real*8    vflx(0:k0)                                   !
    real*8    cufrc(k0)                                    !  Shallow
    real*8    qcu(k0)                                      !  Condensate

    real*8    qlu(k0)                                      !  Liquid water
    real*8    qiu(k0)                                      !  Ice specific
    real*8    dwten(k0)                                    !  Detrained
    real*8    diten(k0)                                    !  Detrained ice
    real*8    fer(k0)                                      !  Fractional
    real*8    fdr(k0)                                      !  Fractional
    real*8    xco(k0)
    real*8    uf(k0)                                       !  Zonal wind at
    real*8    vf(k0)                                       !  Meridional
    real*8    qc(k0)                                       !  Tendency due
    real*8    qlten_det(k0)
    real*8    qiten_det(k0)
    real*8    qldet(k0)
    real*8    qidet(k0)

    real*8    qc_l(k0)                                     !  Tendency due
    real*8    qc_i(k0)                                     !  Tendency due
    real*8    qc_lm
    real*8    qc_im
    real*8    nc_lm
    real*8    nc_im
    ...
    real*8    nl_emf_kbup
    real*8    ni_emf_kbup
    real*8    cnt                                           !  Cumulus top
    real*8    cnb                                           !  Cumulus base
    real*8    qtten(k0)                                    !  Tendency of
    real*8    slten(k0)                                    !  Tendency of
    real*8    ufrc(0:k0)                                   !  Updraft
    real*8    trten(k0,ncnst)                              !  Tendency of
    real*8    trflx(0:k0,ncnst)                            !  Flux of
    real*8    trflx_d(0:k0)                                !  Adjustive
    real*8    trflx_u(0:k0)                                !  Adjustive
    real*8    trmin
    real*8    pdelx, dum 
    ...
    real*8    uemf(0:k0)                                   !  Net updraft mass flux at the interface ( emf + umf ) [ kg/m2/s ]
    real*8    comsub(k0)                                   !  Compensating subsidence
                                                              ! at the layer mid-point ( unit of mass flux, umf ) [ kg/m2/s ]
    real*8    qlten_sink(k0)                               !  Liquid condensate tendency
                                                              ! by compensating subsidence/upwelling [ kg/kg/s ]
    real*8    qiten_sink(k0)                               !  Ice    condensate tendency
                                                              ! by compensating subsidence/upwelling [ kg/kg/s ]
    real*8    thlten_sub, qtten_sub                         !  Tendency of conservative scalars
                                                              ! by compensating subsidence/upwelling
    real*8    qlten_sub, qiten_sub                          !  Tendency of ql0, qi0
                                                              ! by compensating subsidence/upwelling
    real*8    nlten_sub, niten_sub                          !  Tendency of nl0, ni0
                                                              ! by compensating subsidence/upwelling
    real*8    thl_prog, qt_prog                             !  Prognosed 'thl, qt'
                                                              ! by compensating subsidence/upwelling


    !----- Variables describing cumulus updraft

    real*8    wu(0:k0)                                     !  Updraft vertical velocity at the interface [ m/s ]
    real*8    thlu(0:k0)                                   !  Updraft liquid potential temperature at the interface [ K ]
    real*8    qtu(0:k0)                                    !  Updraft total specific humidity at the interface [ kg/kg ]
    real*8    uu(0:k0)                                     !  Updraft zonal wind at the interface [ m/s ]
    real*8    vu(0:k0)                                     !  Updraft meridional wind at the interface [ m/s ]
    real*8    thvu(0:k0)                                   !  Updraft virtual potential temperature at the interface [ m/s ]
    real*8    rei(k0)                                      !  Updraft fractional mixing rate with the environment [ 1/Pa ]
    real*8    tru(0:k0,ncnst)                              !  Updraft tracers [ #, kg/kg ]
    ...
    real*8    thlu_emf(0:k0)                               !  Penetrative downdraft liquid potential temperature
                                                              ! at entraining interfaces [ K ]
    real*8    qtu_emf(0:k0)                                !  Penetrative downdraft total water
                                                              ! at entraining interfaces [ kg/kg ]
    real*8    uu_emf(0:k0)                                 !  Penetrative downdraft zonal wind
                                                              ! at entraining interfaces [ m/s ]
    real*8    vu_emf(0:k0)                                 !  Penetrative downdraft meridional wind
                                                              ! at entraining interfaces [ m/s ]
    real*8    tru_emf(0:k0,ncnst)                          !  Penetrative Downdraft tracers
                                                              ! at entraining interfaces [ #, kg/kg ] 
      ...
      real*8       cin, cinlcl
      real*8       thlsrc, qtsrc, usrc, vsrc, thvlsrc
      real*8       uplus, vplus
      real*8       trsrc(ncnst), tre(ncnst)
      real*8       plcl, plfc, prel, wrel
      real*8       ee2, ud2, wtw, wtwb
      real*8       xc
      real*8       cldhgt, scaleh, tscaleh, cridis
      real*8       sigmaw, tkeavg, dpsum, dpi, thvlmin
      real*8       thlxsat, qtxsat, thvxsat, x_cu, x_en, thv_x0, thv_x1
      real*8       dpe, exne, thvebot, thle, qte, ue, ve, thlue, qtue, wue
      real*8       mu, mumin0, mumin2, mulcl, mulclstar
      real*8       cbmf, wcrit, winv, wlcl, ufrcinv, ufrclcl
      real*8       exql, exqi, ppen

      real*8       thj, qvj, qlj, qij, thvj, tj, thv0j, rhomid0j, rhoifc0j, qse
      real*8       thl0top, thl0bot, qt0bot, qt0top, thvubot, thvutop
      real*8       thl0lcl, qt0lcl, thv0lcl, thv0rel, rho0inv, autodet
      real*8       thlu_top, qtu_top, qlu_top, qiu_top, qlu_mid, qiu_mid, exntop
      real*8       aquad, bquad, cquad, xc1, xc2, excessu, excess0, xsat, xs1, xs2
      real*8       bogbot, bogtop, delbog, drage, expfac
      real*8       rcwp, rlwp, riwp, qcubelow, qlubelow, qiubelow
      real*8       qs
      real         qsat_arg, qsat_pe 
      real*8       pe
      real*8       xsrc, xmean, xtop, xbot, xflx(0:k0)

     
    !----- Some diagnostic internal output variables

    real*8  ufrcinvbase_out(idim)                            !  Cumulus updraft fraction at the PBL top [ fraction ]
    real*8  ufrclcl_out(idim)                                !  Cumulus updraft fraction at the LCL
                                                              ! ( or PBL top when LCL is below PBL top ) [ fraction ]
    real*8  winvbase_out(idim)                               !  Cumulus updraft velocity at the PBL top [ m/s ]
    real  wlcl_out(idim)                                   !  Cumulus updraft velocity at the LCL
                                                              ! ( or PBL top when LCL is below PBL top ) [ m/s ]
!    real*8  pbup_out(idim)                                   !  Highest interface level of positive buoyancy [ Pa ]
    real*8  ppen_out(idim)                                   !  Highest interface evel where Cu w = 0 [ Pa ]
    real  qtsrc_out(idim)                                  !  Sourse air qt [ kg/kg ]
    real  thlsrc_out(idim)                                 !  Sourse air thl [ K ]
    real  thvlsrc_out(idim)                                !  Sourse air thvl [ K ]
    real*8  emfkbup_out(idim)                                !  Penetrative downward mass flux at 'kbup' interface [ kg/m2/s ]
    real*8  cinlclh_out(idim)                                !  Convective INhibition upto LCL (CIN) [ J/kg = m2/s2 ]
    real*8  cbmflimit_out(idim)                              !  Cloud base mass flux limiter [ kg/m2/s ]
    real*8  zinv_out(idim)                                   !  PBL top height [ m ]
    real*8  rcwp_out(idim)                                   !  Layer mean Cumulus LWP+IWP [ kg/m2 ] 
    real*8  rlwp_out(idim)                                   !  Layer mean Cumulus LWP [ kg/m2 ] 
    real*8  riwp_out(idim)                                   !  Layer mean Cumulus IWP [ kg/m2 ] 

    real*8  qtu_emf_out(idim,0:k0)                          !  Penetratively entrained qt [ kg/kg ]   
    real*8  thlu_emf_out(idim,0:k0)                         !  Penetratively entrained thl [ K ]
    real*8  uu_emf_out(idim,0:k0)                           !  Penetratively entrained u [ m/s ]
    real*8  vu_emf_out(idim,0:k0)                           !  Penetratively entrained v [ m/s ]
    real*8  uemf_out(idim,0:k0)                             !  Net upward mass flux
                                                              ! including penetrative entrainment (umf+emf) [ kg/m2/s ]
    real*8  dwten_out(idim,k0)
    real*8  diten_out(idim,k0)

    real*8  tru_out(idim,0:k0,ncnst)                        !  Updraft tracers [ #, kg/kg ]   
    real*8  tru_emf_out(idim,0:k0,ncnst)                    !  Penetratively entrained tracers [ #, kg/kg ]

    real*8  wu_s(0:k0)                                     !  Same as above but for implicit CIN
    real*8  qtu_s(0:k0)
    real*8  thlu_s(0:k0)
    real*8  thvu_s(0:k0)
    real*8  uu_s(0:k0)
    real*8  vu_s(0:k0)
    real*8  qtu_emf_s(0:k0) 
    real*8  thlu_emf_s(0:k0)  
    real*8  uu_emf_s(0:k0)   
    real*8  vu_emf_s(0:k0)
    real*8  uemf_s(0:k0)   
    real*8  tru_s(0:k0,ncnst)
    real*8  tru_emf_s(0:k0,ncnst)   

    real*8  dwten_s(k0)
    real*8  diten_s(k0)

    real*8  excessu_arr_out(idim,k0)
    real*8  excessu_arr(k0) 
    real*8  excessu_arr_s(k0)
    real*8  excess0_arr_out(idim,k0)
    real*8  excess0_arr(k0)
    real*8  excess0_arr_s(k0)
    real*8  xc_arr_out(idim,k0)
    real*8  xc_arr(k0)
    real*8  xc_arr_s(k0)
    real*8  aquad_arr_out(idim,k0)
    real*8  aquad_arr(k0)
    real*8  aquad_arr_s(k0)
    real*8  bquad_arr_out(idim,k0)
    real*8  bquad_arr(k0)
    real*8  bquad_arr_s(k0)
    real*8  cquad_arr_out(idim,k0) 
    real*8  cquad_arr(k0)
    real*8  cquad_arr_s(k0)
    real*8  bogbot_arr_out(idim,k0)
    real*8  bogbot_arr(k0)
    real*8  bogbot_arr_s(k0)
    real*8  bogtop_arr_out(idim,k0)
    real*8  bogtop_arr(k0)
    real*8  bogtop_arr_s(k0)

    real*8       exit_ufrc(idim)
    real*8       exit_wtw(idim)
    real*8       exit_drycore(idim)
    real*8       exit_wu(idim)
    real*8       exit_cufilter(idim)
    real*8       exit_rei(idim)
    real*8       exit_kinv1(idim)
    real*8       exit_klfck0(idim)
    real*8       exit_klclk0(idim)
    real*8       exit_uwcu(idim)
    real*8       exit_conden(idim)

    real*8       limit_cinlcl(idim)
    real*8       limit_cin(idim)
    real*8       ind_delcin(idim)
    real*8       limit_rei(idim)
    real*8       limit_shcu(idim)
    real*8       limit_negcon(idim)
    real*8       limit_ufrc(idim)
    real*8       limit_ppen(idim)
    real*8       limit_emf(idim)
    real*8       limit_cbmf(idim)

    real*8 :: ufrcinvbase_s, ufrclcl_s, winvbase_s, wlcl_s, plcl_s, pinv_s, prel_s, plfc_s, &
              qtsrc_s, thlsrc_s, thvlsrc_s, emfkbup_s, cinlcl_s, pbup_s, ppen_s, cbmflimit_s, &
              tkeavg_s, zinv_s, rcwp_s, rlwp_s, riwp_s 
    real*8 :: ufrcinvbase, winvbase, emfkbup, cbmflimit

    !----- Variables for implicit CIN computation

    real*8, dimension(k0)   :: qv0_s  , ql0_s   , qi0_s   , s0_s    , u0_s    ,          & 
    ...
    real*8, dimension(0:k0) :: umf_s  , slflx_s , qtflx_s , ufrc_s  , uflx_s , vflx_s
    real*8                  :: cush_s , cin_s   , cbmf_s, cnt_s, cnb_s
    real*8                  :: cin_i, cin_f, del_CIN, ke, alpha !, thlj
    real*8                  :: cinlcl_i, cinlcl_f, del_cinlcl
    integer                 :: iter
    
    real*8, dimension(k0,ncnst)   :: tr0_s, trten_s
    real*8, dimension(0:k0,ncnst) :: trflx_s

    !----- Variables for temporary storage

      real*8, dimension(k0) :: qv0_o, ql0_o, qi0_o, t0_o, s0_o, u0_o, v0_o
      real*8, dimension(k0) :: qt0_o    , thl0_o   , thvl0_o   ,                         &
                              thv0bot_o, thv0top_o, thvl0bot_o, thvl0top_o,             &
                              ssthl0_o , ssqt0_o  , ssu0_o    , ssv0_o
    real*8                  :: tkeavg_o , thvlmin_o, qtsrc_o, thvlsrc_o, thlsrc_o ,    &
                               usrc_o, vsrc_o, plcl_o, plfc_o, thv0lcl_o
                                        
    integer               :: kinv_o   , klcl_o   , klfc_o  

    real*8, dimension(k0,ncnst)   :: tr0_o
    real*8, dimension(k0,ncnst)   :: trten_o, sstr0_o  
    real*8, dimension(0:k0,ncnst) :: trflx_o
    real*8, dimension(ncnst)       :: trsrc_o
    ...
    logical :: scverbose
    real*8    :: rpen     ! penetrative entrainment efficiency

    ! ----------------------- !
    ! For lateral entrainment !
    ! ----------------------- !

    real*8 :: rle          !  For critical stopping distance for lateral entrainment [no unit]
    real*8 :: rkm          !  Determine the amount of air that is involved in buoyancy-sorting [no unit]
    real*8 :: rkfre        !  Vertical velocity variance as fraction of  tke. 
    real*8 :: rmaxfrac     !  Maximum allowable 'core' updraft fraction
    real*8 :: mumin1       !  Normalized CIN ('mu') corresponding to 'rmaxfrac' at the PBL top
    ...
    real*8 :: rbuoy        !  For nonhydrostatic pressure effects on updraft [no unit]
    real*8 :: rdrag        !  Drag coefficient [no unit]
    real*8 :: epsvarw      !  Variance of w at PBL top by meso-scale component [m2/s2]          
    real*8 :: PGFc         !  This is used for calculating vertical variations cumulus  
                         !  'u' & 'v' by horizontal PGF during upward motion [no unit]

!!! TEMPORARY:  should be ncnst array of minimum values for all constituents
    real*8, parameter,dimension(4) :: qmin = [0._r8,0._r8,0._r8,0._r8]
...
!!! TEMPORARY test settings
    ...
    cldhgt_out(:idim)            = 0.0_r8
    umf_out(:idim,0:k0)          = 0.0_r8

    cufrc_out(:idim,:k0)         = 0.0_r8
    qcu_out(:idim,:k0)           = 0.0_r8
    qlu_out(:idim,:k0)           = 0.0_r8
    qiu_out(:idim,:k0)           = 0.0_r8
    fer_out(:idim,:k0)           = 0.0_r8
    fdr_out(:idim,:k0)           = 0.0_r8
    xc_out(:idim,:k0)            = 0.0_r8
    cinh_out(:idim)              = MAPL_UNDEF
    cinlclh_out(:idim)           = MAPL_UNDEF
    cbmf_out(:idim)              = 0.0_r8
    qc_out(:idim,:k0)            = 0.0_r8
    qldet_out(:idim,:k0)         = 0.0_r8
    qidet_out(:idim,:k0)         = 0.0_r8
    qlsub_out(:idim,:k0)         = 0.0_r8
    qisub_out(:idim,:k0)         = 0.0_r8
    ndrop_out(:idim,:k0)         = 0.0_r8
    nice_out(:idim,:k0)         = 0.0_r8

    cnt_out(:idim)               = real(k0)
    cnb_out(:idim)               = 0.0_r8
    ufrc_out(:idim,0:k0)         = 0.0_r8
!    uflx_out(:idim,0:k0)         = 0.0_r8
!    vflx_out(:idim,0:k0)         = 0.0_r8


    ufrcinvbase_out(:idim)       = 0.0_r8
    ufrclcl_out(:idim)           = 0.0_r8
    winvbase_out(:idim)          = 0.0_r8
    wlcl_out(:idim)              = 0.0_r8
    plcl_out(:idim)              = 0.0_r8
    pinv_out(:idim)              = 0.0_r8
    plfc_out(:idim)              = 0.0_r8
    prel_out(:idim)              = 0.0_r8
    pbup_out(:idim)              = 0.0_r8
    ppen_out(:idim)              = 0.0_r8
    qtsrc_out(:idim)             = 0.0_r8
    thlsrc_out(:idim)            = 0.0_r8
    thvlsrc_out(:idim)           = 0.0_r8
    emfkbup_out(:idim)           = 0.0_r8
    cbmflimit_out(:idim)         = 0.0_r8
    tkeavg_out(:idim)            = 0.0_r8
    zinv_out(:idim)              = 0.0_r8
    rcwp_out(:idim)              = 0.0_r8
    rlwp_out(:idim)              = 0.0_r8
    riwp_out(:idim)              = 0.0_r8
    ...
    qtu_emf_out(:idim,0:k0)     = 0.0_r8
    thlu_emf_out(:idim,0:k0)    = 0.0_r8
    uu_emf_out(:idim,0:k0)      = 0.0_r8
    vu_emf_out(:idim,0:k0)      = 0.0_r8
    uemf_out(:idim,0:k0)        = 0.0_r8

    dwten_out(:idim,:k0)        = 0.0_r8
    diten_out(:idim,:k0)        = 0.0_r8

    trten_out(:idim,:k0,:ncnst)    = 0.0_r8
    trflx_out(:idim,0:k0,:ncnst)   = 0.0_r8
    tru_out(:idim,0:k0,:ncnst)     = 0.0_r8
    tru_emf_out(:idim,0:k0,:ncnst) = 0.0_r8

    excessu_arr_out(:idim,:k0)   = 0.0_r8
    excess0_arr_out(:idim,:k0)   = 0.0_r8
    xc_arr_out(:idim,:k0)        = 0.0_r8
    aquad_arr_out(:idim,:k0)     = 0.0_r8
    bquad_arr_out(:idim,:k0)     = 0.0_r8
    cquad_arr_out(:idim,:k0)     = 0.0_r8
    bogbot_arr_out(:idim,:k0)    = 0.0_r8
    bogtop_arr_out(:idim,:k0)    = 0.0_r8

    exit_UWCu(:idim)             = 0.0_r8 
    exit_conden(:idim)           = 0.0_r8 
    exit_klclk0(:idim)           = 0.0_r8 
    exit_klfck0(:idim)           = 0.0_r8 
    exit_ufrc(:idim)             = 0.0_r8 
    exit_wtw(:idim)              = 0.0_r8 
    exit_drycore(:idim)          = 0.0_r8 
    exit_wu(:idim)               = 0.0_r8 
    exit_cufilter(:idim)         = 0.0_r8 
    exit_kinv1(:idim)            = 0.0_r8 
    exit_rei(:idim)              = 0.0_r8 

    limit_shcu(:idim)            = 0.0_r8 
    limit_negcon(:idim)          = 0.0_r8 
    limit_ufrc(:idim)            = 0.0_r8
    limit_ppen(:idim)            = 0.0_r8
    limit_emf(:idim)             = 0.0_r8
    limit_cinlcl(:idim)          = 0.0_r8
    limit_cin(:idim)             = 0.0_r8
    limit_cbmf(:idim)            = 0.0_r8
    limit_rei(:idim)             = 0.0_r8

    ind_delcin(:idim)            = 0.0_r8
...
!         pblh            = pblh_in(i)
         ...
         thvl0(:k0)   = ( 1._r8 + zvir*qt0(:k0) )*thl0(:k0)
...
!         print *,'thvl0top=',thvl0top
      ...
      emfkbup             = 0.0_r8 
      cbmflimit           = 0.0_r8
      excessu_arr(:k0)   = 0.0_r8
      excess0_arr(:k0)   = 0.0_r8
      xc_arr(:k0)        = 0.0_r8
      aquad_arr(:k0)     = 0.0_r8
      bquad_arr(:k0)     = 0.0_r8
      cquad_arr(:k0)     = 0.0_r8
      bogbot_arr(:k0)    = 0.0_r8
      bogtop_arr(:k0)    = 0.0_r8

      uemf(0:k0)         = 0.0_r8
      comsub(:k0)        = 0.0_r8
      qlten_sink(:k0)    = 0.0_r8
      qiten_sink(:k0)    = 0.0_r8
      qlten_det(:k0)     = 0.0_r8
      qiten_det(:k0)     = 0.0_r8 
!      nlten_sink(:k0)    = 0.0_r8
!      niten_sink(:k0)    = 0.0_r8 

      do m = 1, ncnst
         trflx(0:k0,m)   = 0.0_r8
         trten(:k0,m)    = 0.0_r8
         tru(0:k0,m)     = 0.0_r8
         tru_emf(0:k0,m) = 0.0_r8
      enddo
        ...
        if( klcl .ge. kinv ) then
            ...
            do k = kinv, k0 - 1
               ...
               elseif( k .eq. klcl ) then
                   ...
                   if( cin .lt. 0._r8 ) limit_cinlcl(i) = 1.
...
!                call endrun
      ...
      ! ----------------------------------------------------------------------- !
       ...
       ! -------------------------------------------------------------------------- !
         ...
         pe      = 0.5_r8 * ( prel + pifc0(krel) )
         qsat_pe = 0.5_r8 * ( prel + pifc0(krel) )
         dpe     = prel - pifc0(krel)
         exne    = exnerfn(pe)
         thvebot = thv0rel
...
!         iter_scaleh = 1
       ...
       do k = krel, k0 - 1 ! Here, 'k' is a layer index.
         ...
         do iter_xc = 1, niter_xc
          ...
          ! cridis = 1._r8*(zifc0(k) - zifc0(k-1))  ! New code
          ...
          ! ----------------------------------------------------------------- !
            ...
            if( ( excessu .le. 0._r8 .and. excess0 .le. 0._r8 ) .or. ( excessu .ge. 0._r8 .and. excess0 .ge. 0._r8 ) ) then
                xc = min(1._r8,max(0._r8,1._r8-2._r8*rbuoy*g*cridis/wue**2.*(1._r8-thvj/thv0j)))
          ...
          ! rei(k) = ( rkm / scaleh / g / rho0j )        ! Default.
            rei(k) = ( 0.5_r8 * rkm / zmid0(k) / g /rhomid0j ) ! Alternative.
            if( xc .gt. 0.5_r8 ) rei(k) = min(rei(k),0.9_r8*log(dp0(k)/g/dt/umf(km1) + 1._r8)/dpe/(2._r8*xc-1.))
            fer(k) = rei(k) * ee2
            fdr(k) = rei(k) * ud2
            xco(k) = xc
          ...
          ! ----------------------------------------------------------- !
            ...
            if( drage*dpe .gt. 1.e-3_r8 ) then
              wtw = wtw*expfac + (delbog + (1._r8-expfac)*(bogbot + delbog/(-2._r8*drage*dpe)))/(rhomid0j*drage)
            else
     ...
     111 continue
          ...
          ! --------------------------------------------------------------------------- !
            ...
            if( use_self_detrain ) then
              ...
              umf(k)  = umf(k) * exp( 0.637_r8*(dpe/rhomid0j/g) * autodet )   
            end if      
          ...
          rhoifc0j  = pifc0(k) / ( r * 0.5_r8 * ( thv0bot(k+1) + thv0top(k) )*exnifc0(k) )
...
45      continue
       ...
       ! ------------------------------------------------------------------------------ ! 
         ...
         if( ppen .eq. -dp0(kpen) .or. ppen .eq. 0._r8 ) limit_ppen(i) = 1.
       ...
       ! ----------------------------------------------------------------------- !
         ...
         rhoifc0j = pifc0(kpen-1)/(r*0.5_r8*(thv0bot(kpen)+thv0top(kpen-1))*exnifc0(kpen-1))  
       ...
       ! ------------------------------------------------------------------------------ !
         ...
         emf(kpen:k0)     = 0._r8
         ufrc(kpen:k0)    = 0._r8
         dwten(kpen+1:k0) = 0._r8
         diten(kpen+1:k0) = 0._r8
         fer(kpen+1:k0)   = 0._r8
         fdr(kpen+1:k0)   = 0._r8
         xco(kpen+1:k0)   = 0._r8
       ...
       ! ---------------------------------------------------------------------- !
         ...
         do k = kpen - 1, kbup, -1  ! Here, 'k' is an interface index at which
          ...
          if( k .eq. kpen - 1 ) then
             ...
             if( ( umf(k)*ppen*rei(kpen)*rpen ) .lt. -0.1_r8*rhoifc0j )         limit_emf(i) = 1.
             if( ( umf(k)*ppen*rei(kpen)*rpen ) .lt. -0.9_r8*dp0(kpen)/g/dt ) limit_emf(i) = 1.             

             emf(k) = max( max( umf(k)*ppen*rei(kpen)*rpen, -0.1_r8*rhoifc0j), -0.9_r8*dp0(kpen)/g/dt)
             ...
             if( use_cumpenent ) then  ! Original Cumulative Penetrative Entrainment
                 ...
                 if( ( emf(k+1)-umf(k)*dp0(k+1)*rei(k+1)*rpen ) .lt. -0.1_r8*rhoifc0j )        limit_emf(i) = 1
                 if( ( emf(k+1)-umf(k)*dp0(k+1)*rei(k+1)*rpen ) .lt. -0.9_r8*dp0(k+1)/g/dt ) limit_emf(i) = 1         
                 emf(k) = max(max(emf(k+1)-umf(k)*dp0(k+1)*rei(k+1)*rpen, -0.1_r8*rhoifc0j), -0.9_r8*dp0(k+1)/g/dt )    
                 ...
                 if( ( -umf(k)*dp0(k+1)*rei(k+1)*rpen ) .lt. -0.1_r8*rhoifc0j )        limit_emf(i) = 1
                 if( ( -umf(k)*dp0(k+1)*rei(k+1)*rpen ) .lt. -0.9_r8*dp0(k+1)/g/dt ) limit_emf(i) = 1         
                 emf(k) = max(max(-umf(k)*dp0(k+1)*rei(k+1)*rpen, -0.1_r8*rhoifc0j), -0.9_r8*dp0(k+1)/g/dt )    
                 thlu_emf(k) = thl0(k+1)
                 qtu_emf(k)  =  qt0(k+1)
                 uu_emf(k)   =   u0(k+1)
                 vu_emf(k)   =   v0(k+1)
       ...
       do k = 1, kpen
          ...
          ! Compute in-cumulus condensate at the layer mid-point.
           ...
           elseif( k .eq. krel ) then 
              ...
              qlu_mid = 0.5_r8 * ( qlubelow + qlj ) * ( prel - pifc0(k) )/( pifc0(k-1) - pifc0(k) )
              qiu_mid = 0.5_r8 * ( qiubelow + qij ) * ( prel - pifc0(k) )/( pifc0(k-1) - pifc0(k) )
           elseif( k .eq. kpen ) then 
              ...
              qlu_mid = 0.5_r8 * ( qlubelow + qlj ) * ( -ppen )        /( pifc0(k-1) - pifc0(k) )
              qiu_mid = 0.5_r8 * ( qiubelow + qij ) * ( -ppen )        /( pifc0(k-1) - pifc0(k) )
              qlu_top = qlj
              qiu_top = qij
           else
              call conden(pifc0(k),thlu(k),qtu(k),thj,qvj,qlj,qij,qse,id_check)
...
!           endif
	   ...
	   ! 
           ...
           if ( ((qc_lm+qlten_sink(k))*dt+ql0(k)).lt.0._r8 ) then
              totsink = qc_lm+qlten_sink(k)
              if (totsink.ne.0._r8) then
                qc_lm = -(ql0(k)/dt) * qc_lm/totsink
                qlten_sink(k) = -(ql0(k)/dt) * qlten_sink(k)/totsink
                qlten_det(k) = qc_l(k) + qc_lm
              end if
           end if
           if ( ((qc_im+qiten_sink(k))*dt+qi0(k)).lt.0._r8 ) then
...
!the indices are not reversed, these variables go into compute_mcshallow_inv
     ...
     ndrop_out(i,:k0)            = qlten_det(:k0)/(4188.787_r8*rdrop**3)
!     ndrop_out(i,:k0)            = qlten_det(:k0)/(4.19e-12_r8) !(1.15e-11_r8) ! /drop mass
     nice_out(i,:k0)             = qiten_det(:k0)/(3.0e-10_r8) ! /crystal mass
     cnt_out(i)                  = cnt
     cnb_out(i)                  = cnb
...
333    if (id_exit) then
     ...
     umf_out(i,0:k0)             = 0._r8   
     qvten_out(i,:k0)            = 0._r8
     qlten_out(i,:k0)            = 0._r8
     qiten_out(i,:k0)            = 0._r8
     sten_out(i,:k0)             = 0._r8
     uten_out(i,:k0)             = 0._r8
     vten_out(i,:k0)             = 0._r8
     qrten_out(i,:k0)            = 0._r8
     qsten_out(i,:k0)            = 0._r8
     cufrc_out(i,:k0)            = 0._r8
     qcu_out(i,:k0)              = 0._r8
     qlu_out(i,:k0)              = 0._r8
     qiu_out(i,:k0)              = 0._r8
     cush_inout(i)               = -1._r8
     cbmf_out(i)                 = 0._r8   
     qc_out(i,:k0)               = 0._r8
     qldet_out(i,:k0)            = 0._r8
     qidet_out(i,:k0)            = 0._r8
     cnt_out(i)                  = 1._r8
     cnb_out(i)                  = real(k0)
     ...
     cinh_out(i)                 = cin 
     cinlclh_out(i)              = cinlcl
     qtten_out(i,k0:1:-1)        = 0._r8
     slten_out(i,k0:1:-1)        = 0._r8
     ufrc_out(i,k0:0:-1)         = 0._r8
!     uflx_out(i,k0:0:-1)         = 0._r8  
!     vflx_out(i,k0:0:-1)         = 0._r8  
     ...
     dwten_out(i,k0:1:-1)        = 0._r8    
     diten_out(i,k0:1:-1)        = 0._r8    

        excessu_arr_out(i,k0:1:-1)  = 0._r8    
        excess0_arr_out(i,k0:1:-1)  = 0._r8    
        xc_arr_out(i,k0:1:-1)       = 0._r8    
        aquad_arr_out(i,k0:1:-1)    = 0._r8    
        bquad_arr_out(i,k0:1:-1)    = 0._r8    
        cquad_arr_out(i,k0:1:-1)    = 0._r8    
        bogbot_arr_out(i,k0:1:-1)   = 0._r8    
        bogtop_arr_out(i,k0:1:-1)   = 0._r8    

        do m = 1, ncnst
          trten_out(i,:k0,m)       = 0._r8
          trflx_out(i,k0:0:-1,m)   = 0._r8  
          tru_out(i,k0:0:-1,m)     = 0._r8
          tru_emf_out(i,k0:0:-1,m) = 0._r8
        enddo
   ...
   function slope(k0,field,p0)
      ...
      integer, intent(in):: k0
      real*8             :: slope(k0)
      real*8, intent(in) :: field(k0)
      real*8, intent(in) :: p0(k0)
      real*8             :: below
      real*8             :: above
      integer            :: k
...
!     end if
   ...
   real*8 function compute_alpha(del_CIN,ke)
   ! ------------------------------------------------ !
  ...
  real*8 function compute_ppen(wtwb,D,bogbot,bogtop,rho0j,dpen)
  ! ----------------------------------------------------------- !
  ...
  ! non-zero fer(kpen).                                         !  
  ! ----------------------------------------------------------- !  
    real*8 :: wtwb, D, bogbot, bogtop, rho0j, dpen
...
!           end if
  ...
  ! ------------------------------------------------------------------------- !            
    ...
    real*8, intent(in)                     :: cbmf, xsrc, xmean, xtopin, xbotin
    real*8, intent(in),  dimension(0:mkx)  :: ps0
    real*8, intent(out), dimension(0:mkx)  :: xflx  
    integer k
    real, intent(in) ::  dt
    real*8 rcbmf, rpeff, dp, rr, pinv_eff, xtop, xbot, pinv, xtop_ori, xbot_ori
    ...
    rpeff = min( max(0._r8,rpeff), 1. )          ! As of this, 0<= rpeff <= 1   
  ...
  ! ------------------------------------------------------------------------------- !
    ...
    real*8, intent(in)     :: xlv, xls
    real*8, intent(in)     :: qvmin, qlmin, qimin
    real*8, intent(in)     :: dp(mkx)
    real, intent(in)       :: dt
    real*8, intent(inout)  :: qv(mkx), ql(mkx), qi(mkx), s(mkx)
    real*8, intent(inout)  :: qvten(mkx), qlten(mkx), qiten(mkx), sten(mkx)
    integer   k
    ...
    if( a .eq. 0._r8 ) then                            ! Form b*x + c = 0
        ...
        if( b .eq. 0._r8 ) then                        ! Form a*x**2 + c = 0
            ...
            if( (b**2 - 4._r8*a*c) .lt. 0._r8 ) then   ! Failure, no real roots
                 ...
                 r1 =  q/a
                 r2 =  c/q
!                 r1 = -0.5_r8*(b + sign(1.0_r8,b)*sqrt(b**2 - 4._r8*a*c))/a
!                 r2 = -0.5_r8*(b - sign(1.0_r8,b)*sqrt(b**2 - 4._r8*a*c))/a
            endif
       endif
    endif
  ...
  ! ----------------------------------------------------------- !
    real*8 pbot,thv0bot,ptop,thv0top,thvubot,thvutop,plfc,cin,frc
  ...
  ! ------------------------------------------------------- ! 
    real*8 :: single_cin
    real*8 :: pbot,thv0bot,ptop,thv0top,thvubot,thvutop 
