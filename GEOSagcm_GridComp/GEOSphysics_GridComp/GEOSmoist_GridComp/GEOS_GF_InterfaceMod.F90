...
subroutine GF_Initialize (MAPL, CLOCK, RC)
    ...
    IF (USE_GF2020==1) THEN
      ...
      if (INT(ZERO_DIFF_ENTR) == 0) then
        ...
        call MAPL_GetResource(MAPL, CUM_ENTR_RATE(DEEP)       , 'ENTR_DP:'               ,default= 1.5e-4,RC=STATUS );VERIFY_(STATUS)
        call MAPL_GetResource(MAPL, CUM_ENTR_RATE(MID)        , 'ENTR_MD:'               ,default= 6.0e-4,RC=STATUS );VERIFY_(STATUS)
      ...
      call MAPL_GetResource(MAPL, QRC_CRIT_OCN              , 'QRC_CRIT_OCN:'          ,default= 1.0e-4,RC=STATUS );VERIFY_(STATUS)
