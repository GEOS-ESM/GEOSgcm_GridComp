...
subroutine GF_Initialize (MAPL, CLOCK, RC)
    ...
    IF (USE_GF2020==1) THEN
      ...
      call MAPL_GetResource(MAPL, CUM_ENTR_RATE(DEEP)       , 'ENTR_DP:'               ,default= 0.5e-3,RC=STATUS );VERIFY_(STATUS)
      call MAPL_GetResource(MAPL, CUM_ENTR_RATE(SHAL)       , 'ENTR_SH:'               ,default= 2.0e-3,RC=STATUS );VERIFY_(STATUS)
      call MAPL_GetResource(MAPL, CUM_ENTR_RATE(MID)        , 'ENTR_MD:'               ,default= 1.0e-3,RC=STATUS );VERIFY_(STATUS)
