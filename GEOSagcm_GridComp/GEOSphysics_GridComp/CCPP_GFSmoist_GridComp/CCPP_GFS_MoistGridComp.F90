...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine MOIST_DRIVER(IM,JM,LM, advanceCount, RC)
      ...
      real, pointer, dimension(:,:,:) :: PLE, T, U, V, W, TH, OMEGA, ZLE
      ...
      real, pointer, dimension(:,:,:) :: DTDT_moist, DTDTCN
      ...
      real, pointer, dimension(:,:  ) :: LS_PRCP,CN_PRCP,SC_PRCP,TT_PRCP, PCU, PLS, XSNO, XICE, FRZR
      real, pointer, dimension(:,:,:) :: RH1, RHX, RH2, XQLLS, XQLCN, XCLLS, XCLCN, XQILS, XQICN
      real, pointer, dimension(:,:,:) :: RL,RR,RI,RS,RG,FCLD
...
#include "phy_var_declarations.inc"
      ...
      call MAPL_GetPointer(IMPORT, ZPBL,    'ZPBL_SC' , RC=STATUS); VERIFY_(STATUS)
  ...
  !   if(localPet == 31) print *, __FILE__, __LINE__, 'Rank=',localPet, maxval(T)
      ...
      call MAPL_GetPointer(IMPORT, KPBLIN,    'KPBL_SC' , RC=STATUS); VERIFY_(STATUS)
      ...
      call MAPL_GetPointer(EXPORT, FCLD,     'FCLD'    , RC=STATUS); VERIFY_(STATUS)
      call MAPL_GetPointer(EXPORT, RL,       'RL'      , RC=STATUS); VERIFY_(STATUS)
      call MAPL_GetPointer(EXPORT, RI,       'RI'      , RC=STATUS); VERIFY_(STATUS)
      call MAPL_GetPointer(EXPORT, RR,       'RR'      , RC=STATUS); VERIFY_(STATUS)
      call MAPL_GetPointer(EXPORT, RS,       'RS'      , RC=STATUS); VERIFY_(STATUS)
      call MAPL_GetPointer(EXPORT, RG,       'RG'      , RC=STATUS); VERIFY_(STATUS)
      ...
      call MAPL_GetPointer(EXPORT, XSNO,     'SNO'     , RC=STATUS); VERIFY_(STATUS)
      call MAPL_GetPointer(EXPORT, XICE,     'ICE'     , RC=STATUS); VERIFY_(STATUS)
...
!Vertical indexing is reversed for CCPP arrays
   ...
   !  if(localPet == 31) print *, __FILE__, __LINE__, 'Rank=',localPet, maxval(gt0)
      ...
      if (associated(FCLD   ))   FCLD    = MIN(CLLS+CLCN,1.0)
      ...
      do j = 1, JM
         do i = 1, IM
            ij =  i + (j-1)*IM
            if (associated(RL)) RL(i,j,1:LM) = rew(ij,LM:1:-1) * 1.e-6
            if (associated(RI)) RI(i,j,1:LM) = rei(ij,LM:1:-1) * 1.e-6
            if (associated(RR)) RR(i,j,1:LM) = rer(ij,LM:1:-1) * 1.e-6
            if (associated(RS)) RS(i,j,1:LM) = res(ij,LM:1:-1) * 1.e-6
            if (associated(RG)) RG(i,j,1:LM) = reg(ij,LM:1:-1) * 1.e-6
            ...
            if (associated(DTDTCN)) DTDTCN(i,j,1:LM) =  dt3dt(ij,LM:1:-1,4)/DT_MOIST
      ...
      if (associated(CN_PRCP)) CN_PRCP = convert_precip(rainc, DT_MOIST)
      if (associated(LS_PRCP)) LS_PRCP = convert_precip(tprcp-rainc, DT_MOIST)
      if (associated(PCU)) PCU = convert_precip(rainc, DT_MOIST)
      if (associated(PLS)) PLS = convert_precip(tprcp-rainc, DT_MOIST)
      if (associated(XSNO))     XSNO     = convert_precip(snow, DT_MOIST)
      if (associated(XICE))     XICE     = convert_precip(ice, DT_MOIST)
