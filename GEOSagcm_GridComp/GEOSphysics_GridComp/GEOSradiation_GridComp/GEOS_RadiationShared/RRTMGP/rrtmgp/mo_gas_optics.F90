...
contains
  ...
  function gas_optics_int(this,                             &
    ...
    real(wp), dimension(2,2,2,size(this%flavor,dim=2),size(play,dim=1), size(play,dim=2)) :: fmajor
    integer,  dimension(2,    size(this%flavor,dim=2),size(play,dim=1), size(play,dim=2)) :: jeta
  ...
  function compute_gas_taus(this,                       &
    ...
    integer,  dimension(2,    size(this%flavor,dim=2),ncol, nlay), intent(  out) :: jeta
    logical,  dimension(            ncol, nlay), intent(  out) :: tropo
    real(wp), dimension(2,2,2,size(this%flavor,dim=2),ncol, nlay), intent(  out) :: fmajor
    character(len=128)                                         :: error_msg
    ...
    real(wp), dimension(2,    size(this%flavor,dim=2),ncol,nlay) :: col_mix ! combination of major species's column amounts
                                                         ! index(1) : reference temperature level
                                                         ! index(2) : flavor
                                                         ! index(3) : layer
    real(wp), dimension(2,2,  size(this%flavor,dim=2),ncol,nlay) :: fminor ! interpolation fractions for minor species
  ...
  function source(this,                               &
    ...
    real(wp), dimension(2,2,2,size(this%flavor,dim=2),ncol,nlay),   &
                                           intent(in ) :: fmajor
    integer,  dimension(2,   size(this%flavor,dim=2),ncol,nlay),   &
                                           intent(in ) :: jeta
  ...
  pure function get_gases(this)
    class(ty_gas_optics), intent(in) :: this
    character(32), dimension(size(this%gas_names))     :: get_gases
