...
contains
  ...
  subroutine init_rng_mkl(this, seeds)
    ...
    if(status /= VSL_STATUS_OK) call stop_on_err("Error initializing random number stream")
  ...
  subroutine end_rng_mkl(this)
    ...
    if(status /= VSL_STATUS_OK) call stop_on_err("Error finalizing random number stream")
