...
! -----------------------------------------------------------
    ...
    call ESMF_GridCompGet( GC, name=COMP_NAME, VM=VMG, Grid=GRID, __RC__)
...
!-------------------------------------------------
    ...
    allocate(   Z0 (IM,JM), __STAT__)
    allocate(   TMP(IM,JM), __STAT__)
    allocate(   TTM(IM,JM), __STAT__)
    allocate(   QTM(IM,JM), __STAT__)
    allocate(   FAC(IM,JM), __STAT__)
    allocate(   TAU(IM,JM), __STAT__)
    allocate(   DTS(IM,JM), __STAT__)
    allocate(   DQS(IM,JM), __STAT__)

    allocate( DRPAR(IM,JM), __STAT__)
    allocate( DFPAR(IM,JM), __STAT__)
    allocate( DRNIR(IM,JM), __STAT__)
    allocate( DFNIR(IM,JM), __STAT__)
    allocate( DRUVR(IM,JM), __STAT__)
    allocate( DFUVR(IM,JM), __STAT__)
    allocate( ZTH  (IM,JM), __STAT__)
    allocate( SLR  (IM,JM), __STAT__)
                                         
    allocate(  DTSTILE(NT), __STAT__)
    allocate(  DQSTILE(NT), __STAT__)

! Get the insolation and zenith angle on grid and tiles
!------------------------------------------------------

    ! get normalized insolation SLR and ZTH=cos(SZA) for the current timestep interval
    ...
    ! actual surface insolation in superbands from normalized solar internals
    ! taking into account solar distance and projection effects (via SLR) and
    ! solar output (via SC).
! PMN: TODO ... code needs an AFU branch since then simple projection (SLR) effect not sufficient
    ...
    ! SLR and ZTH on tiles for current timestep interval

    allocate( SLITILE(NT), __STAT__)
    allocate( ZTHTILE(NT), __STAT__)
    ...
    ZTHTILE = max(0., ZTHTILE)

! We need atmospheric version of the run1 outputs put back on tiles
!------------------------------------------------------------------

    allocate( TSTILE(NT), __STAT__)
    allocate( QSTILE(NT), __STAT__)
    allocate( THTILE(NT), __STAT__)
    allocate( QHTILE(NT), __STAT__)
    allocate( UHTILE(NT), __STAT__)
    allocate( VHTILE(NT), __STAT__)
    allocate( CTTILE(NT), __STAT__)
    allocate( CQTILE(NT), __STAT__)
    allocate( CMTILE(NT), __STAT__)

    call MAPL_LocStreamTransform( LOCSTREAM, THTILE, TH, __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, QHTILE, QH, __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, UHTILE, UH, __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, VHTILE, VH, __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, CTTILE, CT, __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, CQTILE, CQ, __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, CMTILE, CM, __RC__)

! Allocate tile versions of imports
!----------------------------------

    allocate(    PSTILE(NT), __STAT__)
    allocate(    DZTILE(NT), __STAT__)
    allocate(    UUTILE(NT), __STAT__)
    allocate(   EVPTILE(NT), __STAT__)
    allocate(   SHFTILE(NT), __STAT__)
    allocate(   DEVTILE(NT), __STAT__)
    allocate(   DSHTILE(NT), __STAT__)
    allocate(   TMPTILE(NT), __STAT__)
    allocate(   PCUTILE(NT), __STAT__)
    allocate(   PLSTILE(NT), __STAT__)
    allocate( SNOFLTILE(NT), __STAT__)
    allocate( ICEFLTILE(NT), __STAT__)
    allocate(FRZRFLTILE(NT), __STAT__)
    allocate(  TAUXTILE(NT), __STAT__)
    allocate(  TAUYTILE(NT), __STAT__)
    allocate(   DRUTILE(NT), __STAT__)
    allocate(   DFUTILE(NT), __STAT__)
    allocate(   DRPTILE(NT), __STAT__)
    allocate(   DFPTILE(NT), __STAT__)
    allocate(   DRNTILE(NT), __STAT__)
    allocate(   DFNTILE(NT), __STAT__)
    allocate(   LWBTILE(NT), __STAT__)
    allocate(   ALWTILE(NT), __STAT__)
    allocate(   BLWTILE(NT), __STAT__)
    allocate(DUDPTILE(NT,NUM_DUDP), __STAT__)
    allocate(DUSVTILE(NT,NUM_DUSV), __STAT__)
    allocate(DUWTTILE(NT,NUM_DUWT), __STAT__)
    allocate(DUSDTILE(NT,NUM_DUSD), __STAT__)
    allocate(BCDPTILE(NT,NUM_BCDP), __STAT__)
    allocate(BCSVTILE(NT,NUM_BCSV), __STAT__)
    allocate(BCWTTILE(NT,NUM_BCWT), __STAT__)
    allocate(BCSDTILE(NT,NUM_BCSD), __STAT__)
    allocate(OCDPTILE(NT,NUM_OCDP), __STAT__)
    allocate(OCSVTILE(NT,NUM_OCSV), __STAT__)
    allocate(OCWTTILE(NT,NUM_OCWT), __STAT__)
    allocate(OCSDTILE(NT,NUM_OCSD), __STAT__)
    allocate(SUDPTILE(NT,NUM_SUDP), __STAT__)
    allocate(SUSVTILE(NT,NUM_SUSV), __STAT__)
    allocate(SUWTTILE(NT,NUM_SUWT), __STAT__)
    allocate(SUSDTILE(NT,NUM_SUSD), __STAT__)
    allocate(SSDPTILE(NT,NUM_SSDP), __STAT__)
    allocate(SSSVTILE(NT,NUM_SSSV), __STAT__)
    allocate(SSWTTILE(NT,NUM_SSWT), __STAT__)
    allocate(SSSDTILE(NT,NUM_SSSD), __STAT__)
    allocate( DTSDTTILE(NT), __STAT__)

! Transform imports to the tiles
!-------------------------------

    call MAPL_LocStreamTransform( LOCSTREAM, PSTILE,     PS,      __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, DZTILE,     DZ,      __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, UUTILE,     UU,      __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, EVPTILE,    EVAP,    __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, SHFTILE,    SH,      __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, DEVTILE,    DEVAP,   __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, DSHTILE,    DSH,     __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, PCUTILE,    RCU,     __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, PLSTILE,    RLS,     __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, SNOFLTILE,  SNO,     __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, ICEFLTILE,  ICE,     __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, FRZRFLTILE, FRZR,    __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, TAUXTILE,   TAUX,    __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, TAUYTILE,   TAUY,    __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, DRUTILE,    DRUVR,   __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, DFUTILE,    DFUVR,   __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, DRPTILE,    DRPAR,   __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, DFPTILE,    DFPAR,   __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, DRNTILE,    DRNIR,   __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, DFNTILE,    DFNIR,   __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, LWBTILE,    LWDNSRF, __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, ALWTILE,    ALW,     __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, BLWTILE,    BLW,     __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, DTSDTTILE,  DTSDT,   __RC__)

    if (DO_GOSWIM) then
       do K = 1, NUM_DUDP
          call MAPL_LocStreamTransform( LOCSTREAM, DUDPTILE(:,K), DUDP(:,:,K), __RC__)
       end do
       do K = 1, NUM_DUSV
          call MAPL_LocStreamTransform( LOCSTREAM, DUSVTILE(:,K), DUSV(:,:,K), __RC__)
       end do
       do K = 1, NUM_DUWT
          call MAPL_LocStreamTransform( LOCSTREAM, DUWTTILE(:,K), DUWT(:,:,K), __RC__)
       end do
       do K = 1, NUM_DUSD
          call MAPL_LocStreamTransform( LOCSTREAM, DUSDTILE(:,K), DUSD(:,:,K), __RC__)
       end do
       do K = 1, NUM_BCDP
          call MAPL_LocStreamTransform( LOCSTREAM, BCDPTILE(:,K), BCDP(:,:,K), __RC__)
       end do
       do K = 1, NUM_BCSV
          call MAPL_LocStreamTransform( LOCSTREAM, BCSVTILE(:,K), BCSV(:,:,K), __RC__)
       end do
       do K = 1, NUM_BCWT
          call MAPL_LocStreamTransform( LOCSTREAM, BCWTTILE(:,K), BCWT(:,:,K), __RC__)
       end do
       do K = 1, NUM_BCSD
          call MAPL_LocStreamTransform( LOCSTREAM, BCSDTILE(:,K), BCSD(:,:,K), __RC__)
       end do
       do K = 1, NUM_OCDP
          call MAPL_LocStreamTransform( LOCSTREAM, OCDPTILE(:,K), OCDP(:,:,K), __RC__)
       end do
       do K = 1, NUM_OCSV
          call MAPL_LocStreamTransform( LOCSTREAM, OCSVTILE(:,K), OCSV(:,:,K), __RC__)
       end do
       do K = 1, NUM_OCWT
          call MAPL_LocStreamTransform( LOCSTREAM, OCWTTILE(:,K), OCWT(:,:,K), __RC__)
       end do
       do K = 1, NUM_OCSD
          call MAPL_LocStreamTransform( LOCSTREAM, OCSDTILE(:,K), OCSD(:,:,K), __RC__)
       end do
       do K = 1, NUM_SUDP
          call MAPL_LocStreamTransform( LOCSTREAM, SUDPTILE(:,K), SUDP(:,:,K), __RC__)
       end do
       do K = 1, NUM_SUSV
          call MAPL_LocStreamTransform( LOCSTREAM, SUSVTILE(:,K), SUSV(:,:,K), __RC__)
       end do
       do K = 1, NUM_SUWT
          call MAPL_LocStreamTransform( LOCSTREAM, SUWTTILE(:,K), SUWT(:,:,K), __RC__)
       end do
       do K = 1, NUM_SUSD
          call MAPL_LocStreamTransform( LOCSTREAM, SUSDTILE(:,K), SUSD(:,:,K), __RC__)
       end do
       do K = 1, NUM_SSDP
          call MAPL_LocStreamTransform( LOCSTREAM, SSDPTILE(:,K), SSDP(:,:,K), __RC__)
       end do
       do K = 1, NUM_SSSV
          call MAPL_LocStreamTransform( LOCSTREAM, SSSVTILE(:,K), SSSV(:,:,K), __RC__)
       end do
       do K = 1, NUM_SSWT
          call MAPL_LocStreamTransform( LOCSTREAM, SSWTTILE(:,K), SSWT(:,:,K), __RC__)
       end do
       do K = 1, NUM_SSSD
          call MAPL_LocStreamTransform( LOCSTREAM, SSSDTILE(:,K), SSSD(:,:,K), __RC__)
       end do
    end if

    ! option to interpolate effective wind vectors such that
    ! stresses computed in all children will have smooth curl/divergence 
    call MAPL_GetResource(MAPL, iUseInterp, 'INTERPOLATE_ATMTAU:', default=0, __RC__)
    useInterp = (iUseInterp /= 0)

    allocate( UUA(IM,JM), __STAT__)
    allocate( VVA(IM,JM), __STAT__)
...
! The import taus come from turbulence and are stresses on the atmosphere
    ...
    call MKTILE( UUA, UUATILE, NT, __RC__)
    call MKTILE( VVA, VVATILE, NT, __RC__)

    call MAPL_LocStreamTransform( LOCSTREAM, UUATILE, UUA, INTERP=useInterp, __RC__)
    call MAPL_LocStreamTransform( LOCSTREAM, VVATILE, VVA, INTERP=useInterp, __RC__)
...
!--------------------------------------------------------
    ...
    call MKTILE( LST       , LSTTILE       , NT, __RC__)
    call MKTILE( ALBVR     , ALBVRTILE     , NT, __RC__)
    call MKTILE( ALBVF     , ALBVFTILE     , NT, __RC__)
    call MKTILE( ALBNR     , ALBNRTILE     , NT, __RC__)
    call MKTILE( ALBNF     , ALBNFTILE     , NT, __RC__)
    ...
    call MKTILE( EMISS     , EMISSTILE     , NT, __RC__)
    call MKTILE( FRI       , FRTILE        , NT, __RC__)
    call MKTILE( TSOIL1    , TSOIL1TILE    , NT, __RC__)
    call MKTILE( TSOIL2    , TSOIL2TILE    , NT, __RC__)
    call MKTILE( TSOIL3    , TSOIL3TILE    , NT, __RC__)
    call MKTILE( TSOIL4    , TSOIL4TILE    , NT, __RC__)
    call MKTILE( TSOIL5    , TSOIL5TILE    , NT, __RC__)
    call MKTILE( TSOIL6    , TSOIL6TILE    , NT, __RC__)
    call MKTILE( ASNOW     , ASNOWTILE     , NT, __RC__)
    call MKTILE( SHSNOW    , SHSNOWTILE    , NT, __RC__)
    call MKTILE( AVETSNOW  , AVETSNOWTILE  , NT, __RC__)
    call MKTILE( TPSNO     , TPSNOTILE     , NT, __RC__)
    call MKTILE( TPUST     , TPUSTTILE     , NT, __RC__)
    call MKTILE( TPSAT     , TPSATTILE     , NT, __RC__)
    call MKTILE( TPWLT     , TPWLTTILE     , NT, __RC__)
    call MKTILE( TPSURF    , TPSURFTILE    , NT, __RC__)
    call MKTILE( FRSAT     , FRSATTILE     , NT, __RC__)
    call MKTILE( FRUST     , FRUSTTILE     , NT, __RC__)
    call MKTILE( FRWLT     , FRWLTTILE     , NT, __RC__)
    call MKTILE( SNOMAS    , SNOWTILE      , NT, __RC__)
    call MKTILE( SNOWDP    , SNODTILE      , NT, __RC__)
    call MKTILE( WET1      , WET1TILE      , NT, __RC__)
    call MKTILE( WET2      , WET2TILE      , NT, __RC__)
    call MKTILE( WET3      , WET3TILE      , NT, __RC__)
    call MKTILE( WCSF      , WCSFTILE      , NT, __RC__)
    call MKTILE( WCRZ      , WCRZTILE      , NT, __RC__)
    call MKTILE( WCPR      , WCPRTILE      , NT, __RC__)
    call MKTILE( WESNN1    , WESNN1TILE    , NT, __RC__)
    call MKTILE( WESNN2    , WESNN2TILE    , NT, __RC__)
    call MKTILE( WESNN3    , WESNN3TILE    , NT, __RC__)
    call MKTILE( CAPAC     , CAPACTILE     , NT, __RC__)
    call MKTILE( CDCR2     , CDCR2TILE     , NT, __RC__)
    call MKTILE( HLATN     , HLATNTILE     , NT, __RC__)

    call MKTILE( HLATWTR   , HLATWTRTILE   , NT, __RC__)
    call MKTILE( HLATICE   , HLATICETILE   , NT, __RC__)
    call MKTILE( SHWTR     , SHWTRTILE     , NT, __RC__)
    call MKTILE( SHICE     , SHICETILE     , NT, __RC__)
    call MKTILE( TAUXW     , TAUXWTILE     , NT, __RC__)
    call MKTILE( TAUXI     , TAUXITILE     , NT, __RC__)
    call MKTILE( TAUYW     , TAUYWTILE     , NT, __RC__)
    call MKTILE( TAUYI     , TAUYITILE     , NT, __RC__)
    call MKTILE( LWNDWTR   , LWNDWTRTILE   , NT, __RC__)
    call MKTILE( SWNDWTR   , SWNDWTRTILE   , NT, __RC__)
    call MKTILE( LWNDICE   , LWNDICETILE   , NT, __RC__)
    call MKTILE( SWNDICE   , SWNDICETILE   , NT, __RC__)
    call MKTILE( SNOWOCN   , SNOWOCNTILE   , NT, __RC__)
    call MKTILE( RAINOCN   , RAINOCNTILE   , NT, __RC__)
    call MKTILE( TSKINW    , TSKINWTILE    , NT, __RC__)
    call MKTILE( TSKINICE  , TSKINICETILE  , NT, __RC__)

    call MKTILE( DCOOL     , DCOOL_TILE    , NT, __RC__)
    call MKTILE( DWARM     , DWARM_TILE    , NT, __RC__)
    call MKTILE( TDROP     , TDROP_TILE    , NT, __RC__)
    call MKTILE( QCOOL     , QCOOL_TILE    , NT, __RC__)
    call MKTILE( SWCOOL    , SWCOOL_TILE   , NT, __RC__)
    call MKTILE( USTARW    , USTARW_TILE   , NT, __RC__)
    call MKTILE( TBAR      , TBAR_TILE     , NT, __RC__)
    call MKTILE( LCOOL     , LCOOL_TILE    , NT, __RC__)
    call MKTILE( BCOOL     , BCOOL_TILE    , NT, __RC__)
    call MKTILE( TDEL      , TDEL_TILE     , NT, __RC__)
    call MKTILE( TS_FOUND  , TS_FOUND_TILE , NT, __RC__)
    call MKTILE( SS_FOUND  , SS_FOUND_TILE , NT, __RC__)
    call MKTILE( QWARM     , QWARM_TILE    , NT, __RC__)
    call MKTILE( SWWARM    , SWWARM_TILE   , NT, __RC__)
    call MKTILE( LANGM     , LANGM_TILE    , NT, __RC__)
    call MKTILE( PHIW      , PHIW_TILE     , NT, __RC__)
    call MKTILE( TAUTW     , TAUTW_TILE    , NT, __RC__)
    call MKTILE( ZETA_W    , ZETA_W_TILE   , NT, __RC__)
    call MKTILE( TWMTF     , TWMTF_TILE    , NT, __RC__)

    call MKTILE( HICE      , HICETILE      , NT, __RC__)
    call MKTILE( HSNO      , HSNOTILE      , NT, __RC__)
    call MKTILE( FRZMLT    , FRZMLTTILE    , NT, __RC__)
    call MKTILE( TSKINWCICE,TSKINWCICETILE , NT, __RC__)
    call MKTILE( ISTSFC    , ISTSFCTILE    , NT, __RC__)
    call MKTILE( SSKINW    , SSKINWTILE    , NT, __RC__)
    call MKTILE( MELTT     , MELTTTILE     , NT, __RC__)
    call MKTILE( MELTB     , MELTBTILE     , NT, __RC__)
    call MKTILE( MELTS     , MELTSTILE     , NT, __RC__)
    call MKTILE( MELTL     , MELTLTILE     , NT, __RC__)
    call MKTILE( FRAZIL    , FRAZILTILE    , NT, __RC__)
    call MKTILE( CONGEL    , CONGELTILE    , NT, __RC__)
    call MKTILE( SNOICE    , SNOICETILE    , NT, __RC__)
    call MKTILE( DAIDTT    , DAIDTTTILE    , NT, __RC__)
    call MKTILE( DVIDTT    , DVIDTTTILE    , NT, __RC__)
    call MKTILE( DAIDTD    , DAIDTDTILE    , NT, __RC__)
    call MKTILE( DVIDTD    , DVIDTDTILE    , NT, __RC__)
    call MKTILE( FBOT      , FBOTTILE      , NT, __RC__)
    call MKTILE( HFLUX     , HFLUXTILE     , NT, __RC__)
    call MKTILE( WFLUX     , WFLUXTILE     , NT, __RC__)
    call MKTILE( SFLUX     , SFLUXTILE     , NT, __RC__)
    call MKTILE( FSWTHRU   , FSWTHRUTILE   , NT, __RC__)
    call MKTILE( FSWABS    , FSWABSTILE    , NT, __RC__)
    call MKTILE( USTARI    , USTARITILE    , NT, __RC__)
    call MKTILE( FHOCN     , FHOCNTILE     , NT, __RC__)

    call MKTILE( EVAPOU    , EVAPOUTILE    , NT, __RC__)
    call MKTILE( SUBLIM    , SUBLIMTILE    , NT, __RC__)
    call MKTILE( SHOU      , SHOUTILE      , NT, __RC__)
    call MKTILE( HLWUP     , HLWUPTILE     , NT, __RC__)
    call MKTILE( LWNDSRF   , LWNDSRFTILE   , NT, __RC__)
    call MKTILE( SWNDSRF   , SWNDSRFTILE   , NT, __RC__)

!ALT  call MKTILE( RUNOFF   , RUNOFFTILE   , NT, __RC__)
!ALT  call MKTILE( DISCHARGE, DISCHARGETILE, NT, __RC__)
    if (associated( SURF_INTERNAL_STATE%LocalRoutings)) then ! routing file exists
       allocate( DISCHARGETILE(NT), __STAT__)
       DISCHARGETILE = MAPL_Undef
       allocate( RUNOFFTILE(NT), __STAT__)
       RUNOFFTILE = MAPL_Undef
    end if
    call MKTILE( RUNSURF   , RUNSURFTILE   , NT, __RC__)
    call MKTILE( BASEFLOW  , BASEFLOWTILE  , NT, __RC__)
    call MKTILE( ACCUM     , ACCUMTILE     , NT, __RC__)
    call MKTILE( SMELT     , SMELTTILE     , NT, __RC__)
    call MKTILE( EVEG      , EVEGTILE      , NT, __RC__)
    call MKTILE( EINT      , EINTTILE      , NT, __RC__)
    call MKTILE( EICE      , EICETILE      , NT, __RC__)
    call MKTILE( ESNO      , ESNOTILE      , NT, __RC__)
    call MKTILE( ESOI      , ESOITILE      , NT, __RC__)
    call MKTILE( WAT10CM   , WAT10CMTILE   , NT, __RC__)
    call MKTILE( WATSOI    , WATSOITILE    , NT, __RC__)
    call MKTILE( ICESOI    , ICESOITILE    , NT, __RC__)
    call MKTILE( EVLAND    , EVLANDTILE    , NT, __RC__)
    call MKTILE( PRLAND    , PRLANDTILE    , NT, __RC__)
    call MKTILE( SNOLAND   , SNOLANDTILE   , NT, __RC__)
    call MKTILE( DRPARLAND , DRPARLANDTILE , NT, __RC__)
    call MKTILE( DFPARLAND , DFPARLANDTILE , NT, __RC__)
    call MKTILE( LHSNOW    , LHSNOWTILE    , NT, __RC__)
    call MKTILE( SWNETSNOW , SWNETSNOWTILE , NT, __RC__)
    call MKTILE( LWUPSNOW  , LWUPSNOWTILE  , NT, __RC__)
    call MKTILE( LWDNSNOW  , LWDNSNOWTILE  , NT, __RC__)
    call MKTILE( TCSORIG   , TCSORIGTILE   , NT, __RC__)
    call MKTILE( TPSN1IN   , TPSN1INTILE   , NT, __RC__)
    call MKTILE( TPSN1OUT  , TPSN1OUTTILE  , NT, __RC__)
    call MKTILE( GHSNOW    , GHSNOWTILE    , NT, __RC__)
    call MKTILE( LHLAND    , LHLANDTILE    , NT, __RC__)
    call MKTILE( SHLAND    , SHLANDTILE    , NT, __RC__)
    call MKTILE( SWLAND    , SWLANDTILE    , NT, __RC__)
    call MKTILE( SWDOWNLAND, SWDOWNLANDTILE, NT, __RC__)
    call MKTILE( LWLAND    , LWLANDTILE    , NT, __RC__)
    call MKTILE( GHLAND    , GHLANDTILE    , NT, __RC__)
    call MKTILE( GHTSKIN   , GHTSKINTILE   , NT, __RC__)
    call MKTILE( SMLAND    , SMLANDTILE    , NT, __RC__)
    call MKTILE( QINFIL    , QINFILTILE    , NT, __RC__)
    call MKTILE( TWLAND    , TWLANDTILE    , NT, __RC__)
    call MKTILE( TELAND    , TELANDTILE    , NT, __RC__)
    call MKTILE( TSLAND    , TSLANDTILE    , NT, __RC__)
    call MKTILE( DWLAND    , DWLANDTILE    , NT, __RC__)
    call MKTILE( DHLAND    , DHLANDTILE    , NT, __RC__)
    call MKTILE( SPLAND    , SPLANDTILE    , NT, __RC__)
    call MKTILE( SPWATR    , SPWATRTILE    , NT, __RC__)
    call MKTILE( SPSNOW    , SPSNOWTILE    , NT, __RC__)
    call MKTILE( RDU001    , RDU001TILE    , NT, __RC__)
    call MKTILE( RDU002    , RDU002TILE    , NT, __RC__)
    call MKTILE( RDU003    , RDU003TILE    , NT, __RC__)
    call MKTILE( RDU004    , RDU004TILE    , NT, __RC__)
    call MKTILE( RDU005    , RDU005TILE    , NT, __RC__)
    call MKTILE( RBC001    , RBC001TILE    , NT, __RC__)
    call MKTILE( RBC002    , RBC002TILE    , NT, __RC__)
    call MKTILE( ROC001    , ROC001TILE    , NT, __RC__)
    call MKTILE( ROC002    , ROC002TILE    , NT, __RC__)
    call MKTILE( RMELTDU001, RMELTDU001TILE, NT, __RC__)
    call MKTILE( RMELTDU002, RMELTDU002TILE, NT, __RC__)
    call MKTILE( RMELTDU003, RMELTDU003TILE, NT, __RC__)
    call MKTILE( RMELTDU004, RMELTDU004TILE, NT, __RC__)
    call MKTILE( RMELTDU005, RMELTDU005TILE, NT, __RC__)
    call MKTILE( RMELTBC001, RMELTBC001TILE, NT, __RC__)
    call MKTILE( RMELTBC002, RMELTBC002TILE, NT, __RC__)
    call MKTILE( RMELTOC001, RMELTOC001TILE, NT, __RC__)
    call MKTILE( RMELTOC002, RMELTOC002TILE, NT, __RC__)
    call MKTILE( PEATCLSM_WATERLEVEL, PEATCLSM_WATERLEVELTILE, NT, __RC__)
    call MKTILE( PEATCLSM_FSWCHANGE , PEATCLSM_FSWCHANGETILE , NT, __RC__)
    
    if (LSM_CHOICE > 1) then
       call MKTILE( CNLAI  , CNLAITILE     , NT, __RC__)
       call MKTILE( CNTLAI , CNTLAITILE    , NT, __RC__)
       call MKTILE( CNSAI  , CNSAITILE     , NT, __RC__)
       call MKTILE( CNTOTC , CNTOTCTILE    , NT, __RC__)
       call MKTILE( CNVEGC , CNVEGCTILE    , NT, __RC__)
       call MKTILE( CNROOT , CNROOTTILE    , NT, __RC__)
       if (LSM_CHOICE == 3) then
          call MKTILE( CNFROOTC, CNFROOTCTILE, NT, __RC__)
       endif
       call MKTILE( CNNPP  , CNNPPTILE     , NT, __RC__)
       call MKTILE( CNGPP  , CNGPPTILE     , NT, __RC__)
       call MKTILE( CNSR   , CNSRTILE      , NT, __RC__)
       call MKTILE( CNNEE  , CNNEETILE     , NT, __RC__)
       call MKTILE( CNXSMR , CNXSMRTILE    , NT, __RC__)
       call MKTILE( CNADD  , CNADDTILE     , NT, __RC__)
       call MKTILE( CNLOSS , CNLOSSTILE    , NT, __RC__)
       call MKTILE( CNBURN , CNBURNTILE    , NT, __RC__)
       call MKTILE( PARABS , PARABSTILE    , NT, __RC__)
       call MKTILE( PARINC , PARINCTILE    , NT, __RC__)
       call MKTILE( SCSAT  , SCSATTILE     , NT, __RC__)
       call MKTILE( SCUNS  , SCUNSTILE     , NT, __RC__)
       call MKTILE( BTRANT , BTRANTTILE    , NT, __RC__)
       call MKTILE( SIF    , SIFTILE       , NT, __RC__)
       call MKTILE( CNFSEL , CNFSELTILE    , NT, __RC__)
    END IF

    FRTILE = 0.

!  Run all continental children (not ocean yet),
!   collecting RUNOFFTILE exports as we go.

    if (associated(RUNOFFTILE)) RUNOFFTILE = 0.

    do I = 1, NUM_CHILDREN
       if (I == OCEAN) cycle
       call DOTYPE( I, __RC__)
    end do
...
!----------------------------------------------------------
      ...
      call FILLIN_TILE( GIM(type), 'PS',     PSTILE,    XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DZ',     DZTILE,    XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'UU',     UUTILE,    XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'EVAP',   EVPTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'SH',     SHFTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DEVAP',  DEVTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DSH',    DSHTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'SNO',    SNOFLTILE, XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'ICE',    ICEFLTILE, XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'FRZR',   FRZRFLTILE,XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'PCU',    PCUTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'PLS',    PLSTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'TAUX',   TAUXTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'TAUY',   TAUYTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'UUA',    UUATILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'VVA',    VVATILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DRPAR',  DRPTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DFPAR',  DFPTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DRNIR',  DRNTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DFNIR',  DFNTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DRUVR',  DRUTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DFUVR',  DFUTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'LWDNSRF',LWBTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'ALW',    ALWTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'BLW',    BLWTILE,   XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DTSDT',  DTSDTTILE, XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DUDP',   DUDPTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DUSV',   DUSVTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DUWT',   DUWTTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'DUSD',   DUSDTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'BCDP',   BCDPTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'BCSV',   BCSVTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'BCWT',   BCWTTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'BCSD',   BCSDTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'OCDP',   OCDPTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'OCSV',   OCSVTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'OCWT',   OCWTTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'OCSD',   OCSDTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'SUDP',   SUDPTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'SUSV',   SUSVTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'SUWT',   SUWTTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'SUSD',   SUSDTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'SSDP',   SSDPTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'SSSV',   SSSVTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'SSWT',   SSWTTILE,  XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'SSSD',   SSSDTILE,  XFORM, __RC__)
 
      if (associated(DISCHARGETILE)) then
         call FILLIN_TILE( GIM(type), 'DISCHARGE', DISCHARGETILE, XFORM, __RC__)
      end if

      call FILLIN_TILE( GIM(type), 'ZTH',    ZTHTILE,   XFORM, __RC__)

      call FILLIN_TILE( GIM(type), 'THATM',  THTILE,    XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'QHATM',  QHTILE,    XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'UHATM',  UHTILE,    XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'VHATM',  VHTILE,    XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'CTATM',  CTTILE,    XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'CQATM',  CQTILE,    XFORM, __RC__)
      call FILLIN_TILE( GIM(type), 'CMATM',  CMTILE,    XFORM, __RC__)
...
! Some cannot be verified, because some children don't produce them
      ...
      call MAPL_GetPointer( GEX(type), dum, 'LST'       , ALLOC=associated(LSTTILE       ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TP1'       , ALLOC=associated(TSOIL1TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TP2'       , ALLOC=associated(TSOIL2TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TP3'       , ALLOC=associated(TSOIL3TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TP4'       , ALLOC=associated(TSOIL4TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TP5'       , ALLOC=associated(TSOIL5TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TP6'       , ALLOC=associated(TSOIL6TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ASNOW'     , ALLOC=associated(ASNOWTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SHSNOW'    , ALLOC=associated(SHSNOWTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'AVETSNOW'  , ALLOC=associated(AVETSNOWTILE  ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TPSNOW'    , ALLOC=associated(TPSNOTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TPUNST'    , ALLOC=associated(TPUSTTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TPSAT'     , ALLOC=associated(TPSATTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TPWLT'     , ALLOC=associated(TPWLTTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TPSURF'    , ALLOC=associated(TPSURFTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'FRSAT'     , ALLOC=associated(FRSATTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'FRUST'     , ALLOC=associated(FRUSTTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'FRWLT'     , ALLOC=associated(FRWLTTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SNOWMASS'  , ALLOC=associated(SNOWTILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SNOWDP'    , ALLOC=associated(SNODTILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WET1'      , ALLOC=associated(WET1TILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WET2'      , ALLOC=associated(WET2TILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WET3'      , ALLOC=associated(WET3TILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WCSF'      , ALLOC=associated(WCSFTILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WCRZ'      , ALLOC=associated(WCRZTILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WCPR'      , ALLOC=associated(WCPRTILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WESNN1'    , ALLOC=associated(WESNN1TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WESNN2'    , ALLOC=associated(WESNN2TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WESNN3'    , ALLOC=associated(WESNN3TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'CAPAC'     , ALLOC=associated(CAPACTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RUNOFF'    , ALLOC=associated(RUNOFFTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RUNSURF'   , ALLOC=associated(RUNSURFTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'BASEFLOW'  , ALLOC=associated(BASEFLOWTILE  ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ACCUM'     , ALLOC=associated(ACCUMTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SMELT'     , ALLOC=associated(SMELTTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'EVPVEG'    , ALLOC=associated(EVEGTILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'EVPINT'    , ALLOC=associated(EINTTILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'EVPICE'    , ALLOC=associated(EICETILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'EVPSNO'    , ALLOC=associated(ESNOTILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'EVPSOI'    , ALLOC=associated(ESOITILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WAT10CM'   , ALLOC=associated(WAT10CMTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WATSOI'    , ALLOC=associated(WATSOITILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ICESOI'    , ALLOC=associated(ICESOITILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'EVLAND'    , ALLOC=associated(EVLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'PRLAND'    , ALLOC=associated(PRLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SNOLAND'   , ALLOC=associated(SNOLANDTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'DRPARLAND' , ALLOC=associated(DRPARLANDTILE ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'DFPARLAND' , ALLOC=associated(DFPARLANDTILE ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'LHSNOW'    , ALLOC=associated(LHSNOWTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SWNETSNOW' , ALLOC=associated(SWNETSNOWTILE ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'LWUPSNOW'  , ALLOC=associated(LWUPSNOWTILE  ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'LWDNSNOW'  , ALLOC=associated(LWDNSNOWTILE  ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TCSORIG'   , ALLOC=associated(TCSORIGTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TPSN1IN'   , ALLOC=associated(TPSN1INTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TPSN1OUT'  , ALLOC=associated(TPSN1OUTTILE  ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'GHSNOW'    , ALLOC=associated(GHSNOWTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'LHLAND'    , ALLOC=associated(LHLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SHLAND'    , ALLOC=associated(SHLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SWLAND'    , ALLOC=associated(SWLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SWDOWNLAND', ALLOC=associated(SWDOWNLANDTILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'LWLAND'    , ALLOC=associated(LWLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'GHLAND'    , ALLOC=associated(GHLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'GHTSKIN'   , ALLOC=associated(GHTSKINTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SMLAND'    , ALLOC=associated(SMLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'QINFIL'    , ALLOC=associated(QINFILTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TWLAND'    , ALLOC=associated(TWLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TELAND'    , ALLOC=associated(TELANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TSLAND'    , ALLOC=associated(TSLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'DWLAND'    , ALLOC=associated(DWLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'DHLAND'    , ALLOC=associated(DHLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SPLAND'    , ALLOC=associated(SPLANDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SPWATR'    , ALLOC=associated(SPWATRTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SPSNOW'    , ALLOC=associated(SPSNOWTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'FRACI'     , ALLOC=associated(FRTILE        ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RDU001'    , ALLOC=associated(RDU001TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RDU002'    , ALLOC=associated(RDU002TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RDU003'    , ALLOC=associated(RDU003TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RDU004'    , ALLOC=associated(RDU004TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RDU005'    , ALLOC=associated(RDU005TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RBC001'    , ALLOC=associated(RBC001TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RBC002'    , ALLOC=associated(RBC002TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ROC001'    , ALLOC=associated(ROC001TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ROC002'    , ALLOC=associated(ROC002TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RMELTDU001', ALLOC=associated(RMELTDU001TILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RMELTDU002', ALLOC=associated(RMELTDU002TILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RMELTDU003', ALLOC=associated(RMELTDU003TILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RMELTDU004', ALLOC=associated(RMELTDU004TILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RMELTDU005', ALLOC=associated(RMELTDU005TILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RMELTBC001', ALLOC=associated(RMELTBC001TILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RMELTBC002', ALLOC=associated(RMELTBC002TILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RMELTOC001', ALLOC=associated(RMELTOC001TILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RMELTOC002', ALLOC=associated(RMELTOC002TILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'PEATCLSM_WATERLEVEL', ALLOC=associated(PEATCLSM_WATERLEVELTILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'PEATCLSM_FSWCHANGE' , ALLOC=associated(PEATCLSM_FSWCHANGETILE ), notFoundOK=.true., __RC__)

      IF (LSM_CHOICE > 1) THEN
         call MAPL_GetPointer( GEX(type), dum, 'CNLAI'  , ALLOC=associated(CNLAITILE     ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNTLAI' , ALLOC=associated(CNTLAITILE    ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNSAI'  , ALLOC=associated(CNSAITILE     ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNTOTC' , ALLOC=associated(CNTOTCTILE    ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNVEGC' , ALLOC=associated(CNVEGCTILE    ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNROOT' , ALLOC=associated(CNROOTTILE    ), notFoundOK=.true., __RC__)
         if (LSM_CHOICE == 3) then
	    call MAPL_GetPointer( GEX(type), dum, 'CNFROOTC', ALLOC=associated(CNFROOTCTILE), notFoundOK=.true., __RC__)
         endif
         call MAPL_GetPointer( GEX(type), dum, 'CNNPP'  , ALLOC=associated(CNNPPTILE     ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNGPP'  , ALLOC=associated(CNGPPTILE     ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNSR'   , ALLOC=associated(CNSRTILE      ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNNEE'  , ALLOC=associated(CNNEETILE     ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNXSMR' , ALLOC=associated(CNXSMRTILE    ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNADD'  , ALLOC=associated(CNADDTILE     ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNLOSS' , ALLOC=associated(CNLOSSTILE    ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNBURN' , ALLOC=associated(CNBURNTILE    ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'PARABS' , ALLOC=associated(PARABSTILE    ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'PARINC' , ALLOC=associated(PARINCTILE    ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'SCSAT'  , ALLOC=associated(SCSATTILE     ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'SCUNS'  , ALLOC=associated(SCUNSTILE     ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'BTRANT' , ALLOC=associated(BTRANTTILE    ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'SIF'    , ALLOC=associated(SIFTILE       ), notFoundOK=.true., __RC__)
         call MAPL_GetPointer( GEX(type), dum, 'CNFSEL' , ALLOC=associated(CNFSELTILE    ), notFoundOK=.true., __RC__)
      END IF

      call MAPL_GetPointer( GEX(type), dum, 'HLATWTR'   , ALLOC=associated(HLATWTRTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'HLATICE'   , ALLOC=associated(HLATICETILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SHWTR'     , ALLOC=associated(SHWTRTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SHICE'     , ALLOC=associated(SHICETILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TAUXW'     , ALLOC=associated(TAUXWTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TAUXI'     , ALLOC=associated(TAUXITILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TAUYW'     , ALLOC=associated(TAUYWTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TAUYI'     , ALLOC=associated(TAUYITILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'LWNDWTR'   , ALLOC=associated(LWNDWTRTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SWNDWTR'   , ALLOC=associated(SWNDWTRTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'LWNDICE'   , ALLOC=associated(LWNDICETILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SWNDICE'   , ALLOC=associated(SWNDICETILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'RAINOCN'   , ALLOC=associated(RAINOCNTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SNOWOCN'   , ALLOC=associated(SNOWOCNTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TSKINW'    , ALLOC=associated(TSKINWTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TSKINICE'  , ALLOC=associated(TSKINICETILE  ), notFoundOK=.true., __RC__)

      call MAPL_GetPointer( GEX(type), dum, 'DCOOL'     , ALLOC=associated(DCOOL_TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'DWARM'     , ALLOC=associated(DWARM_TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TDROP'     , ALLOC=associated(TDROP_TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'QCOOL'     , ALLOC=associated(QCOOL_TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SWCOOL'    , ALLOC=associated(SWCOOL_TILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'USTARW'    , ALLOC=associated(USTARW_TILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TBAR'      , ALLOC=associated(TBAR_TILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'LCOOL'     , ALLOC=associated(LCOOL_TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'BCOOL'     , ALLOC=associated(BCOOL_TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TDEL'      , ALLOC=associated(TDEL_TILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TS_FOUND'  , ALLOC=associated(TS_FOUND_TILE ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'QWARM'     , ALLOC=associated(QWARM_TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SWWARM'    , ALLOC=associated(SWWARM_TILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'LANGM'     , ALLOC=associated(LANGM_TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'PHIW'      , ALLOC=associated(PHIW_TILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TAUTW'     , ALLOC=associated(TAUTW_TILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ZETA_W'    , ALLOC=associated(ZETA_W_TILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TWMTF'     , ALLOC=associated(TWMTF_TILE    ), notFoundOK=.true., __RC__)

      call MAPL_GetPointer( GEX(type), dum, 'HICE'      , ALLOC=associated(HICETILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'HSNO'      , ALLOC=associated(HSNOTILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'FRZMLT'    , ALLOC=associated(FRZMLTTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'TSKINWCICE', ALLOC=associated(TSKINWCICETILE), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ISTSFC'    , ALLOC=associated(ISTSFCTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SSKINW'    , ALLOC=associated(SSKINWTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'MELTT'     , ALLOC=associated(MELTTTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'MELTB'     , ALLOC=associated(MELTBTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'MELTS'     , ALLOC=associated(MELTSTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'MELTL'     , ALLOC=associated(MELTLTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'FRAZIL'    , ALLOC=associated(FRAZILTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'CONGEL'    , ALLOC=associated(CONGELTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SNOICE'    , ALLOC=associated(SNOICETILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'DAIDTT'    , ALLOC=associated(DAIDTTTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'DVIDTT'    , ALLOC=associated(DVIDTTTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'DAIDTD'    , ALLOC=associated(DAIDTDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'DVIDTD'    , ALLOC=associated(DVIDTDTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'FBOT'      , ALLOC=associated(FBOTTILE      ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'HFLUX'     , ALLOC=associated(HFLUXTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'WATERFLUX' , ALLOC=associated(WFLUXTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SALTFLUX'  , ALLOC=associated(SFLUXTILE     ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'FSWTHRU '  , ALLOC=associated(FSWTHRUTILE   ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'FSWABS'    , ALLOC=associated(FSWABSTILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'USTARI'    , ALLOC=associated(USTARITILE    ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'FHOCN'     , ALLOC=associated(FHOCNTILE     ), notFoundOK=.true., __RC__)

! PMN: temporary ... at moment only catch produces
      call MAPL_GetPointer( GEX(type), dum, 'ALBVR_REF' , ALLOC=associated(ALBVRTILE_REF ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ALBVF_REF' , ALLOC=associated(ALBVFTILE_REF ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ALBNR_REF' , ALLOC=associated(ALBNRTILE_REF ), notFoundOK=.true., __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ALBNF_REF' , ALLOC=associated(ALBNFTILE_REF ), notFoundOK=.true., __RC__)

! All children can produce these                       
                                                                                       
      call MAPL_GetPointer( GEX(type), dum, 'DELTS'     , ALLOC=associated(DTSTILE       ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'DELQS'     , ALLOC=associated(DQSTILE       ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'HLATN'     , ALLOC=associated(HLATNTILE     ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'EVAPOUT'   , ALLOC=associated(EVAPOUTILE    ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SUBLIM'    , ALLOC=associated(SUBLIMTILE    ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SHOUT'     , ALLOC=associated(SHOUTILE      ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'HLWUP'     , ALLOC=associated(HLWUPTILE     ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'LWNDSRF'   , ALLOC=associated(LWNDSRFTILE   ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'SWNDSRF'   , ALLOC=associated(SWNDSRFTILE   ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ALBVR'     , ALLOC=associated(ALBVRTILE     ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ALBVF'     , ALLOC=associated(ALBVFTILE     ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ALBNR'     , ALLOC=associated(ALBNRTILE     ), __RC__)
      call MAPL_GetPointer( GEX(type), dum, 'ALBNF'     , ALLOC=associated(ALBNFTILE     ), __RC__)

! PMN: uncomment when all children can produce ... at moment on catch does
...
!     call MAPL_GetPointer( GEX(type), dum, 'ALBNF_REF' , ALLOC=associated(ALBNFTILE_REF ), __RC__)
      ...
      call MAPL_GetPointer( GEX(type), dum, 'EMIS'      , ALLOC=associated(EMISSTILE     ), __RC__)
