...
contains
   ...
   subroutine  Write_CatchParams(nc, nr, GridName, DL, LBSV, MaskFile, rc )
      integer, intent(in) :: nc
      integer, intent(in) :: nr
      character(len=*),  intent(in) :: GridName
      character(len=*),  intent(in) :: DL
      character(len=*),  intent(in) :: LBSV
...
!$OMP ENDPARALLEL
      ...
      ease_grid = .false.
      if (index(GridName, '_EASE') /=0) ease_grid = .true.
      if (present(rc)) rc = 0
      ...
      if(n_threads == 1) then
         ...
         if(index(Gridname,'CF')/=0) then 
            _ASSERT(DL == 'DE', 'Cube-Sphere Grid - assuming dateline-on-edge (DE)')
         ...
         ! WY Note: If above statement is important, we should check ./clsm/ here
         !          I guess it saves time when this subroutines are called many times
   ...
   end subroutine write_catchparams

end module write_CatchParamsMod
