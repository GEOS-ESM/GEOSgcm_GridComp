...
lat_up = np.loadtxt("temp/lat_upper_M09.txt", dtype=float)
lon_left = np.loadtxt("temp/lon_left_M09.txt", dtype=float)
lon_right = np.loadtxt("temp/lon_right_M09.txt", dtype=float)
...
np.savetxt("temp/loni_tile_M09.txt", loni + 1, fmt='%d')
...
if os.path.exists("temp/map_tile_M09.nc"):
    os.remove("temp/map_tile_M09.nc")

# Create a NetCDF file and write the data
with Dataset("temp/map_tile_M09.nc", "w", format="NETCDF4") as fout:
    # Create dimensions
