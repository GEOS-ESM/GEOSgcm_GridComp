program main
  ...
  integer,parameter :: nc = 291284, nupmax=34
  ...
  character(len=500) :: file_path="input/Pfafcatch-routing.dat" !/discover/nobackup/projects/gmao/bcs_shared/make_bcs_inputs/land/topo/v1/SRTM-TopoData/Pfafcatch-routing.dat
...
! first= The index of the last zero (but not the zero at the very end).  However, if there are no zeroes until the end, first =2 (the second index, since the first index indicates the continent).
  ...
  allocate(downid(nc),finalid(nc),pfaf(nc),pfaf_digit(nc,12),res(nc),pfaf_last(nc),pfaf_msk(nc),pfaf_area(nc))
