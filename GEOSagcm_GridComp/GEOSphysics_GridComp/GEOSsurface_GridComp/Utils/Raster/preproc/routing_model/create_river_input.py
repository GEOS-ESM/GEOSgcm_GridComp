...
# ---------- Catchment datasets ----------
Kstr_catchment = read_ascii("output/Pfaf_Kstr_PR_fac1_0p35_0p45_0p2_n0p2.txt", nc)
Kv_catchment = read_ascii("output/Pfaf_Kv_PR_0p35_0p45_0p2_n0p2.txt", nc)
area_catchment = read_ascii("output/Pfaf_area.txt", nc)
lriv_catchment = read_ascii("output/Pfaf_lriv_PR.txt", nc)
lstr_catchment = read_ascii("output/Pfaf_lstr_PR.txt", nc)
Qin_catchment = read_ascii("output/Pfaf_qin.txt", nc)
Qri_catchment = read_ascii("output/Pfaf_qri.txt", nc)
Qstr_catchment = read_ascii("output/Pfaf_qstr.txt", nc)
tosink_catchment = read_ascii("output/Pfaf_tosink.txt", nc, dtype=int)
dnid_catchment = read_ascii("output/downstream_1D_new_noadj.txt", nc, dtype=int)

# ---------- Reservoir datasets ----------
area_reservoir = read_ascii("output/area_skm_grand.txt", nr)
capmax_reservoir = read_ascii("output/cap_max_grand.txt", nr)
catid_reservoir = read_ascii("output/catid_dam_corr_aca_grand5000.txt", nr, dtype=int)
flag_reservoir = read_ascii("output/flag_all_res.txt", nr, dtype=int)
fldmainsec_reservoir = read_ascii("output/fldmainsec_grand.txt", nr, dtype=int)
elec_reservoir = read_ascii("output/hydroelec_grand.txt", nr, dtype=int)
irr_reservoir = read_ascii("output/irr_grand.txt", nr, dtype=int)
nav_reservoir = read_ascii("output/nav_grand.txt", nr, dtype=int)
other_reservoir = read_ascii("output/other_grand.txt", nr, dtype=int)
rec_reservoir = read_ascii("output/rec_grand.txt", nr, dtype=int)
supply_reservoir = read_ascii("output/watersupply_grand.txt", nr, dtype=int)

# ---------- Lake datasets ----------
catid_lake = read_ascii("output/lake_outlet_catid.txt", nl, dtype=int)
flag_lake = read_ascii("output/lake_outlet_flag_valid_2097.txt", nl, dtype=int)
area_lake = read_ascii("output/lake_outlet_lakearea.txt", nl)
