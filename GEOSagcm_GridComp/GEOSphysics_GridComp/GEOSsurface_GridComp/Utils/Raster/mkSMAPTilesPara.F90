...
!
      ...
      real*8, allocatable, dimension (:) ::  SRTM_catid_r8
      ...
      character*1            :: PF
      character(2)           :: ease_version      

      N_args = iargc()

      if(N_args < 1) then
        print *,'USAGE : bin/mkSMAPTiles -smap_grid MXX -v LBSV -EASE_VERSION v1(v2)'
      ...
      do while ( i < N_args )
         ...
         elseif ( trim(arg) == '-EASE_VERSION' ) then
            i = i+1
            call getarg(i,ease_version)
                        
         else ! stop for any other arguments
            
            print *,'USAGE : bin/mkSMAPTiles -smap_grid MXX -pfaf_til T -v LBSV -EASE_VERSION v1(v2)'
            print *,'Allowed SMAP grids are: M01 M03 M09 M25 M36 Ml'
            stop
      ...
      ! WY noted: shuould do it in the script that calls this script
      ...
      if (index(MaskFile,'GEOS5_10arcsec_mask') /= 0) then         
         ...
         allocate(SRTM_catid_r8  (1:SRTM_maxcat+2),source = 0.0d0)
         ...
         !NF_GET_VARA_INT64 (ncid,3,(/1/),(/SRTM_maxcat/),SRTM_catid(1:SRTM_maxcat))  ! Read pfafstetter IDs
         if(status /=0) then
      ...
      SUBROUTINE mkEASEv2Raster
        ...
        do  j = 1, nr_smap+1
           do i = 1, nc_smap+1
              ...
              call ease_inverse(EASElabel, x, y, yout, xout)
              ys (i,j) = dble(yout)
              xs (i,j) = dble(xout)
           end do
        end do
