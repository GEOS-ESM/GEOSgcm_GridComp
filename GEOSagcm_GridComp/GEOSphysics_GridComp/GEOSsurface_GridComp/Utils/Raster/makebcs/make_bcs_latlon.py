...
if( {LATLON_OCEAN} == True ) then
    ...
    if ( {SKIPLAND} != True) then
       ...
       bin/mkCatchParam.x -x {NX} -y {NY} -g DC{IM}xPC{JM}_DE{IMO}xPE{JMO}-Pfafstetter -v {lbcsv}
       setenv OMP_NUM_THREADS 1
    else
       bin/TileFile_ASCII_to_nc4.x {NX} {NY} DC{IM}xPC{JM}_DE{IMO}xPE{JMO}-Pfafstetter write_catch=.false.
    endif
...
if( {TRIPOL_OCEAN} == True ) then
   bin/mkMOMAquaRaster.x -x {NX} -y {NY}  data/{MOM_VERSION}/{imo}x{jmo}/MAPL_Tripolar.nc > /dev/null
    ...
    if ( {SKIPLAND} != True ) then
       ...
       bin/TileFile_ASCII_to_nc4.x {NX} {NY} DE{IMO}xPE{JMO}_DE{IMO}xPE{JMO}-Pfafstetter write_catch=.false.
    endif
    chmod 755 bin/create_README.csh
    bin/create_README.csh    
endif
