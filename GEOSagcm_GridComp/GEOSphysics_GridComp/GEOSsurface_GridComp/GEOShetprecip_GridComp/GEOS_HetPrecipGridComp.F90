...
!@@    _ASSERT(IM==1 .and. JM == 1, 'Only single column supported')
    ...
    allocate(norm_tile_area(NT), __STAT__)
    norm_tile_area = tile_area
...
! verification!
  ...
  contains
    subroutine normalizeTileArea(norm_tile_area, index)
      real, intent(inout)  :: norm_tile_area(:)
      ...
      totalArea = sum(norm_tile_area(index))

      do i=1,size(index)
         j = index(i)
         norm_tile_area(j) = norm_tile_area(j) / totalArea
      end do
