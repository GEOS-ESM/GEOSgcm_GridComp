...
!    _ASSERT(IM==1 .and. JM == 1, 'Only single column supported')
    ...
    call MAPL_GetResource(MAPL, CORR_TSCALE, Label="HETPR_CORR_TSCALE:", __RC__)
...
!ALT: This is direct port from Randy's idl code
    ...
    pdfv=[.121,.128,.137,.139,.119, .081,.059,.041,.030,.025, &
          ...
          .0008,.0006,.0004,.0002,0.0] ! probability density
    pdfv=pdfv/sum(pdfv) ! scale to ensure it sums to unity
    cdfv(0)=0. !pdfv(0)
    do i=1,NPDF
       cdfv(i)=cdfv(i-1)+pdfv(i-1) ! construct CDF
    end do
...
!             if(qfrac < cdfv(i)) cycle
          do i=0,NPDF-1
            if(qfrac < cdfv(i) .or. qfrac>cdfv(i+1)) cycle
             ...
             if(iopt < NPDF) then
                ...
                if (psub(itile).le.0.) print *,'Line 327: psub(itile)=0'
             else
                !ALT: this logic need to be double checked
                wtpdf=pdfw(iopt)
                psub(itile)=wtpdf*(1.-fracdry)
                if (psub(itile).le.0.) print *,'Line 332: psub(itile)=0'
             endif
             if (psub(itile).lt.0.) then
               print *,'Stopping!  Negative value of psub!'
               print *,'psu(),fracdry,psub(itile) = ',psum(itile),fracdry,psub(itile)
               print *,'wtx,wtpdf,iopt = ',wtx,wtpdf,iopt
               stop
             end if
          end do
       end if
