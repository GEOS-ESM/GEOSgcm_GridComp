...
! real,   parameter :: SURFLAY = 20.  ! moved to GetResource in RUN2  LLT:12Jul2013
...
! if is_offline, some variables ( in the last) are not required
  ...
  call MAPL_AddInternalSpec(GC                  ,&
    LONG_NAME          = 'canopy_temperature_nature'        ,&
    UNITS              = 'K'                         ,&
    SHORT_NAME         = 'TC_NA'                        ,&
    FRIENDLYTO         = trim(COMP_NAME)             ,&
    DIMS               = MAPL_DimsTileOnly           ,&
  ...
  call MAPL_AddInternalSpec(GC                  ,&
    LONG_NAME          = 'canopy_specific_humidity_nature'  ,&
    UNITS              = 'kg kg-1'                   ,&
    SHORT_NAME         = 'QC_NA'                        ,&
    FRIENDLYTO         = trim(COMP_NAME)             ,&
    DIMS               = MAPL_DimsTileOnly           ,&
...
!        data VGRDB / 5.1 ,  7.2, 10.8,  4.8,  1.8,  5.1,  .000, .000/
        ...
        call MAPL_GetPointer(INTERNAL,UHI         ,'UHI'         ,RC=STATUS); VERIFY_(STATUS)        
        ...
        DZSF_in_mm(:) = CATCH_INTERNAL_STATE%SURFLAY       ! same as DZSF but in units of [mm]
              ...
              if(ESMF_AlarmIsRinging(LDAS_ALARM, RC=STATUS))then
                 ...
                 if (fexist) then
                    ...
                    call apply_catch_incr(NTILES,                                                        &
                         DZSF_in_mm, VGWMAX, CDCR1, CDCR2, PSIS, BEE, POROS, WPWET,                      &
...
subroutine RUN0(gc, import, export, clock, rc)
  ...
  VERIFY_(status)
  DZSF_in_mm = CATCH_INTERNAL_STATE%SURFLAY
