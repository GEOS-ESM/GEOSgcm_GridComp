...
! --------
    ...
    call MAPL_GetResource ( MAPL, CATCHCN_VERSION, Label="CATCHCN_VERSION:", DEFAULT="CLM40", RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetResource ( MAPL, SURFRC, label = 'SURFRC:', default = 'GEOS_SurfaceGridComp.rc', RC=STATUS) ; VERIFY_(STATUS)
    SCF = ESMF_ConfigCreate(rc=status) ; VERIFY_(STATUS)
    call ESMF_ConfigLoadFile(SCF,SURFRC,rc=status) ; VERIFY_(STATUS)
    call ESMF_ConfigGetAttribute (SCF, label='RUN_IRRIG:', value=RUN_IRRIG, DEFAULT=0, RC=STATUS); VERIFY_(STATUS)
    call ESMF_ConfigGetAttribute (SCF, label='ATM_CO2:' , value=ATM_CO2,  DEFAULT=2, RC=STATUS); VERIFY_(STATUS)

    CATCHCN = -9999

    if (trim(CATCHCN_VERSION) == 'CLM40') then
       CATCHCN = MAPL_AddChild(GC, NAME='CATCHCN', SS = CLM40_SetServices, RC=STATUS)
       VERIFY_(STATUS)       
    endif
    if (trim(CATCHCN_VERSION) == 'CLM45') then
       CATCHCN = MAPL_AddChild(GC, NAME='CATCHCN', SS = CLM45_SetServices, RC=STATUS)
       VERIFY_(STATUS)       
    endif

    _ASSERT(CATCHCN /= -9999, "should specify CATCHCN_VERSION: CLM40 or CLM45")
