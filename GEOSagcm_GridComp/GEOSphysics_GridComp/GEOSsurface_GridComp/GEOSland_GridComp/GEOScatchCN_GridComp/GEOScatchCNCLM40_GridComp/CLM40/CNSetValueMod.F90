...
!-----------------------------------------------------------------------
    ...
    call CNSetCcf(num_filterc, filterc, 0., clm3%g%l%c%ccf)
    call CNSetCnf(num_filterc, filterc, 0., clm3%g%l%c%cnf)

    ! zero the column-average pft-level C and N fluxes
    call CNSetPcf(num_filterc, filterc, 0., clm3%g%l%c%ccf%pcf_a)
    call CNSetPnf(num_filterc, filterc, 0., clm3%g%l%c%cnf%pnf_a)

    ! zero the pft-level C and N fluxes
    call CNSetPcf(num_filterp, filterp, 0., clm3%g%l%c%p%pcf)
    call CNSetPnf(num_filterp, filterp, 0., clm3%g%l%c%p%pnf)
...
!-----------------------------------------------------------------------
    ...
    do c = begc,endc
       ! C fluxes
       cptr%ccf%dwt_seedc_to_leaf(c) = 0.
       cptr%ccf%dwt_seedc_to_deadstem(c) = 0.
       cptr%ccf%dwt_conv_cflux(c) = 0.
       cptr%ccf%dwt_prod10c_gain(c) = 0.
       cptr%ccf%dwt_prod100c_gain(c) = 0.
       cptr%ccf%dwt_frootc_to_litr1c(c) = 0.
       cptr%ccf%dwt_frootc_to_litr2c(c) = 0.
       cptr%ccf%dwt_frootc_to_litr3c(c) = 0.
       cptr%ccf%dwt_livecrootc_to_cwdc(c) = 0.
       cptr%ccf%dwt_deadcrootc_to_cwdc(c) = 0.
       ! N fluxes
       cptr%cnf%dwt_seedn_to_leaf(c) = 0.
       cptr%cnf%dwt_seedn_to_deadstem(c) = 0.
       cptr%cnf%dwt_conv_nflux(c) = 0.
       cptr%cnf%dwt_prod10n_gain(c) = 0.
       cptr%cnf%dwt_prod100n_gain(c) = 0.
       cptr%cnf%dwt_frootn_to_litr1n(c) = 0.
       cptr%cnf%dwt_frootn_to_litr2n(c) = 0.
       cptr%cnf%dwt_frootn_to_litr3n(c) = 0.
       cptr%cnf%dwt_livecrootn_to_cwdn(c) = 0.
       cptr%cnf%dwt_deadcrootn_to_cwdn(c) = 0.
    end do
    do p = begp,endp
       cptr%p%pcs%dispvegc(p)   = 0.
       cptr%p%pcs%storvegc(p)   = 0.
       cptr%p%pcs%totpftc(p)    = 0.
       cptr%p%pns%dispvegn(p)   = 0.
       cptr%p%pns%storvegn(p)   = 0.
       cptr%p%pns%totvegn(p)    = 0.
       cptr%p%pns%totpftn(p)    = 0.
    end do
