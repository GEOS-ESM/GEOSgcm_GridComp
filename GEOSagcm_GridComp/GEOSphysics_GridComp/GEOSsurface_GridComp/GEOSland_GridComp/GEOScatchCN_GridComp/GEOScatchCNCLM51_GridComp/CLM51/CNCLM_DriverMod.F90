...
contains
  ...
  subroutine CN_exit(nch,ityp,fveg,cncol,cnpft)
    ...
    ! map CTSM structure variables into CatchCN restart arrays (CNCOL and CNPFT) 
          ...
          do p = 0,numpft      ! PFT index loop
             ...
             do nv = 1,num_veg ! defined veg loop
                ...
                if(ityp(nc,nv,nz)==p .and. fveg(nc,nv,nz)>1.e-4) then
                   ...
                   cnpft(nc,nz,nv, 28) = grc%dayl(                                                             nc  ) ! jkolassa Dec 2023: dayl is a gridcell-level variable in CLM, but is stored as patch-level variable in CatchCN restart
