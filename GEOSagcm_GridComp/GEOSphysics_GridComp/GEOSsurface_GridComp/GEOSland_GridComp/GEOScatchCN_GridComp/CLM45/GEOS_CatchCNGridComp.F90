...
! -------------------------------------------------------------
   if(first) then
     if ((PRESCRIBE_DVG == 0) .OR.(PRESCRIBE_DVG == 3)) then 
        ...
        call CN_init(istep_cn,nt,nveg,nzone,ityp,fveg,var_col,var_pft)  
     endif
     first = .false.
   endif
...
! ------------------------------
    ...
    real, allocatable, dimension(:) :: npp, gpp, sr, nee, padd, frootc, vegc, xsmr, fsnow
    real, allocatable, dimension(:) :: burn, closs, nfire, som_closs
...
! solar declination related
    ...
    logical, save :: first_fpar = .true.
        ...
        ! Get file name from configuration, following GEOS_VegdynGridComp.F90
    ...
    DO_FS1 : if  (SCALE_ALBFPAR >= 2) then
        ...
        IF (ntiles > 0) THEN
           ...
           do nz = 1,num_zon
              ...
              call compute_rc(NTILES,nveg,TCx,QAx,T2M10D,                &
                   TA, PS, ZTH,DRPAR,DFPAR,albdir,albdif,                &
...
! -----------------------------------------------
    ...
    RUN_CLM : IF(PRESCRIBE_DVG == 0) THEN
       ...
       if(mod(AGCM_S_ofday,nint(dtcn)) == 0) then
          ...
          if(associated(CNLOSS             )) cnloss              = 1.e-3*closs             * cnsum ! veg fire C loss (kg/m2/s)
          ...
          if(associated(CNFROOTC           )) cnfrootc            = 0.
...
subroutine RUN0(gc, import, export, clock, rc)
  ...
  allocate(   tlai(ntiles,num_veg,num_zon) )
...
! --------------------------------------------------
   ...
   IF(PRESCRIBE_DVG == 0) THEN
      ! obtain LAI from previous time step (from CN model)
      ! --------------------------------------------------
      call get_CN_LAI(ntiles,num_veg,num_zon,ityp,fveg,elai,esai=esai,tlai=tlai)
   ELSE
      call  read_prescribed_LAI (INTERNAL, CLOCK, GC, NTILES, elai,esai)
   ENDIF
