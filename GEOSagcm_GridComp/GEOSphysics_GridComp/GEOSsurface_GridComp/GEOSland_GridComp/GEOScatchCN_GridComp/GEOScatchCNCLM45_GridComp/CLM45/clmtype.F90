...
type, public :: pft_epc_type
   ...
   real(r8), pointer :: rhol(:)                 !leaf reflectance: 1=vis, 2=nir   (numrad). fzeng: only vis
   real(r8), pointer :: rhos(:)                 !stem reflectance: 1=vis, 2=nir   (numrad). fzeng: only vis
   real(r8), pointer :: taul(:)                 !leaf transmittance: 1=vis, 2=nir (numrad). fzeng: only vis
   real(r8), pointer :: taus(:)                 !stem transmittance: 1=vis, 2=nir (numrad). fzeng: only vis
...
!  fire variables added by F. Li and S. Levis
  ...
  ! fzeng:
   ...
   ! fzeng:
   integer,  pointer :: mnNHplantdate(:)    ! Minimum planting date for the Northern Hemipsphere (MMDD)
   integer,  pointer :: mxNHplantdate(:)    ! Maximum planting date for the Northern Hemipsphere (MMDD)
   integer,  pointer :: mnSHplantdate(:)    ! Minimum planting date for the Southern Hemipsphere (MMDD)
   integer,  pointer :: mxSHplantdate(:)    ! Maximum planting date for the Southern Hemipsphere (MMDD)
   real(r8), pointer :: gddmin(:)       ! Minimim growing degree days used in CNPhenology 
   real(r8), pointer :: hybgdd(:)       ! Growing Degree Days for maturity used in CNPhenology (unitless)
   real(r8), pointer :: lfemerg(:)      ! Leaf emergence parameter used in CNPhenology (unitless)
   real(r8), pointer :: grnfill(:)      ! Grain fill parameter used in CNPhenology (unitless)
   real(r8), pointer :: mxmat(:)        ! Maximum number of days to maturity parameter in CNPhenology (days)
   real(r8), pointer :: minplanttemp(:) ! Average 5 day daily minimum temperature needed for planting (K)
   real(r8), pointer :: planttemp(:)    ! Average 10 day temperature needed for planting (K) 
   
   ! new variables for subroutine CNVegStructUpdate
   real(r8), pointer :: laimx(:)        ! Maximum Leaf Area Index used in CNVegStructUpdate  
   real(r8), pointer :: ztopmx(:)       ! Canopy top coefficient used in CNVegStructUpdate (m)

   ! fzeng: for CNAllocation   
   real(r8), pointer :: declfact(:)     ! Decline factor for gddmaturity used in CNAllocation
   real(r8), pointer :: bfact(:)        ! Exponential factor used in CNAllocation for fraction allocated to leaf
   real(r8), pointer :: aleaff(:)       ! Leaf Allocation coefficient parameter used in CNAllocation
   real(r8), pointer :: arootf(:)       ! Root Allocation coefficient parameter used in CNAllocation
   real(r8), pointer :: astemf(:)       ! Stem Allocation coefficient parameter used in CNAllocation
   real(r8), pointer :: arooti(:)       ! Root Allocation coefficient parameter used in CNAllocation
   real(r8), pointer :: fleafi(:)       ! Leaf Allocation coefficient parameter fraction used in CNAllocation
   real(r8), pointer :: allconsl(:)     ! Leaf Allocation coefficient parameter power used in CNAllocation
   real(r8), pointer :: allconss(:)     ! Stem Allocation coefficient parameter power used in CNAllocation
   
   ! fzeng: 
   real(r8), pointer :: grperc(:)       ! Growth respiration factor (unitless)
   real(r8), pointer :: grpnow(:)       ! Growth respiration factor (unitless)
   real(r8), pointer :: mxtmp(:)        ! Max Temperature, parameter used in CN_Driver (degree C)
   real(r8), pointer :: baset(:)        ! Base Temperature, parameter used in CN_Driver (degree C)
...
type, public :: column_pstate_type
   ...
   real(r8), pointer :: psisat(:,:)           !soil water potential at saturation for CN code (MPa), added by fzeng
   real(r8), pointer :: psiwilt(:)            !root-zone soil water potential at wilting point (MPa), added by fzeng
...
type, public :: column_cstate_type
   ...
   real(r8), pointer :: cpool_col(:)     !column-level temporary photosynthate C pool (gC/m2) 
