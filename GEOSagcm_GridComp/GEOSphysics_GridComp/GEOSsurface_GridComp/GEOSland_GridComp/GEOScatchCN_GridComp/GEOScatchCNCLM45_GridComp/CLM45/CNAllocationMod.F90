...
! !USES:
   ...
   use clm_varctl      , only: crop_prog,iulog
   use nanMod          , only: nan
! !ARGUMENTS:
   implicit none
...
!-----------------------------------------------------------------------
   ...
   select case(suplnitro)
      ...
      case default
         ...
         stop 'CNAllocationInit ERROR: supplemental Nitrogen flag is not correct' 
   end select
...
! !USES:
   ...
   use pftvarcon , only: npcropmin, nsoybean 
   use clm_varcon, only: secspday, istcrop
   use clm_time_manager , only : get_step_size
   use nanMod          , only: nan
!
! !ARGUMENTS:
...
!  endif
   ...
   declfact                    => pftcon%declfact
   bfact                       => pftcon%bfact   
   aleaff                      => pftcon%aleaff  
   arootf                      => pftcon%arootf  
   astemf                      => pftcon%astemf  
   arooti                      => pftcon%arooti  
   fleafi                      => pftcon%fleafi  
   allconsl                    => pftcon%allconsl
   allconss                    => pftcon%allconss
