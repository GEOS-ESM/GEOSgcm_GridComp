...
!EOP
    ...
    logical,  pointer :: pactive(:)    ! true=>do computations on this pft (see reweightMod for details), fzeng adopted a modified/simpler way to set pactive in CN_Driver
...
!------------------------------------------------------------------------
    ...
    sumwt(lbc:ubc) = 0.
    do p = lbp,ubp
       if (pactive(p) .and. wtcol(p) /= 0.) then
