...
!
! !INTERFACE:
subroutine NStateUpdate1(num_soilc, filter_soilc, num_soilp, filter_soilp, tileid_soilp)
!
! !DESCRIPTION:
...
! !ARGUMENTS:
   ...
   integer, intent(in) :: tileid_soilp(:) ! tile index for debugging
!
! !CALLED FROM:
...
!       print *, 'tileid_soilp(p),ivt(p),livestemn(p),livestemn_xfer_to_livestemn(p)',tileid_soilp(p),ivt(p),livestemn(p),livestemn_xfer_to_livestemn(p)
