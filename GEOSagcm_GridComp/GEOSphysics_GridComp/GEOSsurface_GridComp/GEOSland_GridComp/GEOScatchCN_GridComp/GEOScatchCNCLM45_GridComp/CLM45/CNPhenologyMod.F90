...
! !CALLED FROM:
! subroutine CNEcosystemDynInit in CNEcosystemDynMod.F90
!
! !REVISION HISTORY:
...
!   crit_onset_fdd = 15.0_r8
    crit_onset_fdd = 7.0 ! this may prevent "January thaw" growth spurt, fzeng, 22 Mar 2017
...
! !OTHER LOCAL VARIABLES:
   ...
   logical, parameter :: init_gdd20 = .false.    ! Always set to .FALSE.!! Will spin up and discard at least 2 years anyways. fzeng, July 2017 
...
!-----------------------------------------------------------------------
   ...
   if ( num_pcropp > 0 )then
      ...
      if ( nyrs == -999 ) then
         ! fzeng: this is done in such way to exclude restyear (i.e. nyrs here) in the restart file, Apr 2017
!         nyrs = CropRestYear()
         if(init_gdd20) then
            nyrs = 0       
          else
         ...
         if (kmo == 1 .and. kda == 1 .and. mcsec == 0) nyrs = nyrs + 1 ! call CropRestIncYear( nyrs )  ! fzeng: temporary fix to make it compile
      end if
   end if
...
!  decl                          => cps%decl
   ...
   do fp = 1,num_soilp
      ...
      if (season_decid(ivt(p)) == 1._r8) then
         ...
         ! Calculation of dayl and prev_dayl moved to CN_DriverMod.F90, fzeng, 22 Mar 2017
!        prev_dayl(p) = dayl(p)
...
!  real(r8), pointer :: decl(:)               ! solar declination (radians)
   ...
   real(r8), pointer :: t_soisno(:,:)         ! soil temperature (Kelvin)  (-nlevsno+1:nlevgrnd)
   real(r8), pointer :: soilpsi(:,:)          ! soil water potential in each soil layer (MPa)
   real(r8), pointer :: psiwilt(:)            ! root-zone soil water potential at wilting point (MPa)
   real(r8), pointer :: leaf_long(:)          ! leaf longevity (yrs)
...
!        psi = soilpsi(c,3)
         ...
         soilpsi_on  = psiwilt(c) ! use catchment wilting point, fzeng, 22 Mar 2017
         soilpsi_off = psiwilt(c) ! use catchment wilting point, fzeng, 22 Mar 2017

         ! use solar declination information stored during Surface Albedo()
         ! and latitude from gps to calcluate daylength (convert latitude from degrees to radians)
         ! the constant 13750.9871 is the number of seconds per radian of hour-angle

         ! Calculation of dayl moved to CN_DriverMod.F90, fzeng, 22 Mar 2017
!        lat = (SHR_CONST_PI/180._r8)*grc%latdeg(pgridcell(p))
!        temp = -(sin(lat)*sin(decl(c)))/(cos(lat) * cos(decl(c)))
!        temp = min(1._r8,max(-1._r8,temp))
!        dayl(p) = 2.0_r8 * 13750.9871_r8 * acos(temp)
         ...
         if (dormant_flag(p) == 1._r8) then
            ...
            if (ivt(p)==14 .or. ivt(p)==18 .or. ivt(p)==10 .or. ivt(p)==16) then ! fzeng: special case; seasonal deciduous
...
! before winter solstice, prevent growth onset
         ...
         ! test for switching from growth period to offset period
            ...
            ! only begin to test for offset daylength once past the summer sol
            if( ivt(p)==14 .or. ivt(p)==18 .or. ivt(p)==10 .or. ivt(p)==16) then ! fzeng: special case
...
! !USES:
  ...
  use pftvarcon       , only : ncorn, nscereal, nwcereal, nsoybean,  &
...
! local pointers to implicit in scalars
      ...
      real(r8), pointer :: hybgdd(:)         ! Growing Degree Days for maturity used in CNPhenology
...
!------------------------------------------------------------------------
      ...
      gddmin         => pftcon%gddmin      
      hybgdd         => pftcon%hybgdd      
      lfemerg        => pftcon%lfemerg     
      grnfill        => pftcon%grnfill     
      mxmat          => pftcon%mxmat       
      minplanttemp   => pftcon%minplanttemp
      planttemp      => pftcon%planttemp   
