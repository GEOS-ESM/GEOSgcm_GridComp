...
contains
  ...
  subroutine RUN1 ( GC, IMPORT, EXPORT, CLOCK, RC )
    ...
    do N=1,NUM_SUBTILES
       ...
       if(.true.)             CHH    = CHH    + CH(:,N)*FR(:,N)
       if(.true.)             CQQ    = CQQ    + CQ(:,N)*FR(:,N)
  ...
  contains
    ...
    subroutine LAKECORE(NT,RC)
      ...
      real, pointer, dimension(:)    :: PS     
      real, pointer, dimension(:)    :: TA       ! needed for offline
      real, pointer, dimension(:)    :: QA       ! needed for offline
