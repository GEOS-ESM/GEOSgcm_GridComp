...
!replaced with  parameter statement
  !!use shr_kind_mod,   only: GW_PRC => shr_kind_GW_PRC
...
!
  ...
  type interp_type
     real(GW_PRC), pointer :: wgts(:)
     real(GW_PRC), pointer :: wgtn(:)
     integer, pointer  :: jjm(:)
     integer, pointer  :: jjp(:)
...
contains
  subroutine lininterp_full1d (arrin, yin, nin, arrout, yout, nout)
    ...
    real(GW_PRC), intent(in) :: arrin(nin), yin(nin), yout(nout)
    real(GW_PRC), intent(out) :: arrout(nout)
...
!
    ...
    real(GW_PRC), intent(in) :: yin(:)           ! input mesh
    real(GW_PRC), intent(in) :: yout(:)         ! output mesh
    integer, intent(in) :: extrap_method       ! if 0 set values outside output grid to 0
                                               ! if 1 set to boundary value
                                               ! if 2 set to cyclic boundaries
    real(GW_PRC), intent(in), optional :: cyclicmin, cyclicmax

    type (interp_type), intent(out) :: interp_wgts
    real(GW_PRC) :: cmin, cmax
    real(GW_PRC) :: extrap
    real(GW_PRC) :: dyinwrap
    real(GW_PRC) :: ratio
    real(GW_PRC) :: avgdyin
    integer :: i, j, icount
    integer :: jj
    ...
    case (extrap_method_cycle)
       ...
       avgdyin = abs(yin(nin)-yin(1))/(nin-1._GW_PRC)
       ratio = dyinwrap/avgdyin
       if (ratio < 0.9_GW_PRC .or. ratio > 1.1_GW_PRC) then
    ...
    do j=1,nout
       ...
       ratio=wgts(j)+wgtn(j)
       if((ratio<0.9_GW_PRC.or.ratio>1.1_GW_PRC).and.extrap_method.ne.0) then
  ...
  subroutine lininterp1d (arrin, n1, arrout, m1, interp_wgts)
    ...
    real(GW_PRC), intent(in) :: arrin(n1)    ! input array of values to interpolate
    type(interp_type), intent(in) :: interp_wgts
    real(GW_PRC), intent(out) :: arrout(m1) ! interpolated array
  ...
  subroutine lininterp2d2d(arrin, n1, n2, arrout, m1, m2, wgt1, wgt2)
    ...
    real(GW_PRC), intent(in) :: arrin(n1,n2)    ! input array of values to interpolate
    type(interp_type), intent(in) :: wgt1, wgt2
    real(GW_PRC), intent(out) :: arrout(m1,m2) ! interpolated array
    !
    ! locals
    !
    integer i,j                ! indices
    ...
    real(GW_PRC), pointer :: wgts1(:), wgts2(:)
    real(GW_PRC), pointer :: wgtn1(:), wgtn2(:)

    real(GW_PRC) :: arrtmp(n1,m2)
  ...
  subroutine lininterp2d1d(arrin, n1, n2, arrout, m1, wgt1, wgt2, fldname)
    ...
    real(GW_PRC), pointer :: wgts(:), wgte(:)
    real(GW_PRC), pointer :: wgtn(:), wgtw(:)
  ...
  subroutine lininterp3d2d(arrin, n1, n2, n3, arrout, m1, len1, wgt1, wgt2)
    ...
    real(GW_PRC), intent(in) :: arrin(n1,n2,n3)    ! input array of values to interpolate
    type(interp_type), intent(in) :: wgt1, wgt2
    real(GW_PRC), intent(out) :: arrout(len1, n3) ! interpolated array
  ...
  subroutine lininterp_original (arrin, yin, nlev, nlatin, arrout, &
    ...
    real(GW_PRC), intent(in) :: arrin(nlev,nlatin)    ! input array of values to interpolate
    real(GW_PRC), intent(in) :: yin(nlatin)           ! input mesh
    real(GW_PRC), intent(in) :: yout(nlatout)         ! output mesh

    real(GW_PRC), intent(out) :: arrout(nlev,nlatout) ! interpolated array
    !
    ! Local workspace
    !
    ...
    real(GW_PRC) extrap            ! percent grid non-overlap
    !
    ! Dynamic
    !
    integer :: jjm(nlatout)
    ...
    real(GW_PRC) :: wgts(nlatout)
    real(GW_PRC) :: wgtn(nlatout)
    !
    ! Check validity of input coordinate arrays: must be monotonically increasing,
    ! and have a total of at least 2 elements
    !
  ...
  subroutine bilin (arrin, xin, yin, nlondin, nlonin, &
    ...
    real(GW_PRC), intent(in) :: arrin(nlondin,nlevdin,nlatin) ! input array of values to interpolate
    real(GW_PRC), intent(in) :: xin(nlondin)                  ! input x mesh
    real(GW_PRC), intent(in) :: yin(nlatin)                   ! input y mesh
    real(GW_PRC), intent(in) :: xout(nlondout,nlatout)        ! output x mesh
    real(GW_PRC), intent(in) :: yout(nlatout)                 ! output y mesh
    !
    ! Output arguments
    !
    real(GW_PRC), intent(out) :: arrout(nlondout,nlevdout,nlatout) ! interpolated array
    !
    ! Local workspace
    !
    ...
    real(GW_PRC) :: extrap               ! percent grid non-overlap
    real(GW_PRC) :: dxinwrap             ! delta-x on input grid for 2-pi
    real(GW_PRC) :: avgdxin              ! avg input delta-x
    real(GW_PRC) :: ratio                ! compare dxinwrap to avgdxin
    real(GW_PRC) :: sum                  ! sum of weights (used for testing)
    !
    ! Dynamic
    !
    ...
    real(GW_PRC) :: wgts(nlatout)        ! interpolation weight to the north
    real(GW_PRC) :: wgtn(nlatout)        ! interpolation weight to the north
    real(GW_PRC) :: wgte(nlondout)       ! interpolation weight to the north
    real(GW_PRC) :: wgtw(nlondout)       ! interpolation weight to the north
    real(GW_PRC) :: igrid(nlonin)        ! interpolation weight to the north
    !
    ! Check validity of input coordinate arrays: must be monotonically increasing,
    ! and have a total of at least 2 elements
    !
    ...
    if (xin(1) < 0._GW_PRC .or. xin(nlonin) > 360._GW_PRC) then
    ...
    do j=1,nlatout
       if (xout(1,j) < 0._GW_PRC .or. xout(nlonout(j),j) > 360._GW_PRC) then
...
30     jjprev = jj
    ...
    dxinwrap = xin(1) + 360._GW_PRC - xin(nlonin)
    !
    ! Check for sane dxinwrap values.  Allow to differ no more than 10% from avg
    !
    avgdxin = (xin(nlonin)-xin(1))/(nlonin-1._GW_PRC)
    ...
    do j=1,nlatout
       ...
       sum = wgts(j) + wgtn(j)
       if (sum < 0.99999_GW_PRC .or. sum > 1.00001_GW_PRC) icount = icount + 1
       if (wgts(j) < 0._GW_PRC .or. wgts(j) > 1._GW_PRC) icount = icount + 1
       if (wgtn(j) < 0._GW_PRC .or. wgtn(j) > 1._GW_PRC) icount = icount + 1
    end do
    ...
    do j=1,nlatout
       ...
       do iw=1,nlonout(j)
          ...
          wgte(iw) = (xout(iw,j)+360._GW_PRC - xin(nlonin))/dxinwrap
       end do
       ...
       do ie=nlonout(j),1,-1
          ...
          iim(ie) = nlonin
          iip(ie) = 1
          wgtw(ie) = (xin(1)+360._GW_PRC - xout(ie,j))   /dxinwrap
...
60        iiprev = ii
       ...
       do i=1,nlonout(j)
          ...
          sum = wgtw(i) + wgte(i)
          if (sum < 0.99999_GW_PRC .or. sum > 1.00001_GW_PRC) icount = icount + 1
          if (wgtw(i) < 0._GW_PRC .or. wgtw(i) > 1._GW_PRC) icount = icount + 1
          if (wgte(i) < 0._GW_PRC .or. wgte(i) > 1._GW_PRC) icount = icount + 1
       end do
  ...
  subroutine vertinterp(ncol, ncold, nlev, pmid, pout, arrin, arrout)
    ...
    real(GW_PRC), intent(in)  :: pmid(ncold,nlev)  ! input level pressure levels
    real(GW_PRC), intent(in)  :: pout              ! output pressure level
    real(GW_PRC), intent(in)  :: arrin(ncold,nlev) ! input  array
    real(GW_PRC), intent(out) :: arrout(ncold)     ! output array (interpolated)
    !--------------------------------------------------------------------------

    !---------------------------Local variables-----------------------------
    integer i,k               ! indices
    integer kupper(ncold)     ! Level indices for interpolation
    real(GW_PRC) dpu              ! upper level pressure difference
    real(GW_PRC) dpl              ! lower level pressure difference
  ...
  subroutine get_timeinterp_factors (cycflag, np1, cdayminus, cdayplus, cday, &
    ...
    real(GW_PRC), intent(in) :: cdayminus          ! calendar day of rearward time slice
    real(GW_PRC), intent(in) :: cdayplus           ! calendar day of forward time slice
    real(GW_PRC), intent(in) :: cday               ! calenar day to be interpolated to
    real(GW_PRC), intent(out) :: fact1             ! time interpolation factor to apply to rearward time slice
    real(GW_PRC), intent(out) :: fact2             ! time interpolation factor to apply to forward time slice

    character(len=*), intent(in) :: str        ! string to be added to print in case of error (normally the callers name)
    !
    ! Local workspace
    !
    real(GW_PRC) :: deltat                         ! time difference (days) between cdayminus and cdayplus
    real(GW_PRC), parameter :: daysperyear = 365._GW_PRC  ! number of days in a year
    !
    ...
    if (cycflag) then
       if ((cday < 1._GW_PRC) .or. (cday > (daysperyear+1._GW_PRC))) then
          write(iulog,*) 'GETFACTORS:', str, ' bad cday=',cday
          STOP ! call endrun ()
       end if
    else
       if (cday < 1._GW_PRC) then
          write(iulog,*) 'GETFACTORS:', str, ' bad cday=',cday
          STOP ! call endrun ()
       end if
    end if
  ...
  logical function valid_timeinterp_factors (fact1, fact2)
    ...
    real(GW_PRC), intent(in) :: fact1, fact2           ! time interpolation factors

    valid_timeinterp_factors = .true.

    ! The fact1 .ne. fact1 and fact2 .ne. fact2 comparisons are to detect NaNs.
    if (abs(fact1+fact2-1._GW_PRC) > 1.e-6_GW_PRC .or. &
         fact1 > 1.000001_GW_PRC .or. fact1 < -1.e-6_GW_PRC .or. &
         fact2 > 1.000001_GW_PRC .or. fact2 < -1.e-6_GW_PRC .or. &
