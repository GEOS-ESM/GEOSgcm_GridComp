...
!
! Export diagnostics for thermodynamic state in TURBULENCE
!
...
!    VERIFY_(STATUS)
    ...
    call MAPL_AddExportSpec(GC,                                  &
       SHORT_NAME = 'wqt_mf',                                    &
       LONG_NAME  = 'edmf_qt_flux_contribution',                 &
       UNITS      = 'Wm-2',                                      &
       DIMS       = MAPL_DimsHorzVert,                           &
       VLOCATION  = MAPL_VLocationEdge,               RC=STATUS  )
    VERIFY_(STATUS)
...
!
    ...
    call MAPL_AddInternalSpec(GC,                                &
       SHORT_NAME = 'tke_new2',                                  &
       LONG_NAME  = 'turbulent_kinetic_energy_new2',             &
       UNITS      = 'm+2 s-2',                                   &
       DEFAULT    = 0.,                                          &
       FRIENDLYTO = 'TURBULENCE',                                &
       DIMS       = MAPL_DimsHorzVert,                           &
    ...
    call MAPL_AddInternalSpec(GC,                                               &
         SHORT_NAME = 'KM_mynn',                                              &
         LONG_NAME  = 'momentum_diffusivity_from_MYNN',                       &  
         UNITS      = 'm+1 s-2',                                              &
         DEFAULT    = 0.0,                                         &
         FRIENDLYTO = 'TURBULENCE',                                &
         DIMS       = MAPL_DimsHorzVert,                                      &
    ...
    call MAPL_AddInternalSpec(GC,                                               &
         SHORT_NAME = 'KH_mynn',                                              &
         LONG_NAME  = 'scalar_diffusivity_from_MYNN',                         &  
         UNITS      = 'm+1 s-2',                                              &
         DEFAULT    = 0.0,                                         &
         FRIENDLYTO = 'TURBULENCE',                                &
         DIMS       = MAPL_DimsHorzVert,                                      &
    ...
    call MAPL_AddInternalSpec(GC,                                        &
         SHORT_NAME = 'tket_T',                                          &
         LONG_NAME  = 'diffusive_turbulent_transport_of_turbulent_kinetic_energy', &
         UNITS      = 'm+2 s-3',                                         &
         FRIENDLYTO = 'TURBULENCE',                                      &
         DEFAULT    = 0.,                                                &
         DIMS       = MAPL_DimsHorzVert,                                 &
    ...
    call MAPL_AddInternalSpec(GC,                                        &
         ...
         LONG_NAME  = 'advective_turbulent_transport_of_turbulent_kinetic_energy', &
         UNITS      = 'm+2 s-3',                                         &
         FRIENDLYTO = 'TURBULENCE',                                      &
         DEFAULT    = 0.,                                                &
         DIMS       = MAPL_DimsHorzVert,                                 &
    ...
    call MAPL_AddInternalSpec(GC,                                    &
         ...
         LONG_NAME  = 'explicit_water_vapor_specific_humidity_flux', &
         UNITS      = 'kg kg-1 m s-1',                               &
         DEFAULT    = 0.0,                                           &
         FRIENDLYTO = 'TURBULENCE',                                  &
         DIMS       = MAPL_DimsHorzVert,                             &
    ...
    call MAPL_AddInternalSpec(GC,                                            &
       LONG_NAME  = 'momentum_stability_function_from_mynn',                 &
       UNITS      = '1',                                                     &
       DEFAULT    = 0.,                                                      &
       FRIENDLYTO = 'TURBULENCE',                                            &
       SHORT_NAME = 'SM25',                                              &
    ...
    call MAPL_AddInternalSpec(GC,                                            &
       LONG_NAME  = 'thermodynamic_stability_function_from_mynn',            &
       UNITS      = '1',                                                     &
       DEFAULT    = 0.,                                                      &
       FRIENDLYTO = 'TURBULENCE',                                            &
       SHORT_NAME = 'SH25',                                                  &
...
!----------------------
    ...
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, tke_new2,  'tke_new2',   RC=STATUS)
    VERIFY_(STATUS)
    ...
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, tket_T_mf, 'tket_T_mf', RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, whl_edmf,    'whl_edmf',    RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, wqt_edmf,    'wqt_edmf',    RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, wthv_edmf,   'wthv_edmf',   RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, itau_mynn, 'itau_mynn', RC=STATUS)
    VERIFY_(STATUS)

    call MAPL_GetPointer(INTERNAL, K_tke, 'K_tke', RC=STATUS)
    VERIFY_(STATUS)

    call MAPL_GetPointer(INTERNAL, tket_T_mf1, 'tket_T_mf1', RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, tket_T_mf2, 'tket_T_mf2', RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, tket_T_mf3, 'tket_T_mf3', RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, tket_T_mf4, 'tket_T_mf4', RC=STATUS)
    VERIFY_(STATUS)

    call MAPL_GetPointer(INTERNAL, au,   'au', RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, Mu,   'Mu', RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, E,    'E',  RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, D,    'D',  RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, wu,   'wu',  RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, wdet, 'wdet',  RC=STATUS)
    VERIFY_(STATUS)
    ...
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, qdiv,            'qdiv',         RC=STATUS)
    VERIFY_(STATUS)
    call MAPL_GetPointer(INTERNAL, SM25,            'SM25',         RC=STATUS)
    VERIFY_(STATUS)
...
!-----------------------------------
     ...
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT, qt_turb,    'qt_turb',     RC=STATUS)
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT, thlv_turb,  'thlv_turb',   RC=STATUS)
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT, ql_turb,    'ql_turb',     RC=STATUS)
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT, ac_turb,    'ac_turb',     RC=STATUS)
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT, thv_turb,   'thv_turb',    RC=STATUS)
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT, rh_turb,    'rh_turb',     RC=STATUS)
     VERIFY_(STATUS)
...
!========== MYNN ===========
     ...
     ! Second-order moment budget terms
     ...
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT, qtu_full,  'qtu_full',  ALLOC=.TRUE., RC=STATUS)
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT, acu_full,  'acu_full',  ALLOC=.TRUE., RC=STATUS)
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT, Tu_full,  'Tu_full',  ALLOC=.TRUE., RC=STATUS)
     VERIFY_(STATUS)
     ...
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT, wqt_mf,   'wqt_mf',  RC=STATUS)
     VERIFY_(STATUS)
...
! Initialize some arrays
   ...
   ! get thl and qt
    ...
    if ( associated( thl_turb ) )  thl_turb   = THL
    if ( associated( qt_turb ) )   qt_turb    = QT
    if ( associated( thlv_turb ) ) thlv_turb  = THV*( 1. + QT )
    if ( associated( ql_turb ) )   ql_turb    = QL
    if ( associated( ac_turb ) )   ac_turb    = QA
    if ( associated( thv_turb ) )  thv_turb   = THV

    if ( associated(rh_turb) ) then
       allocate( qsat_turb(IM,JM,LM), dqs_turb(IM,JM,LM) )
       qsat_turb = MAPL_EQsat(T, PLO, dqs_turb)
       rh_turb   = q/qsat_turb
    end if
...
if ( ET == 1 ) then
    ...
    if ( EDMF_SCHEME == 0 ) then
       call EDMF(1,IM*JM,1,LM,DT,Z,EXF,ZLE,PLE,RHOE,NumUp,&
                 ...
                 WHL_EDMF,WQT_EDMF,WTHV_EDMF, & ! for MYNN  
...
ENDIF
 ...
 if ( associated(wthl_mf) ) then
    wthl_mf = mapl_cp*rhoe*whl_edmf/(ple/mapl_p00)**(MAPL_RGAS/MAPL_CP)
 end if

 if ( associated(wqt_mf) ) then
    wqt_mf = mapl_alhl*rhoe*wqt_edmf
 end if
...
!EOP
    ...
    real, dimension(:,:,:), pointer     :: tket_M, tket_B, tket_D, &
                                           ...
                                           ws_ed, wqv_ed, wql_ed, wthl_ed, wqt_ed, &
                                           A_moist, B_moist, ace_moist
...
!--------------------------------------------------------
    if ( DO_MYNN /= 0 ) then
       call MAPL_GetPointer(EXPORT, wthl_ed, 'wthl_ed', RC=STATUS)
       VERIFY_(STATUS)
       call MAPL_GetPointer(EXPORT, wqt_ed,  'wqt_ed',  RC=STATUS)
       VERIFY_(STATUS)
       if ( associated(wthl_ed) .or. associated(wqt_ed) ) then 
          call MAPL_GetPointer(EXPORT, ws_ed,  'ws_ed',  ALLOC=.TRUE., RC=STATUS)
          VERIFY_(STATUS)
          call MAPL_GetPointer(EXPORT, wqv_ed, 'wqv_ed', ALLOC=.TRUE., RC=STATUS)
          VERIFY_(STATUS)
          call MAPL_GetPointer(EXPORT, wql_ed, 'wql_ed', ALLOC=.TRUE., RC=STATUS)
          VERIFY_(STATUS)
       else
          call MAPL_GetPointer(EXPORT, ws_ed,  'ws_ed',  RC=STATUS)
          VERIFY_(STATUS)
          call MAPL_GetPointer(EXPORT, wqv_ed, 'wqv_ed', RC=STATUS)
          VERIFY_(STATUS)
          call MAPL_GetPointer(EXPORT, wql_ed, 'wql_ed', RC=STATUS)
          VERIFY_(STATUS)
       end if
...
! --------------------------------------------------------------------------------
       ...
       if ( trim(name) == 'tke_new' .and. DO_MYNN /= 0 ) then
          ...
          if ( associated(wql_ed) ) then
             wql_ed(:,:,0)      = 0.
             wql_ed(:,:,1:LM-1) = -mapl_alhl*rhoe(:,:,1:LM-1)*KH_mynn(:,:,1:LM-1)*( QL(:,:,1:LM-1) - QL(:,:,2:LM) )/( ZLO(:,:,1:LM-1) - ZLO(:,:,2:LM) ) 
             wql_ed(:,:,LM)     = 0.
          end if

          if ( associated( wthl_ed ) ) then
             wthl_ed = ( ws_ed - wql_ed )/(ple/mapl_p00)**(MAPL_RGAS/MAPL_CP)
          end if
          if ( associated( wqt_ed ) ) then
             wqt_ed = wqv_ed + wql_ed
          end if

          ! Get M and B source terms implicitly 
          call implicit_M(IM, JM, LM, &                                        ! in
                          ...
                          whl_edmf, wqt_edmf, wthv_edmf, &                           ! in
                          hl2, qt2, hlqt, &                                    ! in
...
if ( trim(name) /= 'S' .and. trim(name) /= 'Q' .and. trim(name) /= 'QLLS' & 
     ...
     .and. trim(name) /= 'tke_new' .and.  trim(name) /= 'hl2' .and. trim(name) /= 'qt2' .and. trim(name) /= 'hlqt' &
     .and. trim(name) /= 'tke_new2' ) then
...
!------------------
       ...
       if ( associated(SOI) ) then
          if (     trim(name) == 'tke_new' .or. trim(name) == 'hl2' .or. trim(name) == 'qt2' .or. trim(name) == 'hlqt' &
              .or. trim(name) == 'tke_new2') then
             SOI = ( ( SX_HALF - S )/DT )
...
!       else if ( trim(name) == 'tke_new2' ) then
!          tket_D(:,:,1:LM-1) = 0.5*( tket_D(:,:,1:LM-1) - itau_mynn(:,:,1:LM-1)*SX_HALF(:,:,1:LM-1)/B1 )
       end if
...
!          else if ( trim(name) == 'tke_new2' .and. associated(tket_T) .and. associated(tket_D) ) then ! temporary
!             tket_T(:,:,1:LM-1) = tket_T(:,:,1:LM-1) + ( SOI(:,:,1:LM-1) - tket_M(:,:,1:LM-1) - tket_B(:,:,1:LM-1) - tket_D(:,:,1:LM-1) - tket_T_mf(:,:,1:LM-1) )
       ...
       ! Save updated mean state variables for implicit mean-gradient production
       if ( DO_MYNN /= 0 ) then
          if (trim(name) == 'U') then
             U = SX
             ...
             QLLS = SX
          else if (trim(name) == 'QICN') then
             QICN = SX
          else if (trim(name) == 'QILS') then
             QILS = SX
          end if
       end if
...
!------------------
       ...
       if( FRIENDLY ) then
          if ( trim(name) == 'tke_new' .or. trim(name) == 'hl2' .or. trim(name) == 'qt2' .or. trim(name) == 'hlqt' ) then
             S = SX_HALF
!          else if ( trim(name) == 'tke_new2' ) then
!             tke_new2 = tke_new ! Save first prediction
!             tke_new  = SX_half ! Set tke_new equal to corrected prediction
          else
             S = SX
          end if
       end if
...
!--------------------------
         ...
         if (     trim(name) == 'tke_new' .or. trim(name) == 'hl2' .or. trim(name) == 'qt2' .or. trim(name) == 'hlqt' &
             .or. trim(name) == 'tke_new2') cycle TRACERS
...
SUBROUTINE EDMF(its,ite,kts,kte,dt,zlo3,exf3,zw3,pw3,rhoe3,nup,&
           ...
           !  &dth,dqv,dqc,du,dv,&
            ...
            ! outputs - variables needed for solver 
             ...
             thl_plume6,thl_plume7,thl_plume8,thl_plume9,thl_plume10, &
#endif
             discrete_type, implicit_flag, stochastic_flag)
...
! s_aw3,s_awthl3 ... (ITS:ITE,KTS-1:KTE)
       ...
       REAL,DIMENSION(ITS:ITE,KTS:KTE), INTENT(IN) :: U3,V3,THL3,QT3,THV3,T3,QV3,QL3,QI3,ZLO3,EXF3
      ...
      ! REAL,DIMENSION(ITS:ITE,KTS:KTE+1), INTENT(IN) :: ZW
       ...
       INTEGER, INTENT(IN) :: discrete_type, implicit_flag, stochastic_flag
...
! internal surface cont
      REAL :: UST,WTHL,WQT,PBLH
       REAL,DIMENSION(KTS-1:KTE) :: dry_a, moist_a,dry_w,moist_w, &
               dry_qt,moist_qt,dry_thl,moist_thl,dry_u,moist_u,dry_v,moist_v, moist_qc, moist_ql
...
! exner function
        ...
        real                      :: thlu_full_flip
        REAL,DIMENSION(KTS-1:KTE) :: rhoe, au_flip, wu, Mu_flip, thlu, qtu, qlu
        REAL,DIMENSION(KTS:KTE)   :: E_flip, D_flip, au_full_flip, hlu_full_flip, &
                                     qtu_full_flip, acu_full_flip, Tu_full_flip, qlu_full_flip
!  
...
! min values to avoid singularities
  REAL,PARAMETER :: &
     ...
     ! set updraft properties to zero/undef
      ...
      ! Outputs needs for MYNN-EDMF cloud scheme in MOIST
      au_full  = 0.
      qlu_full = 0.
      hlu_full = exf3*thl3 + (mapl_grav/mapl_cp)*zlo3
      qtu_full = qt3
      Tu_full  = exf3*thl3
      acu_full = 0.
 ...
 !        print *,'adjusting surface THV for a factor',wthv/THVsrfF
  ...
  !     
     ...
     thl_plume10(IH,k)= upw(kte+kts-k-1,10)
         ...
         DO I=1,NUP ! first sum over all i-updrafts
            IF ((UPQL(K,I)>0.) .OR. UPQI(K,I)>0.)  THEN
               ...
               moist_ql(K)  = moist_ql(K)  + UPA(K,I)*UPQL(K,I)
            ELSE
         ...
         IF (moist_a(k)>0.) THEN
            ...
            moist_ql(k)  = moist_ql(k)/moist_a(k)
         ELSE
         ...
         if ( au_flip(k) > 0. ) then            
            ...
            Mu_flip(k) = rhoe(k)*au_flip(k)*wu(k)
         else
            wu(k)   = 0.
            thlu(k) = thli(k)
            qtu(k)  = qti(k)
   ...
   do k = kts,kte
      ...
      ! Compute quantities need in MOIST
      if ( au_flip(k-1) > 0. .or. au_flip(k) > 0. ) then
         au_full_flip(k)  = 0.5*( au_flip(k-1) + au_flip(k) )
         qlu_full_flip(k) = 0.5*( qlu(k-1)     + qlu(k)     )
         thlu_full_flip   = 0.5*( thlu(k-1)    + thlu(k)    ) 
         qtu_full_flip(k) = 0.5*( qtu(k-1)     + qtu(k)     )

         acu_full_flip(k) = min( au_full_flip(k), 0.5*( moist_a(k-1) + moist_a(k) ) )
      else
         au_full_flip(k)  = 0.
         qlu_full_flip(k) = 0.
         thlu_full_flip   = thl(k)
         qtu_full_flip(k) = qt(k)

         acu_full_flip(k) = 0.
      end if
      hlu_full_flip(k) = exf(k)*thlu_full_flip + (mapl_grav/mapl_cp)*zlo(k)
      Tu_full_flip(k)  = exf(k)*thlu_full_flip + (mapl_alhl/mapl_cp)*qlu_full_flip(k)
   end do
...
! buoyancy is defined on full levels    
  DO k=kts,kte
      ...
      au_full(IH,K)  = au_full_flip(KTE+KTS-K)
      hlu_full(IH,K) = hlu_full_flip(KTE+KTS-K)
      qtu_full(IH,K) = qtu_full_flip(KTE+KTS-K)
      acu_full(IH,K) = acu_full_flip(KTE+KTS-K)
      Tu_full(IH,K)  = Tu_full_flip(KTE+KTS-K)
      qlu_full(IH,K) = qlu_full_flip(KTE+KTS-K)
  ENDDO  
