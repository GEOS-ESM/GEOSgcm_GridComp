...
! Initialize EDMF output variables needed for update_moments
    ...
    IF(DoMF /= 0) then
      ...
      call EDMF(1, IM*JM, 1, LM, DT,          & ! in
               Z, ZLE, PLE, RHOE, L02, NUMUP, & ! in
               U, V, THL, THV, QT,            & ! in
               Q, QL, QI, USTAR,              & ! in
...
subroutine calc_mf_depth(im,jm,nlev,t,z,q,p,ztop)

  integer, intent(in   )                        :: nlev, im, jm
  real,    intent(in   ), dimension(im,jm,nlev) :: t, z, q, p
