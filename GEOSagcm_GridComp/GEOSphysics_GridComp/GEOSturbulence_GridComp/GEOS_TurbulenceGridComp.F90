...
!    VERIFY_(STATUS)
    ...
    call MAPL_AddExportSpec(GC,                              &
         SHORT_NAME = 'W_ENTR',                                   &
         LONG_NAME  = 'PBL_top_entrainment_velocity',             &
         UNITS      = 'm s-1',                                    &
         DIMS       = MAPL_DimsHorzOnly,                          &
         VLOCATION  = MAPL_VLocationNone,                         &
         RC=STATUS  )
...
!---------------------------------------------
     ...
     if (DO_SHOC /= 0) then
       ...
       call MAPL_GetResource (MAPL, SHOCPARAMS%LENFAC3, trim(COMP_NAME)//"_SHC_LENFAC3:",     default=0.3,  RC=STATUS); VERIFY_(STATUS)
     ...
     call MAPL_GetResource (MAPL, SKEW_TDIS,  'SKEW_TDIS:',  DEFAULT = 1800.0, RC=STATUS); VERIFY_(STATUS)
...
!========== SHOC ===========
     ...
     call MAPL_GetPointer(EXPORT, W_DIAG, 'W_DIAG', RC=STATUS)
     VERIFY_(STATUS)
...
! get updraft constants
    ...
    if ( DOMF /= 0 ) then
      ...
      call MAPL_GetResource (MAPL, MFPARAMS%AlphaQT,   "EDMF_ALPHAQT:",       default=1.5,   RC=STATUS)
      call MAPL_GetResource (MAPL, MFPARAMS%AlphaTH,   "EDMF_ALPHATH:",       default=1.5,   RC=STATUS) 
...
!       print *,'ustar=',ustar_scm,' cu=',cu      
       bstar_scm = 0.002
!       bstar_scm = (MAPL_GRAV/(RHOE(:,:,LM)*sqrt(CU*max(U(:,:,LM),1.e-30)/RHOE(:,:,LM)))) *  &
!                   (CT*(TH(:,:,LM)-TA-(MAPL_GRAV/MAPL_CP)*DZ)/TA + MAPL_VIREPS*CQ*(QH-QA))
...
!          if (maxval(cnv_mfc(i,j,:)).gt.0.01) drycblh(i,j) = 1.4*drycblh(i,j)
        end do
      end do
...
!--------------------
      ...
      if (associated(W_ENTR)) W_ENTR = ZPBL
...
!=========================================================================                                      
    if (associated(ZPBLRFRCT)) then
     ...
     if (associated(W_DIAG).or.associated(W_ENTR)) W_DIAG = -OMEGA/(MAPL_GRAV*PLO/(MAPL_RGAS*T)) 
...
!              end if
     ...
     ! PBL top entrainment velocity, diagnosed from PBLH tendency and large-scale ascent
     if (associated(W_ENTR)) then
        do I = 1, IM
          do J = 1, JM
             W_ENTR(I,J) = (ZPBL(I,J)-W_ENTR(I,J))/DT - W_DIAG(I,J,INT(KPBL(I,J)))
          end do
        end do
     end if
...
! Add mass flux contribution
  ...
  if (MFPARAMS%IMPLICIT == 1) then
     if (MFPARAMS%DISCRETE == 0) then
        ...
        BKSS(:,:,1:LM-1) = BKSS(:,:,1:LM-1) + DMI(:,:,1:LM-1)*RHOAW3(:,:,1:LM-1)
        BKQQ(:,:,1:LM-1) = BKQQ(:,:,1:LM-1) + DMI(:,:,1:LM-1)*RHOAW3(:,:,1:LM-1)
        BKUU(:,:,1:LM-1) = BKUU(:,:,1:LM-1) + DMI(:,:,1:LM-1)*RHOAW3(:,:,1:LM-1)
     end if
  end if
...
! Fill exports of U,V and S after diffusion
      ...
      if( TYPE=='V' ) then
       ...
       if( TYPE=='Q' ) then  !!! should this be NAME=='Q' ?
