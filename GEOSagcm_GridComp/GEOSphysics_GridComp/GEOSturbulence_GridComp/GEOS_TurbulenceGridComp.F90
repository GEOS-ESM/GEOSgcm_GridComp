...
!    VERIFY_(STATUS)
   ...
   call MAPL_AddExportSpec(GC,                                               &
    ...
    call MAPL_AddExportSpec(GC,                                              &
       LONG_NAME  = 'radar_wind_profiler_simulated_backscatter',             &
       SHORT_NAME = 'RWPCN2',                                                   &
       UNITS      = '1',                                                     &
       DIMS       = MAPL_DimsHorzVert,                                       &
       VLOCATION  = MAPL_VLocationEdge,                                      &
    ...
    call MAPL_AddExportSpec(GC,                                              &
       LONG_NAME  = 'radar_wind_profiler_boundary_layer_height',             &
       SHORT_NAME = 'ZPBLRWP',                                               &
       UNITS      = 'm',                                                     &
       DIMS       = MAPL_DimsHorzOnly,                                       &
       VLOCATION  = MAPL_VLocationNone,                                      &
    ...
    call MAPL_AddExportSpec(GC,                                              &
       LONG_NAME  = 'radar_wind_profiler_signal_noise_ratio',                &
       SHORT_NAME = 'RWPSNR',                                                &
       UNITS      = '1',                                                     &
       DIMS       = MAPL_DimsHorzOnly,                                       &
       VLOCATION  = MAPL_VLocationNone,                                      &
...
!     real, dimension(:,:,:), pointer     :: MFQTSRC, MFTHSRC, MFW, MFAREA
     ...
     real, dimension(:,:,:), pointer     :: CN2 => null()
     real, dimension(:,:  ), pointer     :: ZPBLRWP => null()
     real, dimension(:,:  ), pointer     :: RWPSNR => null()
...
! EDMF variables
   ...
   real, dimension(:,:,:,:), pointer    :: EDMF_PLUMES_W, EDMF_PLUMES_THL, EDMF_PLUMES_QT
     ...
     real,               dimension(IM,JM,LM  ) :: tcrib,tmpp3d !TransCom bulk Ri
...
!-----------------------------------
     ...
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT,    CN2,  'RWPCN2', ALLOC=.TRUE.,  RC=STATUS)
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT,    ZPBLRWP,  'ZPBLRWP',           RC=STATUS)
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT,    RWPSNR,  'RWPSNR', ALLOC=.TRUE., RC=STATUS)
     VERIFY_(STATUS)
...
!                          MFHL2,          &
      ...
      if (associated(ZPBLRWP)) Then
!         if (.not. associated(CN2)) ALLOCATE(CN2(IM,JM,0:LM), __STAT__)

         ZPBLRWP = 0.  ! Default value if no RWP

         ! N = 77.6 * ( P + 4811.*e/T ) / T   ! this is n-1 multiplied by 1e6
         tmpp3d = (77.6/T)*PLO*(1.+48.11*Q/(T*(0.622+Q)))
         Cn2(:,:,1:LM-1) = ( (tmpp3d(:,:,1:LM-1)-tmpp3d(:,:,2:LM)) / (Z(:,:,1:LM-1)-Z(:,:,2:LM)) )**2
         Cn2(:,:,LM-1) = Cn2(:,:,LM-2)
         Cn2(:,:,LM) = Cn2(:,:,LM-2)
         Cn2(:,:,0) = Cn2(:,:,1)

         ! eta = 0.379*Cn2*lam**(-0.333)  ! reflectivity/backscatter

         tmpp3d(:,:,1:LM-1) = 0.333*(CN2(:,:,0:LM-2)+CN2(:,:,1:LM-1)+CN2(:,:,2:LM))
         tmpp3d(:,:,LM) = CN2(:,:,LM)

         tmpp3d(:,:,2:LM-1) = 0.333*(tmpp3d(:,:,1:LM-2)+tmpp3d(:,:,2:LM-1)+tmpp3d(:,:,3:LM))
         tmpp3d(:,:,1) = 0.333*(tmpp3d(:,:,1)+tmpp3d(:,:,1)+tmpp3d(:,:,2))

         CN2(:,:,1:LM) = CN2(:,:,1:LM) - tmpp3d
         CN2(:,:,0) = CN2(:,:,1)
         ...
         do I = 1,IM
          do J = 1,JM
            ...
            ! algorithm 2: Molod et al 2015
!            b1 = SUM( Cn2SMTH(I,J,LTOP:LM-2) ) / (LM-LTOP-1)  ! average from 6km to near-surface
            b1 = SUM( CN2(I,J,LTOP:LM-2)*DELP(I,J,LTOP:LM-2) ) / SUM(DELP(I,J,LTOP:LM-2))  ! average from 6km to near-surface

!            if (BSTAR(I,J).ge.0.) then

              b2 = SQRT( SUM( DELP(I,J,LTOP:LM-2)*(Cn2(I,J,LTOP:LM-2)-b1)**2 ) / SUM(DELP(I,J,LTOP:LM-2)) ) ! std dev

              L = LM-2
              do while (L.gt.LTOP)
                 if (Cn2(I,J,L).gt.Cn2(I,J,L+1) .and. Cn2(I,J,L).gt.Cn2(I,J,L-1)) then ! if local max
                    LMAX = L-1
                    do while (Cn2(I,J,LMAX).gt.Cn2(I,J,LMAX-1) .and. LMAX.gt.LTOP) ! scan up for local min
                       LMAX = LMAX-1
                    end do
                    b1 = Cn2(I,J,L)-Cn2(I,J,LMAX)
                    LMAX = L+1
                    do while (Cn2(I,J,LMAX).gt.Cn2(I,J,LMAX+1) .and. LMAX.lt.LM) ! scan down for local min
                       LMAX = LMAX+1
                    end do
!                    b1 = 0.5*(b1 + Cn2(I,J,L)-Cn2(I,J,LMAX))   ! use the average up and down max-min difference
