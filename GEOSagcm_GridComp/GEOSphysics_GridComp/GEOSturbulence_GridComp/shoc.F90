...
!
 ...
 subroutine run_shoc( nx, ny, nzm, nz, dtn,                      &  ! in
                 ...
                 bruntmst_inv, bruntdry_inv, bruntcld_inv,       &  ! out
                 bruntedg_inv, ri_inv, prnum_inv,                &  ! out
                 shocparams )
...
!=== Assign exports and flip vertical ===!
  ...
  if (associated(bruntcld_inv)) bruntcld_inv(:,:,1:nzm) = brunt_cld(:,:,nzm:1:-1)
...
! Calculate Brunt-Vaisalla frequency using centered differences in the vertical
             ...
             brunt_cld(i,j,k) = betdz*(bbb*(hl(i,j,kc)-hl(i,j,kb))               &
             ...
             brunt(i,j,k) = cld_sgs(i,j,k)*brunt_cld(i,j,k)

! Calculate again on edges, using mean of adjacent cloud fractions
...
!                 smixt1(i,j,k) = vonk*zl(i,j,k)*shocparams%LENFAC1
                 ...
                 if (shocparams%LENOPT .eq. 1) then  ! JPL blending approach (w/SHOC length scales)
                    ...
                    smixt(i,j,k) = min(max_eddy_length_scale, 7.5/SQRT(1./smixt1(i,j,k)**2+1./smixt2(i,j,k)**2+1./smixt3(i,j,k)**2) )
                 end if
              ...
              smixt2(i,j,k) = 3.3*shocparams%LENFAC1/wrk2
              smixt3(i,j,k) = 3.3*shocparams%LENFAC1/wrk3
              smixt1(i,j,k) = 3.3*shocparams%LENFAC1*vonk*zl(i,j,k)
           end if

           ! Enforce minimum and maximum length scales
           if (zl(i,j,k) .lt. 2000.) then
             smixt(i,j,k) = max(20., smixt(i,j,k))
           else if (zl(i,j,k).gt.zpbl(i,j)) then 
...
!    print *,'tke=',TKE(1,1,LM-20:LM)
!    print *,'adzl=',adzl(1,1,LM-20:LM)
!    print *,'isotropy=',ISOTROPY(1,1,LM-20:LM)
!    print *,'brunt=',BRUNT(1,1,LM-20:LM)
!    print *,'hl2=',hl2(1,1,LM-20:LM)
!    print *,'whl_can=',whl_can(1,1,LM-20:LM)
    do k=2,LM
...
! therefore it's easy to diagnose other third order moments obtained in C01 using this code.
    ...
    do j=1,JM
      ...
      enddo
    enddo
!    print *,'w3can=',w3can(1,1,LM-20:LM)
!    w3 = w3can
