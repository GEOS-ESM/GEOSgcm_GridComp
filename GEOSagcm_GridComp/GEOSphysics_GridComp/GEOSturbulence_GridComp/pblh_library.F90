module GEOSturbulence_PBLH_Library
  ...
  use ESMF
  use MAPL
  use MAPL_ConstantsMod, only: mapl_grav, mapl_cp, mapl_karman, mapl_rdry, &
                               mapl_pi, mapl_p00, mapl_pi
...
contains
   ...
   subroutine find_turning_angle(im,jm,lm,u,v,z,zpbl,kpbl,turnangle)
      ...
      turnangle = (180./mapl_pi)*atan2( utop*v(:,:,lm)-vtop*u(:,:,lm), utop*u(:,:,lm) + vtop*v(:,:,lm) )
