module moist_subroutines_GF2020
    ...
    REAL,   DIMENSION(maxiens) :: CUM_HEI_DOWN_LAND =(/0.65,  0.00,  0.65/)!= [0.2,0.8] height of the max Z Downdraft , default = 0.50
    REAL,   DIMENSION(maxiens) :: CUM_HEI_DOWN_OCEAN=(/0.65,  0.00,  0.50/)!= [0.2,0.8] height of the max Z Downdraft , default = 0.50
    REAL,   DIMENSION(maxiens) :: CUM_HEI_UPDF_LAND =(/0.35,  0.10,  0.35/)!= [0.2,0.8] height of the max Z Updraft   , default = 0.35
    REAL,   DIMENSION(maxiens) :: CUM_HEI_UPDF_OCEAN=(/0.35,  0.10,  0.35/)!= [0.2,0.8] height of the max Z Updraft   , default = 0.35
    REAL,   DIMENSION(maxiens) :: CUM_MAX_EDT_LAND  =(/0.50,  0.00,  0.50/)!= maximum evap fraction allowed over the land  ,default= 0.9
    REAL,   DIMENSION(maxiens) :: CUM_MAX_EDT_OCEAN =(/0.20,  0.00,  0.20/)!= maximum evap fraction allowed over the ocean ,default= 0.9
   
    REAL,   DIMENSION(maxiens) :: CUM_FADJ_MASSFLX  =(/1.00,  1.00,  0.50/)!= multiplicative factor for tunning the mass flux at cloud base
                                                                           != default = 1.0
    INTEGER,DIMENSION(maxiens) :: CUM_USE_EXCESS    =(/2,     1,     2   /)!= use T,Q excess sub-grid scale variability
    ...
    REAL,public    ::  HEI_DOWN_LAND     != [0.2,0.8] height of the max Z Downdraft , default = 0.50
    REAL,public    ::  HEI_DOWN_OCEAN    != [0.2,0.8] height of the max Z Downdraft , default = 0.50
    REAL,public    ::  HEI_UPDF_LAND     != [0.2,0.8] height of the max Z Updraft   , default = 0.35
    REAL,public    ::  HEI_UPDF_OCEAN    != [0.2,0.8] height of the max Z Updraft   , default = 0.35
    REAL,public    ::  MAX_EDT_LAND      != default= 0.9 - maximum evap fraction allowed over the land
    REAL,public    ::  MAX_EDT_OCEAN     != default= 0.9 - maximum evap fraction allowed over the ocean
    REAL,public    ::  FADJ_MASSFLX      != default= 1.0 - multiplicative factor for the mass flux at cloud base
   ...
   END FUNCTION satur_spec_hum
    ...
    SUBROUTINE get_partition_liq_ice(ierr,tn,z1,zo_cup,po_cup, p_liq_ice,melting_layer         &
        ,itf,ktf,its,ite, kts,kte, cnvfrc, srftype, cumulus_1 )
        IMPLICIT NONE
        CHARACTER,INTENT (IN   )                             :: cumulus_1
        INTEGER  ,INTENT (IN   )                             :: itf,ktf, its,ite, kts,kte
        ...
        IF(MELT_GLAC .and. cumulus_1 == 'd') then
            DO k=kts,ktf
                DO i=its,itf
                    if(ierr(i) /= 0) cycle
    ...
    SUBROUTINE precip_cwv_factor(itf,ktf,its,ite,kts,kte,ierr,t,po,qo,po_cup,cumulus_1,p_cwv_ave)
        implicit none
        character,intent (in )                             :: cumulus_1
        integer  ,intent (in )                             :: itf,ktf, its,ite, kts,kte
        integer  ,intent (in ), dimension(its:ite)         :: ierr
   ...
   !
    ...
    SUBROUTINE get_inversion_layers(cumulus_1,ierr,psur,po_cup,to_cup,zo_cup,k_inv_layers,&
    ...
    SUBROUTINE get_zu_zd_pdf(cumulus_1, draft,ierr,kb,kt,zu,kts,kte,ktf,kpbli,k22,kbcon,klcl,po_cup,psur&
        ,xland,random)
...
!---------------------------------------------------------
        ...
        ELSEIF(draft == "DOWN" ) then
            IF(cumulus_1 == 's') return
            IF(cumulus_1 == 'm' ) beta =2.5
            IF(cumulus_1 == 'd') beta =2.5
    ...
    SUBROUTINE cup_up_moisture_light(name,start_level,klcl,ierr,ierrc,z_cup,qc,qrc,pw,pwav,hc,tempc,xland &
        ...
        if(name.eq.'s')  c0 = c0_shal
        if(name.eq.'m'    )  c0 = c0_mid
        if(name.eq.'d'   )  c0 = c0_deep
        do i=its,itf
            pwav (i)=0.
            psum (i)=0.
            psumh(i)=0.
    ...
    SUBROUTINE cup_up_moisture(name,start_level,klcl,ierr,ierrc,z_cup,qc,qrc,pw,pwav,hc,tempc,xland,&
        ...
        if(name == 's' .and. use_linear_subcl_mf == 1) then
            do i=its,itf
            if(ierr(i) /= 0) cycle
            call get_delmix(kts,kte,ktf,xland(i),start_level(i),po(i,kts:kte) &
            ...
            DO k=start_level(i) + 1,ktop(i) + 1
                ...
                ELSEIF (autoconv == 5 ) then
                    ...
                    if(clw_all(i,k) <= min_liq) then !=> more heating at upper levels, more detrained ice
                        ...
                        !cx0     = (c1d(i,k)+c0)*(1.+ 2.*fract_liq_f(tempc(i,k),cnvfrc(i),srftype(i)))
                        !--- v0
    ...
    SUBROUTINE get_melting_profile(ierr,tn_cup,po_cup, p_liq_ice,melting_layer,qrco    &
        ...
        ,itf,ktf,its,ite, kts,kte, cumulus_1                   )
        IMPLICIT NONE
        CHARACTER, INTENT (IN)                               :: cumulus_1
        INTEGER  ,INTENT (IN   )                             :: itf,ktf, its,ite, kts,kte
        INTEGER  ,INTENT (IN   ), DIMENSION(its:ite)         :: ierr
    ...
    SUBROUTINE get_delmix(kts,kte,ktf,xland,subcl_level,po,ain,aout)
        implicit none
        ! character   ,intent (in)      :: cumulus
    ...
    SUBROUTINE get_jmin(cumulus_1,itf,ktf,its,ite, kts,kte,ierr,kdet,ktop,kbcon,jmin,ierrc  &
        ...
        if(cumulus_1 == 'd') beta=0.05
        if(cumulus_1 == 'm' ) beta=0.02
        ...
        do i=its,itf
            ...
            if(cumulus_1 == 'd' .and. melt_glac) jmin(i)=max(jmin(i),maxloc(melting_layer(i,:),1))
    ...
    SUBROUTINE get_lateral_massflux_down(cumulus_1,itf,ktf, its,ite, kts,kte                    &
        ,ierr,jmin,zo_cup,zdo,xzd,zd,cdd,mentrd_rate_2d      &
        ,dd_massentro,dd_massdetro ,dd_massentr, dd_massdetr &
        ,mentrd_rate,dd_massentru,dd_massdetru,lambau)
    ...
    SUBROUTINE cup_dd_moisture(cumulus_1,ierrc,zd,hcd,hes_cup,qcd,qes_cup,                &
    ...
    SUBROUTINE cup_up_aa1bl(version,aa1_bl,aa1_fa,aa1,t,tn,q,qo,dtime,po_cup, &
        ...
        xland,ztexec,xlons,xlats, h_sfc_flux,le_sfc_flux,tau_bl,tau_ecmwf,t_star,tn_bl,qo_bl )
    ...
    SUBROUTINE cup_dd_edt(cumulus_1,ierr,us,vs,z,ktop,kbcon,edt,p,pwav, &
    ...
    SUBROUTINE get_precip_fluxes(klcl,kbcon,ktop,k22,ierr,xland,pre,xmb  &
    ...
    SUBROUTINE rain_evap_below_cloudbase(cumulus_1,itf,ktf, its,ite, kts,kte,ierr,kbcon,ktop,xmb,psur,xland&
    ...
    SUBROUTINE cloud_dissipation(cumulus_1,itf,ktf, its,ite, kts,kte,ierr,kbcon,ktop,dtime,xmb,xland&
    ...
    SUBROUTINE CUP_gf_GF2020(its,ite,kts,kte ,itf,ktf, mtp, nmp &
        ,cumulus           &
        ,ichoice           &
        ,entr_rate_input   &
        ,use_excess        &
        ...
        ! CK : OpenACC cannot use the 'trim' feature, so I'll use a character instead
        if (trim(cumulus) == 'deep')    cumulus_1 = 'd'
        if (trim(cumulus) == 'mid')     cumulus_1 = 'm'
        if (trim(cumulus) == 'shallow') cumulus_1 = 's'

        !
        !--- maximum depth (mb) of capping inversion (larger cap = no convection)
        !
        IF(ZERO_DIFF==1 .or. MOIST_TRIGGER==0) THEN
            if(cumulus_1 == 'd'   ) then ; cap_max_inc=20. ; endif ! cap_maxs=50.
            if(cumulus_1 == 'm'    ) then ; cap_max_inc=10. ; endif ! cap_maxs=50.
            if(cumulus_1 == 's') then ; cap_max_inc=25. ; endif ! cap_maxs=50.
        ELSE
            if(cumulus_1 == 'd'   ) then ; cap_max_inc=90. ; endif !--- test cap_maxs=10.  ; cap_max_inc=50
            if(cumulus_1 == 'm'    ) then ; cap_max_inc=90. ; endif !--- test cap_maxs=10.  ; cap_max_inc=50
            if(cumulus_1 == 's') then ; cap_max_inc=10. ; endif !--- test cap_maxs=25.  ; cap_max_inc=50
        ENDIF
        !
        !--- lambda_U parameter for momentum transport
        !
        if(cumulus_1 == 'd'   ) then;lambau_dp (:) = lambau_deep;  lambau_dn (:) = lambau_shdn ;endif
        if(cumulus_1 == 'm'    ) then;lambau_dp (:) = lambau_shdn;  lambau_dn (:) = lambau_shdn ;endif
        if(cumulus_1 == 's') then;lambau_dp (:) = lambau_shdn;  lambau_dn (:) = lambau_shdn ;endif
        ...
        !
        if( cumulus_1 == 'd' .and. use_random_num > 1.e-6) then
        ...
        !
        if(cumulus_1 == 'd'                         ) depth_min=1000.
        if(cumulus_1 == 'm' .or. cumulus_1 == 's') depth_min=500.
        !
        !--- max height(m) above ground where updraft air can originate
        !
        if(cumulus_1 == 'd'                         ) zkbmax=4000.
        if(cumulus_1 == 'm' .or. cumulus_1 == 's') zkbmax=3000.
        !
        ...
        !
        z_detr=1000.
        if(cumulus_1 == 'd'                         ) z_detr= 1000.
        if(cumulus_1 == 'm' .or. cumulus_1 == 's') z_detr= 300.
        !
        !- mbdt ~ xmb * timescale
        do i=its,itf
        ...
        !-- get the pickup of ensemble ave prec, following Neelin et al 2009.
        !
        call precip_cwv_factor(itf,ktf,its,ite,kts,kte,ierr,t,po,qo,po_cup,cumulus_1,p_cwv_ave)
        !
        ...
        DO i=its,itf
            ...
            if(ierr(i) == 0)then
                ...
                x_add = zqexec(i)
                call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),q_cup (i,kts:kte),rlll,k22(i),x_add)
                x_add = ztexec(i)
                call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),t_cup (i,kts:kte),tlll,k22(i),x_add)
                call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),p_cup (i,kts:kte),plll,k22(i))
                ...
                if(dzlcl >= 0.) then ! LCL found (if dzlcl<0 => not found)
                    call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),z_cup (i,kts:kte),zlll,k22(i))
                    loop0:       do k=kts,ktf
        ...
        if(USE_LCL .and. cumulus_1 == 'm')then
            do i=its,itf
                if(ierr(i).eq.0)then
        ...
        IF(ADV_TRIGGER==1 .and. cumulus_1 /= 's') THEN
            wkf = 0.02 ! m/s
            do i=its,itf
                if(ierr(i) /= 0) CYCLE
        ...
        do i=its,itf
            if(ierr(i) /= 0)cycle
            x_add = (xlv*zqexec(i)+cp*ztexec(i)) +  x_add_buoy(i)
            call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),he_cup (i,kts:kte),hkb (i),k22(i),x_add,Tpert(i,kts:kte))
            call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),heo_cup(i,kts:kte),hkbo(i),k22(i),x_add,Tpert(i,kts:kte))
        enddo
        !
            ...
            if(cumulus_1 /= 's') then
                ...
                !                            entr_rate_2d(i,k)=entr_rate(i)*(1.3-frh)*(qeso_cup(i,k)/qeso_cup(i,klcl(i)))**1.25
                    ...
                    !- v 1
                        ...
                        if(cumulus_1 == 'd') cd(i,k)=1.e-2*entr_rate(i)
                        if(cumulus_1 == 'm' ) cd(i,k)=0.75*entr_rate_2d(i,k)
                    endif
                enddo
        ...
        !--- DETERMINE THE LEVEL OF CONVECTIVE CLOUD BASE  - KBCON
        !
        CALL cup_cloud_limits(cumulus_1,ierrc,ierr,cap_max_increment,cap_max,heo_cup,heso_cup,qo_cup   &
        ...
        IF(cumulus_1 == 'm') THEN
            if(USE_INV_LAYERS) then
                ...
                call get_inversion_layers(cumulus_1,ierr,psur,po_cup,tn_cup,zo_cup,k_inv_layers,&
                                    dtempdz,itf,ktf,its,ite, kts,kte)
                do i=its,itf
                    if(ierr(i) /= 0)cycle
        ...
        do i=its,itf
            zuo(i,:)=0.
            if(ierr(i) /= 0) cycle
            CALL get_zu_zd_pdf(cumulus_1,trim(cumulus)//"_up",ierr(i),k22(i),ktop(i),zuo(i,kts:kte),kts,kte,ktf  &
        ...
        CALL get_lateral_massflux(itf,ktf, its,ite, kts,kte,min_entr_rate              &
                        ,ierr,ktop,zo_cup,zuo,cd,entr_rate_2d,po_cup          &
                        ,up_massentro, up_massdetro ,up_massentr, up_massdetr &
                        ,cumulus_1,kbcon,k22,kpbl,up_massentru,up_massdetru,lambau_dp)
        uc  =0.
        vc  =0.
        hc  =0.
        hco =0.
        do i=its,itf
            IF(ierr(i).eq.0)THEN
                do k=kts,start_level(i)
                    ...
                    call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),u_cup(i,kts:kte),uc(i,k),k22(i))
                    call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),v_cup(i,kts:kte),vc(i,k),k22(i))
                enddo
            ENDIF
        enddo
        !
        ...
        !--- get "c1d" profile ----------------------------------------
        if(cumulus_1 == 'd' .and. USE_C1D) then
            Do i=its,itf
                if(ierr(i) /= 0) cycle
        ...
        if(FIRST_GUESS_W .or. AUTOCONV == 4) then
            call cup_up_moisture_light(cumulus_1,start_level,klcl,ierr,ierrc,zo_cup,qco,qrco,pwo,pwavo,hco,tempco,xland   &
        ...
        call cup_up_moisture(cumulus_1,start_level,klcl,ierr,ierrc,zo_cup,qco,qrco,pwo,pwavo,hco,tempco,xland   &
        ...
        !
        !--- option to produce linear fluxes in the sub-cloud layer.
        if(cumulus_1 == 's' .and. use_linear_subcl_mf == 1) then
            do i=its,itf
                if(ierr(i) /= 0) cycle
                call get_delmix(kts,kte,ktf,xland(i),start_level(i),po(i,kts:kte) &
        ...
        call get_jmin(cumulus_1,itf,ktf,its,ite, kts,kte,ierr,kdet,ktop,kbcon,jmin,ierrc  &
            ,beta,depth_min,heso_cup,zo_cup,melting_layer)   
        !
        ...
        do i=its,itf
            zd(i,:)=0.
            if(ierr(i) /= 0) cycle
            call get_zu_zd_pdf(cumulus_1,"DOWN",ierr(i),kdet(i),jmin(i),zdo(i,:),kts,kte,ktf&
                        ,kpbl(i),k22(i),kbcon(i),klcl(i),po_cup(i,kts:kte),psur(i),xland(i),random(i))
        enddo
        !
        !---  calls routine to get lateral mass fluxes associated with downdrafts
        !
        call get_lateral_massflux_down(cumulus_1,itf,ktf, its,ite, kts,kte &
                                ,ierr,jmin,zo_cup,zdo,xzd,zd,cdd,mentrd_rate_2d      &
                                ,dd_massentro,dd_massdetro ,dd_massentr, dd_massdetr &
                                ,mentrd_rate,dd_massentru,dd_massdetru,lambau_dn)

        !---  calls routine to get wet bulb temperature and moisture at jmin
        !
        IF(USE_WETBULB == 1 .and. cumulus_1 /= 's' ) THEN
            do i=its,itf
                if(ierr(i) /= 0) cycle
                k = jmin(i)
        ...
        do i=its,itf
            bud(i)=0.
            if(ierr(i)/= 0 .or. cumulus_1 == 's')cycle
            i_wb=0
            !--for future test
            if(use_wetbulb==1) then
                !--option 1
        ...
        !
        call cup_dd_moisture(cumulus_1,ierrc,zdo,hcdo,heso_cup,qcdo,qeso_cup,     &
        ...
        if(cumulus_1=='m' .or. cumulus_1=='s') then
            wmeanx=3
            T_star=40.
        endif
        ...
        IF( (DICYCLE==1 .or. DICYCLE == 6) .or. (DICYCLE==0 .and. cumulus_1=='m') ) THEN
            iversion=0
        ...
        IF(DICYCLE==5 .or. DICYCLE==2) then
            ...
            PHY2:  DO step=fa,fa
                ...
                do i=its,itf
                    ...
                    call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),heo_cup_x(i,kts:kte),hkbo_x(i),k22(i),x_add,Tpert(i,kts:kte))
                enddo
                do i=its,itf
                    hco_x (i,:)=0.
                    IF(ierr(i) /= 0)cycle
        ...
        !
        IF(ADV_TRIGGER == 3 .and. cumulus_1 /= 's') THEN
            ...
            do step=1,2
                ...
                do i=its,itf
                    ...
                    call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),heo_cup_x(i,kts:kte),hkbo_x(i),k22(i))
            ...
            enddo
            !--- only for output
            if(cumulus_1 == 'd') AA0_(:) = daa_adv_dt(:)*3600. ! J/kg/hour
        ...
        !--- DETERMINE DOWNDRAFT STRENGTH IN TERMS OF WINDSHEAR
        !
        call cup_dd_edt(cumulus_1,ierr,us,vs,zo,ktop,kbcon,edt,po,pwavo, &
        ...
        do iedt=1,maxens2
            ...
            do i=its,itf
                ...
                !-ztexec and zqexec
                call get_cloud_bc(cumulus_1,kts,kte,ktf,xland(i),po(i,kts:kte),DELLAH (i,kts:kte),DELLAH_aver,k22(i))
            ...
            do nens=1,maxens
                do i=its,itf
                    ...
                    do k=kts,ktop(i)
                        ...
                        enddo
                    enddo
                    if(pr_ens(i,7).lt.1.e-6 .and. c0_mid > 0. .and.  cumulus_1 /= 's' )then
                        ierr(i)=18
        ...
        !--- FEEDBACK
        !
        call cup_output_ens_3d(cumulus_1,xff_shal,xff_mid,xf_ens,ierr,dellat,dellaq,          &
        ...
        call get_precip_fluxes(klcl,kbcon,ktop,k22,ierr,xland,pre,xmb  &
                        ,pwo,pwavo,edto,pwevo,pwdo,t_cup,tempco          &
                        ,prec_flx,evap_flx                               &
                        ,itf,ktf,its,ite, kts,kte)
        ...
        !
        if(USE_REBCB == 1)                                                              &
            call rain_evap_below_cloudbase(cumulus_1,itf,ktf, its,ite, kts,kte,ierr,kbcon,ktop&
        ...
        if(use_cloud_dissipation >= 0.)                                             &
            call cloud_dissipation(cumulus_1,itf,ktf, its,ite, kts,kte,ierr,kbcon,ktop    &
        ...
        !
        if( LIGHTNING_DIAG == 1 .and. cumulus_1 == 'd') then
            call cup_up_cape(cape,z,zu,dby,gamma_cup,t_cup,k22,kbcon,ktop,ierr &
                        ,tempco,qco,qrco,qo_cup,itf,ktf,its,ite, kts,kte   )
        ...
        IF(USE_TRACER_TRANSP==1)  THEN
            ...
            call get_incloud_sc_chem_up(cumulus_1,mtp,se_chem,se_cup_chem,sc_up_chem,pw_up_chem,tot_pw_up_chem      &
            ...
            ! b) chem - downdraft
            call get_incloud_sc_chem_dd(cumulus_1,mtp,se_chem,se_cup_chem,sc_dn_chem,pw_dn_chem ,pw_up_chem,sc_up_chem &
            ...
            do i=its,itf
                ...
                IF(USE_FLUX_FORM == 1 .and. alp1 == 0. ) THEN
                    ...
                    !- include evaporation
                    if(USE_TRACER_EVAP == 1 .and. cumulus_1 /= 's') then
                        do k=kts,ktop(i)
                            dp=100.*(po_cup(i,k)-po_cup(i,k+1))
                    ...
                    !- include scavenging
                    if(USE_TRACER_SCAVEN > 0 .and. cumulus_1 /= 's') then
                        do k=kts,ktop(i)
                            dp=100.*(po_cup(i,k)-po_cup(i,k+1))
                            out_chem(:,i,k) = out_chem(:,i,k) &
        ...
        if(wrtgrads) then
            if(cumulus_1 == 'd'   ) then ; cty='1'; nvarbegin =  0 ;endif
            if(cumulus_1 == 's') then ; cty='2'; nvarbegin =101 ;endif
            if(cumulus_1 == 'm'    ) then ; cty='3'; nvarbegin =201 ;endif
            do i=its,itf
                !if(ierr(i).eq.0) then
                !- 2-d section
