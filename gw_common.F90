   ...
   do k = 1, pver+1
      do i = 1, ncol
         ...
         if (k == 1) then
            ...
            dtdp = pint(i,k-1) / (rair * 0.5 * (t(i,k-2)+t(i,k-1)))  &
                  * (t(i,k-1)-t(i,k-2)) / ( pmid(i,k-1)-pmid(i,k-2) ) ! * p%rdst(i,k-1)
            n2 = gravit*gravit/(0.5 * (t(i,k-2)+t(i,k-1))) * (1./cpair - dtdp)
            ni(i,k) = sqrt(max(n2min, n2))
            
         else
         ! Interior points use centered differences.
            ! ti(i,k) = 0.5_r8 * (t(i,k-1)+t(i,k))
            rhoi(i,k) = pint(i,k) / (rair * 0.5 * (t(i,k-1)+t(i,k)))
            dtdp = (t(i,k)-t(i,k-1)) / ( pmid(i,k)-pmid(i,k-1) ) ! * p%rdst(i,k-1)
            n2 = gravit*gravit/(0.5 * (t(i,k-1)+t(i,k))) * (1./cpair - rhoi(i,k)*dtdp)
            ni(i,k) = sqrt(max(n2min, n2))
         endif
      enddo
   enddo
...
subroutine momentum_fixer(ncol, pver, tend_level, p, um_flux, vm_flux, utgw, vtgw)
 ...
 subroutine energy_change(ncol,pver, dt, pdel, u, v, dudt, dvdt, dtdt, de)
   ...
   de = 0.0
   do k = 1, pver
      de = de + pdel(:,k)/gravit * (cpair*dtdt(:,k) + &
