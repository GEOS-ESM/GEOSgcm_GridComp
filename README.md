...
3. Run `./TEST_MOIST /discover/nobackup/projects/geosongpu/physics_standalone_data/moist/gfdl_cloud_microphysics/ <Dataset Number>` to execute the standalone.
...
## Other Notes
- More work must be done to ensure that the standalone verifies with the comparison dataset when computing either on the CPU or GPU.
- OpenACC directives can offload portions of standalone code to execute on the GPU.
    ...
    - Versions 10.4.0 and 11.4.0 of the `gfortran` compiler has been tested to build the standalone with OpenACC directives; however, a `gfortran` compiled binary cannot successfully execute on device.
