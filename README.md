...
## Other Notes
- Currently, this standalone has been verified to execute on CPUs and GPUs.  GPU execution is performed via OpenACC pragma calls.  The `nvfortran` compiler has been tested up to version 23.11 and can build and execute via OpenACC.  The `gfortran` compiler has been tested up to version 11.4.0 and can build the standalone with OpenACC calls; however, a `gfortran` compiled binary cannot execute on device.
