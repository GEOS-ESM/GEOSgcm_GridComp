...
!$acc declare copyin(UTBL,TYPE,FIRST,ESTFRZ,ESTLQU,ESTBLE,ESTBLW,ESTBLX,TMINLQU,TMINICE)
...
#undef  PX
       ...
       function QSATLQU0_1(TL,PL,DQ) result(QS)
!$acc routine seq
         real,              intent(IN) :: TL
         real, optional,    intent(IN) :: PL
         real, optional,    intent(OUT):: DQ
...
#undef  PX
       ...
       function QSATICE0_1(TL,PL,DQ) result(QS)
!$acc routine seq
         real,              intent(IN) :: TL
         real, optional,    intent(IN) :: PL
         real, optional,    intent(OUT):: DQ
...
!$acc update device(UTBL,TYPE,FIRST,ESTFRZ,ESTLQU,ESTBLE,ESTBLW,ESTBLX,TMINLQU,TMINICE)
...
! NOTE : This loop could potentially be done using acc loop vector if
!        "T" is declared to be private
          do I=1,TABLESIZE

            T = (I-1)*DELTA_T + TMINTBL

            ESTBLW(I) = QSATLQU0_1(T)

            if(T>ZEROC) then
               ESTBLE(I) = ESTBLW(I)
            else
               ESTBLE(I) = QSATICE0_1(T)
            end if
         ...
         ESTFRZ = QSATLQU0_1(ZEROC  )
         ESTLQU = QSATLQU0_1(TMINLQU)
